...
- Forge 1.20.1 默认使用 **Mojang official（mojmap）**：`mappings channel: 'official', version: mapping_version`
- `mapping_version` 在 official 通道下与 Minecraft 版本字符串一致（1.20.1），**不是** MCP snapshot 号；不要把 MCP 通道的日期戳填成另一个 MC 版本
- **禁止**在 `build.gradle` 中切换到 `yarn`（除非用户明确要求且项目已适配）
- official 映射将混淆名映射为 Mojang 可读名，如 `getHealth`
