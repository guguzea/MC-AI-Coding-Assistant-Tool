...
- 本仓库 `scaffold/` 的 wrapper 三件套**不是** MDK 原件（props 声明 8.5，jar 来源未核实，逐条登记在 `mcp-server/data/wrapper-jars.json` 的 `unverifiedWrapper`），只当目录结构参考；要真构建请用 `download_official_mdk` 取官方 MDK 的 wrapper。
