...
public MyEntity(EntityType<?> type, Level level) {
    super(type, level);
    this.getAttribute(Attributes.MAX_HEALTH).setBaseValue(100.0); // ❌ 太早，Attributes 系统未就绪
}
```

**错误症状**：`NullPointerException` 或属性不生效

**正确方案**：使用 `DeferredRegister<Attribute>` + modEventBus（在 mod 构造函数中完成）
...
public void load(CompoundTag nbt) {
    super.load(nbt);
    BlockState state = level.getBlockState(pos); // ❌ level 可能为 null 或 world 未完全加载
...
**错误症状**：`NullPointerException`，游戏崩溃在 `BlockEntity.load`

**正确方案**：在 `onLoad()` 中处理需要世界数据的逻辑（`load()` 仅用于读写 NBT）

```java
// load() 只读写 NBT，不访问 world
@Override
public void load(CompoundTag nbt) {
    super.load(nbt);
...
// onLoad() 在 BlockEntity 进入 world 后调用，此时 level 已可用
@Override
public void onLoad() {
    super.onLoad();
...
### ❌ 忘记在 `BlockEntity.setRemoved()` 中清理监听器
...
**错误症状**：内存泄漏，World 不释放，卸载区块后大量对象无法 GC
...
**正确方案**：使用 `CompoundTag` 或自定义 `BinaryBuffer` 批量序列化
...
**错误症状**：`IllegalStateException: Do not modify the game from the render thread`
...
**正确方案**：
- `FMLClientSetupEvent` 只用于注册按键绑定（KeyMapping）和渲染器
- 游戏逻辑使用网络包或服务端事件
...
player.getCapability(ModCapabilities.MY_CAP).ifPresent(cap -> {
    cap.setData(someData); // 如果 Capability 未附加，会走到这里但数据丢失
});
```
...
if (player.getCapability(ModCapabilities.MY_CAP).isPresent()) {
    player.getCapability(ModCapabilities.MY_CAP).ifPresent(cap -> {
...
### ❌ Mixin 中在 `@Inject` 的构造函数里修改 final 字段
...
public class MixinEntity {
    ...
    private void init(CallbackInfo ci) {
        this.someFinalField = value; // ❌ 不能修改 final 字段
    }
}
```
...
**正确方案**：Mixin 中避免修改 final 字段，在 `@Inject` 方法的参数中操作
...
2. 检查 `mods.toml` 语法和 mod ID 是否一致
3. 检查资源文件路径是否全小写
4. 检查是否混用了服务端和客户端线程
5. 检查 `enqueueWork` 是否正确使用
6. 检查 Registry 注册是否通过事件系统
7. 崩溃 / 缺前置 / 软依赖 / 机器 GUI 实务 → MCP `crash_analyze`，或 `search_community_docs`（社区库，不替代官方文档）
8. **依据社区短文写方案时若不清楚** → 打开短文给出的原文 URL / 官方文档（见仓库 `community_knowledge/AGENT_USAGE.md`），禁止臆造 API

### 机器 GUI 相关（社区短文已覆盖）

| 症状 | 查 |
|------|-----|
| GUI 进度条不同步 / Screen 直接读 BE | `authored/menu-screen-sync` |
| 漏斗管道进不出 | `authored/itemhandler-capability` |
| WORKING 狂闪或卡顿 | `authored/machine-be-gui-working`（仅变化时 setBlock） |
