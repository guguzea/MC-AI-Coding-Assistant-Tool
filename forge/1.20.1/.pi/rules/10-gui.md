---
description: Client-Server separation and GUI (Forge 1.20.1)
---

# Client-Server separation and GUI (Forge 1.20.1)
...
Player interacts
      │
      ▼
Block#use (Server) ──► creates MenuProvider ──► player.openMenu (Server)  // Player/ServerPlayer#openMenu(MenuProvider)
                                                       │
                                                       ▼
...
// In Menu constructor —— AbstractContainerMenu 只有 (MenuType, int) 一个构造器
super(menuType, windowId);
this.addDataSlots(dataSlots);   // ContainerData 须显式加入才会同步；inventory / extraData 在构造器体内自行 addSlot(...)
