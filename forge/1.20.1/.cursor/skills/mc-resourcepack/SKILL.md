...
# mc-resourcepack（1.20.1）

> 一手来源：`forge/1.20.1/knowledge/common/resourcepack-format.md`（pack_format 对齐官方 1.20.1-47.4.10 MDK，sha256 钉在 `mcp-server/data/mdk-checksums.json`）。
...
1.20–1.20.1 的 pack_format = **15**，资源包与数据包相同（1.20.3 起才再分开：资源 22 / 数据见 mc-datapack）。
...
- `pack_format` 填错（本版 = **15**，不是 12/13/22）。

## 下一步

- 完整速查：`forge/1.20.1/knowledge/common/resourcepack-format.md`。
- 反模式库：`forge/1.20.1/knowledge/antipatterns/`。
