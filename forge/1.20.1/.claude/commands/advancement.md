...
> `forge-docs/1.20.1/processed/resources_server_advancements.md`、
> `forge-docs/1.20.1/processed/datagen_server_advancements.md`。
> 页内没有的签名一律留 `TODO(未核实)`；**禁止**用 `data/forge_1.20.4` 或 NeoForge 文档补全。
> 本仓这两页与 1.20.4 档同名页面**逐字节相同**（`diff` 实测），所以「1.20.1 与 1.20.4 有差异」这个假设在 advancement 面上**不成立**——但这不代表可以跨档抄，仍以本档路径为唯一出处。

## JSON 结构（页内给出的部分）

进度在原版里全部数据驱动：不需要模组也能新增进度，只要一个数据包。页内只给出 `criteria` / `requirements` / `rewards` 三块的确切字段，其余顶层键（`parent`、`display`、`sends_telemetry_event` 一类）**本页未出现** ⇒ `TODO(未核实)`。
...
完整的进度 JSON 格式本页**不自己维护**，而是明确指向 Minecraft Wiki `Advancement/JSON_format`；trigger 的 JSON 格式指向同页 `#List_of_triggers`。要写这两个面的细节就必须去开那两个链接（见 `community_knowledge/AGENT_USAGE.md` 同源纪律：指路不等于入库，禁止凭记忆填）。
...
原版自定义 trigger 的名字清单在 `CriteriaTriggers` 里，页内只给出这个类名，没有逐项列出 ⇒ 想知道某个具体 trigger 名（例如原版某个 `player_interacted` 之类）**本页无证据**，改走 `search_forge_docs(version="1.20.1", query="CriteriaTriggers")` 或反编译。
...
最后加一个「拿当前数据状态、返回是否满足条件」的方法。玩家侧条件已经由 `SimpleCriterionTrigger#trigger(ServerPlayer, Predicate)` 检查过了，所以这里只查实例自己存的条件下；页内示例签名 `#matches`：
...
// ⇒ // TODO(未核实)：register(...) 的参数顺序/返回值请核 `get_minecraft_source` 或用户自备 forge 1.20.1 jar。
```
...
- 条件性加载 / 默认值：页内指向 `../conditional/#implementations`，本档 `processed/` 无 `conditional` 页（实测目录清单无此文件）⇒ 缺来源，不写。
- 进度 Tab / 图标 / 背景图、`announce_to_chat`、`show_toast` 等显示开关：本档语料无 ⇒ 不写。

## 相关

- 注册（含「不属于 Forge registry 的注册表」一节）：`01-registry.mdc`、`mc-registry`；同档页 `data/forge_1.20.1/forge-docs/1.20.1/processed/concepts_registries.md`
...
- 反模式：`forge/1.20.1/knowledge/antipatterns/registry.md`、`09-anti-patterns.mdc`
- 全文核对：`search_forge_docs(version="1.20.1", query="advancement")`，或直接读上面声明的两个盘上页面
