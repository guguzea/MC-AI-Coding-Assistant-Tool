...
本件仍是**技能骨架**（forge 1.20.1）。本档工程不走 Yarn 映射，因此本仓的 mojmap ↔ Yarn 对照表**对本件不适用**（不要把 Fabric 档的 Yarn 名当 Forge 名写）。本件触发词：`ForgeEnergy` / `IFNFEnergyStorage`。
...
- 本件触发词在本档语料 0 命中 = **该面在本档没有一手教程正文**（原因随主题与版本不同：能量/村民交易这类 Forge 官方教程本就不写，GameTest 这类是该版本还没有该系统）。需要签名时 `search_forge_docs version=1.20.1 query=…`，或用户自备 jar 走 `query_loader_api` / 反编译；0 命中就留 `// TODO(未核实)`。
