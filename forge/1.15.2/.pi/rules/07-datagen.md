...
- `shapedRecipe` / `patternLine` / `key` / `addCriterion` / `build`
- 熔炉：`CookingRecipeBuilder.smelting`，不要 `FurnaceRecipeProvider`
- 必须 `build(consumer)`，不是 `.save()`
...
```
IF 配方有固定形状（工具、武器等）
  → ShapedRecipeBuilder.shapedRecipe + patternLine

IF 配方成分无固定位置（药水、染料混合等）
  → ShapelessRecipeBuilder.shapelessRecipe + addIngredient
...
- ❌ `modLoc()` 与 `mcLoc()` 用反
- ❌ `SimpleCookingRecipeBuilder` / `.save()` — 那是 1.16+ 官方映射名
