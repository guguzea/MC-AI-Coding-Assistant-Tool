...
```
IF 自然方块（石头、木头、草）
  → Material.ROCK / Material.WOOD   // TODO(未核实)：Material.GRASS 未在 1.15.2 语料出现，待核实

通道口径：以上是 MCP 名（官方 1.15.2 docs 页写 `Material.ROCK`）。official 通道该常量为 `STONE`
（本档 scaffold 以 `net.minecraft.block.material.Material.STONE` 真机构建通过，2026-09-11），两套名字不可混用。
