...
EntityType.Builder.create(MyEntity::new, MobCategory.CREATURE)
    ...
    .trackerTrackingRange(int range)              // 客户端追踪范围
...
IF 自然生成的动物/怪物
  ...
  → MobCategory.WATER_CREATURES（水生生物）
...
IF 乘坐实体
  → 使用 MobCategory.MISC 或在实体类中覆盖 getType().getClassification()
```
...
> 注意：`ForgeRegistries.ATTRIBUTES` 是注册表，`DeferredRegister.create()` 需要传入 `IRegistry` 而非直接的 `Attribute`。属性注册在 mod 构造函数中通过 modEventBus 完成，而不是在 `FMLCommonSetupEvent` 中。
