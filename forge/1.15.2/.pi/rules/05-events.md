...
IF 监听玩家右键点击方块
  ...
  → 注意：此事件双侧触发（客户端+服务端），改世界前用 `world.isClientSide` 守卫（2026-09-14 javap 实测：official 1.15.2 的 `World` 有 `isClientSide` 字段与方法；`isClientSide` 是 MCP 名，语料 `concepts_sides.md:26` 是 MCP 写法）
...
IF 监听配方解锁（合成/烧炼/烟熏等）
  → 没有 FurnaceSmeltEvent（那是 Bukkit）；用熔炉配方 JSON
...
| 监听烧炼 | 熔炉配方 JSON | Forge 无 FurnaceSmeltEvent |
| 监听药水效果 | `PotionEvent` | 多个子事件 |
| 监听实体生成 | `EntityJoinWorldEvent` | 注意不要做重操作 |
| 修改物品 NBT | `AnvilUpdateEvent` | |
