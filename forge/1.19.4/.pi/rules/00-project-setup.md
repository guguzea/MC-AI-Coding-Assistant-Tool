...
- **必须使用 Java 17**（Forge 1.19.4 要求；Java 21 仅在 Minecraft 1.20.6+ 才需要）
...
IF 这是新项目（没有 build.gradle）
  → 使用 forge/1.19.4/scaffold/ 中的模板生成项目骨架
...
## 示例：正确的 build.gradle 结构（Forge 1.19.4）
...
modId = "forge"               # 必需
mandatory = true              # 必需
versionRange = "[45,)"        # 必需：Forge 版本范围（证据：scaffold/gradle.properties:10 forge_version_range=[45,)）
...
- `loaderVersion` 的格式：`"[45,)"` 表示 Forge / FML 45 及以上（本档实值证据：`scaffold/gradle.properties:11` `loader_version_range=[45,)`，同文件 `:10` `forge_version_range=[45,)`）
