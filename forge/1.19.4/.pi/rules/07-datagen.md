...
- `LootItemFunction` 应用修饰器（如 `setCount`、`ApplyExplosionDecay.explosionDecay`）
...
IF 生成方块标签（哪些方块可被某工具挖掘）
  → 使用 net.minecraft.data.tags.TagsProvider 子类（实现 #addTags；语料 datagen.md:62）
  → 放到 data/{modid}/tags/blocks/

IF 生成物品标签
  → 使用 net.minecraft.data.tags.ItemTagsProvider
