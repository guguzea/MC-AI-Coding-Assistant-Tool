...
- **物品的创造模式标签通过 `CreativeModeTabEvent.BuildContents` 注册**（在 Mod 构造函数的 modEventBus 上），不写在 `Item.Properties` 里
...
> **重要**：物品不写在 `Item.Properties` 中，而是通过 `CreativeModeTabEvent.BuildContents` 注册。
...
//                           ^^^ int 参数：基础攻击加成
//                                    ^^^^ float 参数：攻击速度
// 创造模式标签通过 CreativeModeTabEvent.BuildContents 注册到 CreativeModeTabs.COMBAT
```
...
// 创造模式标签通过 CreativeModeTabEvent.BuildContents 注册到 CreativeModeTabs.FOOD_AND_DRINKS
```
