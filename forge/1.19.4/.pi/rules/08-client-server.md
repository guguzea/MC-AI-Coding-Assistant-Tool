...
> 注意：`InputEvent.Key` 在 Forge 1.19.4 中**已废弃**，应使用 `ClientTickEvent` + `consumeClick()`。
...
> 注意：
> - 1.19.4 的 BlockEntity 同步推荐使用 `getUpdateTag()` / `handleUpdateTag()`（无需手动实现 `getUpdatePacket`）
> - `setChanged()` 标记数据已修改
