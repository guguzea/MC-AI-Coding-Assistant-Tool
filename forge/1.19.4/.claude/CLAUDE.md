# Forge 1.19.4 — AI Coding Assistant

This file contains Forge 1.19.4 mod development knowledge for AI coding assistants that support `CLAUDE.md`.
When you detect a Forge 1.19.4 project, read this file first.
...
### Registry
- **Always use DeferredRegister** — the official Forge 1.19.4 recommended pattern
...
- Parchment provides parameter names and javadoc
...
- **MobCategory** is used for entity spawn groups (`net.minecraft.world.entity.MobCategory`); `SpawnGroup` is a Yarn (Fabric) name that exists in **no** Forge/Mojmap version — there is no "some Forge minor version renamed it" event, so never rename on that basis
