...
| Mappings | **Parchment**（`1.19.4-2023.06.26`，带参数名和文档） |
...
→ IF 项目中存在 src/main/resources/META-INF/mods.toml
    → IF mods.toml 中 modLoader = "javafml"
        → IF build.gradle 中 minecraft = "1.19.4"
            → 继续加载本规则集（Forge 1.19.4）
...
1. **推荐使用 DeferredRegister**：`DeferredRegister` 是 Forge 官方推荐的注册方式，自 Forge 1.18 起即可用于所有注册表，1.19.4 完全支持
