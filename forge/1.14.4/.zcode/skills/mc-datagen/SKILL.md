...
public class ModBlockStatesProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        simpleBlock(ModBlocks.MY_BLOCK.get(), // TODO(未核实：simpleBlock 未在 forge 1.14.4 语料命中，需 search_forge_docs 复核或反编译)
            models().cubeAll(ModBlocks.MY_BLOCK.getId().getPath(), modLoc("block/my_block")) // TODO(未核实：cubeAll / modLoc 未在 forge 1.14.4 语料命中，需 search_forge_docs 复核或反编译；本行只有 models() 有出处 data/forge_1.14.4/forge-docs/1.14.4/processed/datagen_modelproviders.md:21)
        );
    }
}
```
...
public class ModItemModelsProvider extends ItemModelProvider {
    ...
    protected void registerModels() {
        withExistingParent(ModItems.MY_BLOCK_ITEM.getId().getPath(), modLoc("block/my_block")); // TODO(未核实：withExistingParent / modLoc 未在 forge 1.14.4 语料命中，需 search_forge_docs 复核或反编译)
    }
}
```
