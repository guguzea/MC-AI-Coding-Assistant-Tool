...
├── client/
│   ├── ClientSetup.java          # 仅客户端（@Mod.EventBusSubscriber(value=Dist.CLIENT)）
│   └── rendering/
...
IF 代码涉及输入（KeyBinding、MouseInput、KeyboardInput）
  → 客户端
  → 放在 client/ 包下
  → 使用 @OnlyIn(Dist.CLIENT)
...
IF 只需在客户端显示额外信息（如 GUI 覆盖层）
  → 使用 ClientTickEvent / RenderOverlayEvent
  → 检查 OverlayMessage 来判断状态

IF 需要在客户端渲染自定义模型
  → 继承 Model 或 ItemModel
  → 在 ClientSetup 中注册
...
// KeyBinding 定义（静态字段，在 ClientRegistry 中注册）
public static final KeyBinding TEST_KEY = new KeyBinding(
    "key.examplemod.test",
...
## 示例：服务端数据同步（EntityDataManager）

```java
// entities/MyEntity.java（服务端设置，客户端自动同步）
public class MyEntity extends LivingEntity {
    private static final EntityDataManager<Integer> DATA_CUSTOM =
...
public class MyTileEntity extends TileEntity {
    ...
    // 在服务端逻辑中修改数据后，同步到客户端
    public void modifyData(int newValue, World world, BlockPos pos) {
...
> 注意：
> - 1.14.4 的 TileEntity 同步使用 `getUpdateTag()` / `handleUpdateTag()`（无需手动实现 `getUpdatePacket`）
> - `markDirty()` 标记数据已修改
