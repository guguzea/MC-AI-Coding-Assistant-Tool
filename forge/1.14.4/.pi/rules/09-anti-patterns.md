...
public MyEntity(EntityType<?> type, World world) {
    super(type, world);
    this.getAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(100.0); // ❌ 太早，Attributes 系统未就绪
}
```

**错误症状**：`NullPointerException` 或属性不生效

**正确方案**：在 `registerAttributes()` 里设置基值，不要 `ForgeRegistries.ATTRIBUTES`（1.16+）。
...
**正确方案**：在 `onLoad()` 中处理需要世界数据的逻辑（`read()` 仅用于读写 NBT）
...
// onLoad() 在 TileEntity 进入 world 后调用，此时 world 已可用
@Override
public void onLoad() {
    super.onLoad();
...
### ❌ 忘记在 `TileEntity.invalidate()` 中清理监听器
...
```java
// 错误：1.12 SimpleImpl 的 MessageContext / getServerHandler
public static void handle(MyMessage message, MessageContext ctx) {
...
**错误症状**：本档网络是 `SimpleChannel`，没有 SimpleImpl；网络线程改世界会崩。
...
public static void handle(MyMessage message, Supplier<NetworkEvent.Context> ctx) {
    ctx.get().enqueueWork(() -> {
        ...
        world.setBlockState(player.getPosition(), Blocks.AIR.getDefaultState());
    });
...
public void onServerTick(TickEvent.ServerTickEvent event) {
    if (event.phase == TickEvent.Phase.END) {
        for (Entity entity : world.getEntities()) { // ❌ 每 tick 遍历所有实体
            processHeavy(entity);
        }
    }
}
