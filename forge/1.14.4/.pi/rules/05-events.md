...
DistExecutor.runWhenOn(Dist.CLIENT, () -> () -> clientMethod());

// 服务端执行
DistExecutor.runWhenOn(Dist.DEDICATED_SERVER, () -> () -> serverMethod());
```

> **注意**：本档 DistExecutor 只有 `runWhenOn` / `callWhenOn` / `runForDist`，没有 `unsafe*`。
...
IF 监听玩家右键点击方块
  ...
  → 注意：此事件双侧触发（客户端+服务端），改世界前用 world.isRemote 守卫
...
IF 监听配方解锁（合成/烧炼/烟熏等）
  → `PlayerEvent.ItemCraftedEvent`（玩家完成合成时）
...
IF 监听 Capability 附加
  → AttachCapabilitiesEvent<Entity / TileEntity / ItemStack>
...
| 添加合成配方 | `PlayerEvent.ItemCraftedEvent` 或数据生成器 | 优先用数据生成器 |
| 监听烧炼 | `FurnaceSmeltEvent` | 熔炉烧炼时触发 |
| 监听药水效果 | `PotionEvent` | 多个子事件 |
| 监听实体生成 | `EntityJoinWorldEvent` | 注意不要做重操作 |
| 修改物品 NBT | `PlayerEvent.ItemCraftedEvent` 或 `AnvilUpdateEvent` | |
...
public class ClientSetup {
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        // KeyBinding 注册
        ClientRegistry.registerKeyBinding(EXAMPLE_KEY);
    }
}
```

KeyBinding 注册应使用 `ClientRegistry.registerKeyBinding()`，而非直接调用 `KeyBinding`。
...
> 关键点：`AttachCapabilitiesEvent` 可以在 Entity、TileEntity、ItemStack 上附加 Capability，每个只能附加一次（ID 唯一）。`deserializeNBT` 在 `isInvalid()` 返回 true 时不会被调用。
