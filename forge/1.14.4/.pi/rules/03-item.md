...
new Item.Properties()
    ...
    .containerItem(Item copyFrom)                  // 消耗后保留物品
...
new Item.Properties()
    .food(new Food.Builder()
        ...
        .saturation(float modifier)     // 饱和度修正
        .meat()                         // 狗可食用
...
- 工具/盔甲使用 `IItemTier` 时，需要在构造中传入 `.defaultMaxDamage(durability)`
- **物品不显示在创造模式标签中**（1.14.4 没有 CreativeTabs，需要使用 ItemGroup）
...
IF 需要工具（剑、镐、斧、铲）
  ...
  → 需要定义 IItemTier
  → 需要在 constructor 传入：speed, damage, durability, tier

IF 需要盔甲
  → extends ArmorItem
  → 需要定义 IArmorMaterial
  → 需要在 constructor 传入：material, slotIndex, properties
...
IF 需要附魔书以外的附魔物品
  → 不支持，Forge 1.14.4 附魔只能通过附魔台/附魔桌
```
...
public class ModSetup {
    @SubscribeEvent
    public static void onSetup(FMLSetupEvent event) {
        // 设置创造模式物品组
    }
}
```
...
public class MyItem extends Item {
    ...
    @Override
    public ActionResult onItemUse(UseOnContext context) {
        // 右键点击方块时的逻辑
