...
- 无需实现 `IMessageHandler`，处理器通过 `registerMessage` 的第 5 参数注入
- 消息类必须有无参构造函数（用于反序列化）
...
IF 需要服务端主动推送给客户端（服务器通知客户端）
  → 使用 ServerToClientPacket 或 SimpleNetworkWrapper.sendTo()
...
│                     │
│  INSTANCE.sendToAll() │──────────→ 所有客户端
│                     │
│  客户端发来消息 ────────→│ 客户端发来消息
└─────────────────────┘
```

- `sendTo(Packet, EntityPlayerMP)` — 服务端发送给指定玩家
- `sendToAll(Packet)` — 服务端广播给所有玩家
- `sendToAllTracking(Packet, Entity)` — 发送给追踪该实体的玩家
- 客户端直接调用 `INSTANCE.sendToServer(Packet)` — 客户端发送给服务端
...
public class MyMessageHandler implements IMessageHandler<MyMessage, IMessage> {
    ...
    public IMessage onMessage(MyMessage msg, MessageContext ctx) {
        ctx.getServerHandler().execute(() -> {
            // 始终检查逻辑端
            if (ctx.getServerHandler().player.getServerWorld().isRemote) {
                // 这是客户端收到消息
            }
        });
        return null;
...
public class NetworkHandler {
    ...
    public static void register() {
        ...
        INSTANCE.registerMessage(
            ...
            MyOpenGuiHandler::onMessage
        );
    }

    // 服务端发送消息给指定玩家
    public static void sendTo(EntityPlayerMP player, MyOpenGuiMessage message) {
...
> 重要：收到消息后**永远不要**直接修改世界或实体。必须用 `ctx.getServerHandler().execute()` 将操作排队到主线程执行。
