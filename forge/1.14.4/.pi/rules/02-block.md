...
- 禁止重写 `use`、`onDestroyedByPlayer` 等与物理交互相关的旧版方法（Forge 1.14+ 已在大部分方法上标注 `@Deprecated`）
...
Block.Properties.create(Material material)
    ...
    .hardnessAndResistance(float hardness)                    // 硬度相同，抗爆性 = 硬度 * 5
    .harvestLevel(int level)                                 // 挖掘等级（0=木,1=石,2=铁,3=钻）
    ...
    .lightValue(int value)                                    // 发光等级 0-15
```
...
- TileEntity 的 `write`/`read` 方法处理 NBT 序列化（使用 `NBTTagCompound`，不是 `CompoundNBT`）
...
IF 需要存储数据（箱子、熔炉等）
  ...
  → 需要注册 TileEntityType
  → 需要在 mods.toml 中添加 tileEntity 字段

IF 需要可交互（右键打开 GUI、触发事件）
  → 基础方块 + onBlockActivated 事件
  → 或使用 IHasMenu + ITickableTileEntity 实现 GUI
...
IF 需要特殊渲染（多面材质、透明度、动画）
  → 使用 ISimpleBlockRenderingHandler 的流体方块
...
IF 自然方块（石头、木头、草）
  ...
  → 需要 .harvestLevel() + .requiresCorrectToolForDrops() 时用 ROCK/WOOD
...
IF 需要存储玩家数据（容器内容、熔炉燃料/物品）
  → 必须使用 TileEntity
  → 在方块类重写 createTileEntity()
  → 注册时先注册方块再注册 TileEntityType

IF 需要每 tick 逻辑（自动机、计时器）
  → 必须使用 TileEntity + ITickable
