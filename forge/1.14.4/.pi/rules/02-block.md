---
description: 02 — 方块开发
---

# 02 — 方块开发

> 适用版本：Forge 1.14.4

---

## 约束

### Block 子类规范

- 方块类必须继承 `Block`（`net.minecraft.block.Block`）
- 推荐使用 `Block.Properties.create(Material)` 创建属性
- 右键交互重写 `onBlockActivated`（返回 `ActionResultType`）。1.14.4 **没有**后期 Mojmap `use`

### Block.Properties 常用配置

来源：ForgeJavaDocs-NG `1.14.4-28.2.23` 的 `Block.Properties`。

```java
Block.Properties.create(Material material)
    .hardnessAndResistance(float hardness, float resistance)  // 硬度和抗爆性
    .hardnessAndResistance(float hardness)                    // 抗爆性 = 硬度 * 5
    .harvestLevel(int level)                                 // 挖掘等级
    .harvestTool(ToolType tool)                              // 所需工具类型
    .doesNotBlockMovement()                                   // 不阻挡实体移动
    .slipperiness(float slipperiness)
    .sound(SoundType type)
    .lightValue(int value)                                    // 发光 0-15
    .tickRandomly()
    .variableOpacity()
    .noDrops()
    .lootFrom(Block block)
```

另有 `create(Material, DyeColor)` / `create(Material, MaterialColor)` / `from(Block)`。

> 1.14.4 **没有** `requiresCorrectToolForDrops()`、**没有** `notSolid()`、**没有** `BlockBehaviour.Properties.of()`。

### 注册约束

- **禁止**通过 `new Block(...)` 构造后直接 `public static final` 使用
- 必须通过 `DeferredRegister<Block>` 注册（参见 `01-registry.mdc`）
- 注册名称必须全小写
- `BlockItem` 需要单独注册（与方块使用相同 registry name）

### 方块实体（TileEntity）约束

- 有状态或需要持久化的方块必须使用 TileEntity（而非仅靠 Block）
- 方块侧重写 `hasTileEntity(BlockState)` + `createTileEntity(BlockState, IBlockReader)`（`IForgeBlock`）
- TileEntity 的 `write(CompoundNBT)` **返回** `CompoundNBT`；读入 `read(CompoundNBT)`。不要用 1.12 的 `NBTTagCompound`
- **禁止**在 `read` 中直接读取世界数据（会导致 NPE）

### BlockState 配置

- 使用 `stateContainer` 定义所有方块状态属性
- 使用 `defaultBlockState()` 设置默认状态
- `getStateForPlacement` 处理放置时的状态变化

---

## Decision Flow

### Decision: 需要什么类型的方块？

```
IF 需要可放置物品、不可交互的基础方块
  → 基础方块（extends Block）
  → 注册后注册同名 BlockItem
  → 创建 blockstates JSON 和 models JSON

IF 需要存储数据（箱子、熔炉等）
  → 方块实体方块（Block + TileEntity）
  → 方块类重写 hasTileEntity / createTileEntity
  → 注册 TileEntityType
  → 不要在 mods.toml 里写伪造的 tileEntity 字段

IF 需要可交互（右键打开 GUI、触发事件）
  → 基础方块 + onBlockActivated
  → GUI 容器走 INamedContainerProvider + Container，不要伪造 IHasMenu
  → 每 tick 逻辑让 TileEntity 实现 ITickableTileEntity（不是 ITickable）

IF 需要流体
  → 使用 Forge 专用流体系统
  → 注册 Fluid（ForgeFlowingFluid）+ FluidAttributes，不要 FluidType
  → 参考 05-events.mdc 中的流体相关事件

IF 需要特殊渲染（多面材质、透明度、动画）
  → 自定义 BlockState + IBakedModel / TESR
  → 不要抄 1.7/1.12 的 ISimpleBlockRenderingHandler
  → 参考 08-client-server.mdc 中的渲染规则
```

### Decision: 方块属性（Material）选择

```
IF 自然方块（石头、木头、草）
  → Material.ROCK / Material.WOOD / Material.GRASS
  → 需要指定挖掘工具时用 .harvestTool() + .harvestLevel()

IF 泥土类
  → Material.EARTH / Material.SAND / Material.SNOW

IF 玻璃/冰/透明
  → Material.GLASS / Material.ICE
  → 不碰撞时用 .doesNotBlockMovement()（本档 Properties 没有 notSolid）

IF 植物
  → Material.PLANTS / Material.REPLACEABLE_PLANT
  → 不能放置方块，需要特殊放置逻辑

IF 液体
  → 不在 Block 中实现，使用 Fluid 系统
```

### Decision: BlockEntity vs 普通方块

```
IF 需要存储玩家数据（容器内容、熔炉燃料/物品）
  → 必须使用 TileEntity
  → 在方块类重写 hasTileEntity / createTileEntity
  → 注册时先注册方块再注册 TileEntityType

IF 需要每 tick 逻辑（自动机、计时器）
  → TileEntity + ITickableTileEntity
  → 在 ModBlocks 中注册 TileEntityType 时关联 Block

IF 只需要静态方块（装饰、完整方块）
  → 普通 Block 即可，不需要 TileEntity
  → 减少内存占用和复杂度
```

---

## 示例：基础方块（带 BlockItem）

```java
// blocks/MyBlock.java
public class MyBlock extends Block {
    public MyBlock() {
        super(Block.Properties.create(Material.ROCK)
            .hardnessAndResistance(1.5f, 6.0f)
            .harvestLevel(1)
            .sound(SoundType.STONE)
        );
    }

    @Override
    public void onBlockPlacedBy(World world, BlockPos pos, BlockState state, LivingEntity placer, ItemStack stack) {
        // 方块被放置时的逻辑
    }

    @Override
    public void onBlockExploded(World world, BlockPos pos, Explosion explosion) {
        // 方块被爆炸破坏时的逻辑
        // 注意：1.14.4 用 onBlockExploded，不是新的 wasExploded
    }
}
```

```java
// Registration using DeferredRegister (recommended)
public static final DeferredRegister<Block> BLOCKS =
    DeferredRegister.create(ForgeRegistries.BLOCKS, MOD_ID);

public static final RegistryObject<Block> MY_BLOCK =
    BLOCKS.register("my_block", MyBlock::new);
```

```java
// ❌ 错误示例：setRegistryName + 直接 new（已过时）
event.getRegistry().register(
    new MyBlock().setRegistryName(new ResourceLocation(MOD_ID, "my_block"))
);
```

## 示例：带 TileEntity 的方块

```java
// blocks/MyBlockEntityBlock.java
public class MyBlockEntityBlock extends Block {
    public MyBlockEntityBlock() {
        super(Block.Properties.create(Material.WOOD).doesNotBlockMovement());
    }

    @Override
    public boolean hasTileEntity(BlockState state) {
        return true;
    }

    @Override
    public TileEntity createTileEntity(BlockState state, IBlockReader world) {
        return new MyTileEntity();
    }
}
```

```java
// tiles/MyTileEntity.java
public class MyTileEntity extends TileEntity {
    private CompoundNBT data = new CompoundNBT();

    public MyTileEntity() {
        super(ModTileEntities.MY_TILE_ENTITY.get());
    }

    @Override
    public CompoundNBT write(CompoundNBT compound) {
        compound = super.write(compound);
        compound.put("data", data.copy());
        return compound;
    }

    @Override
    public void read(CompoundNBT compound) {
        super.read(compound);
        if (compound.contains("data", Constants.NBT.TAG_COMPOUND)) {
            this.data = compound.getCompound("data");
        }
    }
}
```

> 注意：`read` 中读取 NBT 是安全的，但你**不能**在 `read` 中读取世界数据（如 `world.getBlockState(pos)`）。如需基于世界的逻辑，在 `onLoad()` 或 `markDirty()` 中处理。
