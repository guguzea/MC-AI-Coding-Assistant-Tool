...
public ExampleMod() {
    ...
    BLOCKS.register(FMLJavaModLoadingContext.get().getModEventBus());   // ← 必须
    ITEMS.register(FMLJavaModLoadingContext.get().getModEventBus());   // ← 必须
}
```
...
## 错误：使用 RegistryEvent.Register 方式注册实体（1.14.4）

**症状：** 实体不在游戏中生成

**说明：** 在 1.14.4 的 Forge 中，推荐使用 DeferredRegister。
