...
IF 需要服务端数据同步到客户端
  ...
  → 使用 BlockEntity#getUpdateTag / ClientboundBlockEntityDataPacket.create（方块数据）
...
public class ClientSetup {
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        ...
        // KeyMapping 注册应在 RegisterKeyMappingsEvent 中完成（见下方）
    }
...
> **KeyMapping 注册方式**：必须通过 `RegisterKeyMappingsEvent` + `event.register()`，而非 `FMLClientSetupEvent` 中的 `ClientRegistry.registerKeyBinding()`。

## 示例：KeyMapping 处理

```java
// client/input/ModKeyBindings.java
// KeyMapping 定义（静态字段，在 RegisterKeyMappingsEvent 中注册）
...
> 注意：`InputEvent.Key` 在 Forge 1.20.4 中**已废弃**，应使用 `ClientTickEvent` + `consumeClick()`。
...
> 注意：
> - 1.20.4 的 BlockEntity 同步推荐使用 `getUpdateTag()` / `handleUpdateTag()`（无需手动实现 `getUpdatePacket`）
> - `setChanged()` 标记数据已修改
