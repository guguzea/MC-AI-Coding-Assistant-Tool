...
- 事件类必须在 `@Mod.EventBusSubscriber` 或手动注册到事件总线
- `bus` 属性决定事件总线：`Bus.FORGE`（默认）或 `Bus.MOD`（Mod 总线）
- **禁止**在错误的事件总线监听事件
...
| 事件总线 | 使用场景 | 订阅方式 |
|----------|----------|----------|
| `Bus.FORGE`（游戏总线，默认） | 游戏内事件（PlayerInteract、LivingDrops、`AttachCapabilitiesEvent` 等） | `@Mod.EventBusSubscriber(bus = Bus.FORGE)` |
| `Bus.MOD`（Mod 总线） | FML 生命周期与**注册**事件（`RegisterEvent`、`NewRegistryEvent`、`FMLCommonSetupEvent` 等） | `@Mod.EventBusSubscriber(bus = Bus.MOD)` 或 mod 构造器内 `modEventBus.addListener(...)` |

> 证据：本档语料 `concepts_events.md:28-29`「// This event is on the mod bus」→ `modEventHandler(RegisterEvent event)`、`:33-34`「// This event is on the forge bus」→ `AttachCapabilitiesEvent<Entity>`；`concepts_lifecycle.md:3,5`（生命周期/注册事件 fired on the mod-specific event bus，用 `@EventBusSubscriber(bus = Bus.MOD)`）、`:34`（本档注册事件 = `NewRegistryEvent` + `DataPackRegistryEvent$NewRegistry` + `RegisterEvent`）。**在 `Bus.FORGE` 上订阅 `RegisterEvent` 永不触发。**
...
> **注意**：`safe*` 变体在 Java 9+ 开发环境中有问题（抛出 `BootstrapMethodError`），**推荐始终使用 `unsafe*`**。
...
- `@OnlyIn(Dist.CLIENT)` 在 `@Mod.EventBusSubscriber(value = Dist.CLIENT)` 上使用时是**标准用法**
- 在普通方法上直接使用 `@OnlyIn` 不是推荐做法，应优先使用 `DistExecutor`
- `@OnlyIn` 是编译器级别的物理端隔离，会导致另一端完全无法加载该类

> **警告**：`FMLEnvironment.dist` 返回物理端，但单玩家世界（逻辑服务端+逻辑客户端在同一物理客户端内）的 `dist` 恒为 `Dist.CLIENT`，不要用此判断逻辑端（用 `level.isClientSide`）
...
IF 监听玩家数据保存/加载
  → PlayerEvent.SaveToFile / LoadFromFile
...
IF 监听 Tick（每帧逻辑）
  → TickEvent.ServerTickEvent / LevelTickEvent（Forge 45+/1.19.4 起更名）
...
IF 监听 Registry 注册
  → RegisterEvent（必须订阅在 Bus.MOD / modEventBus；挂 Bus.FORGE 永不触发）
  → 参考 01-registry.mdc
```
...
IF 事件涉及世界修改（放置方块、生成实体）
  → 服务端处理
  → 使用 @OnlyIn(Dist.DEDICATED_SERVER) 或确保在服务端线程
...
```java
// 注意：必须在 @OnlyIn(Dist.CLIENT) 标记的类或方法中
...
KeyMapping 注册应使用 `RegisterKeyMappingsEvent` + `event.register()`，而非 `KeyMappingRegistry.registerKeyBinding()`。
