...
- **推荐使用 `DeferredRegister`**：`DeferredRegister` 是 Forge 官方推荐的注册方式，自 Forge 1.18 起即可用于所有注册表，1.20.4 完全支持
...
public class ExampleMod {
    ...
    // 在 mod 构造函数中将 DeferredRegister 注册到 modEventBus（49.x 也支持构造注入；无参+`.get()` 仍可用）
    public ExampleMod() {
...
> 注意：
> - DeferredRegister 在 modEventBus 上注册后，会在正确的 RegisterEvent 时机自动执行所有 register() 调用（证据：本档语料 `concepts_registries.md:18`「registers the objects from those suppliers during `RegisterEvent`」；该事件只派发于 Mod 总线，见 `:39`）
