...
public class DataGenerators {
    ...
    public static void gatherData(GatherDataEvent event) {
        ...
        // TODO(未核实：getPackOutput 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
        // 本档语料逐字出现的只有 `PackOutput` 形参类型与 `event.getGenerator()`；
        // 语料取 PackOutput 的形态是给 addProvider 传工厂 lambda `output -> new XxxProvider(output, ...)`
        //（出处：data/forge_1.20.4/forge-docs/1.20.4/processed/datagen_server_tags.md:12-22、
...
public class ModBlockStatesProvider extends BlockStateProvider {
    ...
    protected void registerStatesAndModels() {
        // 无变体方块
        // TODO(未核实：cubeAll 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
        ...
        //（出处：data/forge_1.20.4/forge-docs/1.20.4/processed/datagen_client_modelproviders.md:150-156）；
...
public class ModBlockTagsProvider extends BlockTagsProvider {
    // TODO(未核实：NEEDS_DIAMOND_TOOL 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
...
public class ModItemTagsProvider extends ItemTagsProvider {
    // TODO(未核实：blockLookupProvider 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
    //（该形参名本档语料从未逐字出现；语料里 ItemTagsProvider 只出现在 provider 对照表
    //  data/forge_1.20.4/forge-docs/1.20.4/processed/datagen_server_tags.md:72 与 `#copy` 用法 :89-99，均无构造签名；
    ...
    @Override
    protected void addTags(HolderLookup.Provider provider) {
        // TODO(未核实：PIGLIN_LOVED 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
...
public class ModAdvancementProvider extends AdvancementProvider {
    ...
    // FinishedAdvancement 在 forge 1.20.4 语料零命中；语料里 writer 的元素类型逐字是 `Advancement`
    //（出处：data/forge_1.20.4/forge-docs/1.20.4/processed/datagen_server_advancements.md:33,42）
    private static class ModAdvancementSubProvider implements Consumer<Consumer<Advancement>> {
        @Override
        public void accept(Consumer<Advancement> consumer) {
            // 根进度
            Advancement.Builder.advancement()
                // TODO(未核实：FrameType / TASK 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
                //（本档语料对 `display` 只给了方法名与一句描述，未给参数类型 ⇒ 同档 mc-advancement 亦留 TODO(未核实)；
                //  `FrameType` / `TASK` 在本档语料含机器 dump `data/forge_1.20.4/extracted/api-index.json` 都零命中，
                //  出处：data/forge_1.20.4/forge-docs/1.20.4/processed/datagen_server_advancements.md:56）
                .display(
            ...
            Advancement.Builder.advancement()
                ...
                // TODO(未核实：FrameType / GOAL 与 Items.DIAMOND 未在 forge 1.20.4 语料命中，需 search_forge_docs 复核或反编译)
                .display(
                    Items.DIAMOND,
