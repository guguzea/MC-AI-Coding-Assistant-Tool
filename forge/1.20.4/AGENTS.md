# Forge 1.20.4 — Agent 总纲

> 本规则集适用于 **Forge 1.20.4**，推荐使用 `DeferredRegister` 注册模式。
...
→ IF 项目中存在 src/main/resources/META-INF/mods.toml
    → IF mods.toml 中 modLoader = "javafml"
        → IF build.gradle 中 minecraft = "1.20.4"
            → 继续加载本规则集（Forge 1.20.4）
...
1. **推荐使用 DeferredRegister**：`DeferredRegister` 是 Forge 官方推荐的注册方式，自 Forge 1.18 起即可用于所有注册表，1.20.4 完全支持
...
| 功能 | 1.20.4 Forge | 1.20.4+ NeoForge | 备注 |
|------|---------------|-------------------|------|
| 注册方式 | `DeferredRegister` | `DeferredRegister` | 一致（均推荐） |
| Fluid 注册 | `FluidType` | 有 Breaking Changes，参考 NeoForge 文档 | 1.20.4 有 FluidType |
| BlockEntity | 旧版签名 | 相同 | - |
