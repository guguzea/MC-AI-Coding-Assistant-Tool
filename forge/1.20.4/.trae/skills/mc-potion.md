...
> 本档正文的方法名 / 事件名 / 注册表名只来自 `data/forge_1.20.4` 本档语料。实读页面（均在 `forge-docs/1.20.4/processed/`）：
> `resources_server_recipes_incode.md`（§Brewing Recipes / §IBrewingRecipe）、`resources_server_tags.md`（§Tags 的 `Potion` 面）、`concepts_registries.md`（`registryName = "potion"` 的 `@ObjectHolder` 示例）、`concepts_events.md`（`PotionBrewEvent` 作为多阶段事件的举例）。
> 页内没有的签名一律留 `TODO(未核实)`，禁止用邻版（1.20.1 / 1.19.4 / NeoForge 档）或 Fabric 档补全。
>
> ⚠️ mappings：本档 frontmatter 记 `mappings: mcp`，但这四页正文与代码示例一律用官方名（`PotionBrewing`、`ItemStack`、`CompoundTag`、`ResourceLocation`）。本档实际通道以 `00-project-setup.mdc` 与工程 `gradle.properties` 声明为准（本档 scaffold 推荐 Parchment，即 mojmap 名 + 社区参数名）。**禁止**写 `func_XXXXX` / `field_XXXXX`，也禁止写 Yarn 名或 `class_` / `method_` 中间名。
...
`BrewingRecipeRegistry#addRecipe` 的**确切重载与参数顺序、其所在包名，本档页面未给出代码块** ⇒ `// TODO(未核实)`：写工程代码前用 `get_forge_doc_full(version="1.20.4", id="resources_server_recipes_incode")` 复核，或对用户自备的 Forge jar 跑 `query_loader_api`（`platform=forge`, `minecraftVersion=1.20.4`）逐签名核对。禁止凭记忆补参数表。
...
event.enqueueWork(() -> {
    // TODO(未核实): BrewingRecipeRegistry.addRecipe(...) 的具体参数形态本档页面未给代码
});
```
...
`concepts_events.md` 里 `PotionBrewEvent` 只作为「一个事件有多个阶段」的**举例**出现（同例还有 `PlayerEvent`），并提醒：监听父事件类会收到**所有**子类的调用。该页未展开 `PotionBrewEvent` 的阶段划分、可取消性与字段 ⇒ 需要监听酿造过程时先 `get_forge_doc_full(version="1.20.4", id="concepts_events")` 现读现抄，禁止按 NeoForge 记忆写 `@SubscribeEvent(priority = ...)` 的具体形态。

## 本档未覆盖（禁止默写）

- **`MobEffect` / `MobEffectInstance` 的注册与施加**：本档语料里 `MobEffect` 只在 `datagen_client_localization.md` 的一句 `LanguageProvider` 方法枚举中出现一次，无任何效果开发正文 ⇒ 见 `mc-effect`（该主题本档同样无正文，须联网补）。
- **自定义 `Potion` 的注册代码、`Potion` 与效果列表的关系**：本档无示例 ⇒ 不写。
...
- **药水物品组件化（`ItemLikeObject` / data component 一系）**：那是更新版本面的内容，本档语料未出现 ⇒ 禁止借邻档 / NeoForge。
- **同页的 Anvil / Loom 小节**（`AnvilUpdateEvent`、`BannerPattern`、`minecraft:no_item_required`）属别的主题，不要当药水证据用。
...
- 全文核对：`get_forge_doc_full(version="1.20.4", id="resources_server_recipes_incode")` / `id="resources_server_tags"`
