...
> 本规则集适用于 **Forge 1.17.1**，推荐使用 `RegistryEvent` 注册模式。
...
| 平台 | Forge |
| Minecraft 版本 | 1.17.1 |
| 注册模式 | `RegistryEvent` / `RegisterEvent` |
| Java 版本 | **Java 16**（Forge 1.17.1 要求） |
| Gradle | Gradle 7.x + ForgeGradle 4.x |
| Mappings | **MCP**（`minecraft "1.17.1"` 下默认） |
| pack_format | 8 |
...
1. **使用 RegistryEvent.register 注册**：`RegistryEvent` 是 Forge 1.17.1 的标准注册方式
...
|------|---------------|----------------|------|
| 注册方式 | `RegistryEvent` | `DeferredRegister` | 1.17.1 使用 RegistryEvent |
| Java 版本 | Java 16 | Java 17 | 1.17.1 要求 Java 16 |
| Gradle | Gradle 7.x | Gradle 8.x | |
| pack_format | 8 | 15 (1.20.x) | 1.17.1 使用 pack_format 8 |
| DataGen | 旧版 DataGenerator | 新版 PackOutput | 1.17.1 API 不同 |
