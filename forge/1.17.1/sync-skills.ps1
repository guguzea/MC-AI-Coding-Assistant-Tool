# =============================================================================
# MC Skill Sync Script — forge/1.17.1
#
...
function Normalize-Content {
    param([string]$text)
    # Replace: `forge/1.17.1/.cursor/rules/XX-name.mdc` → `XX-name.mdc`
    $text = $text -replace '`forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)`', '`$1`'
    # Replace: forge/1.17.1/.cursor/rules/XX-name.mdc (no backticks)
    $text = $text -replace 'forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '$1'
    # Replace: 参考 forge/1.17.1/.cursor/rules/XX-name.mdc → 参考 `XX-name.mdc`
    $text = $text -replace '参考 forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '参考 `$1`'
    # Replace: 参见 `forge/1.17.1/.cursor/rules/XX-name.mdc` → 参见 `XX-name.mdc`
    $text = $text -replace '参见\s*`?forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)`?', '参见 `$1`'
    # Replace: 参考 forge/.../XX-name.mdc (no backticks, no space) → 参考 `XX-name.mdc`
    $text = $text -replace '参考\s+forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '参考 `$1`'
    # Replace:  → 参考 forge/1.17.1/.cursor/rules/XX-name.mdc  (in code blocks)
    $text = $text -replace '→\s+参考\s+forge/1\.17\.1/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '→ 参考 `$1`'
    return $text
}
