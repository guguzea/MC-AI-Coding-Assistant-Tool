...
Forge 1.17.1 模组项目校验工具

校验内容：
1. mods.toml 语法和必填字段（优先级高于 gradle.properties）
2. mod ID 在所有文件间的一致性
3. @Mod 注解检查（与 mods.toml modId 一致）
4. Registry 命名与 static/final 修饰符
5. RegistryEvent 注册完整性（是否注册了方块、物品等）
6. 类名与文件名一致性
7. 资源文件路径完整性（单次遍历优化）
...
class Validator:
    ...
    def _validate_java_file(self, path: Path):
        ...
        for match in re.finditer(
            r'\.setRegistryName\s*\(\s*new\s+ResourceLocation\s*\(\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\)',
            content
        ):
        ...
        # ── @SubscribeEvent 注册检查 ─────────────────────────
        subscribe_events = len(re.findall(r'@SubscribeEvent', content))
        if subscribe_events > 0:
            registry_events = len(re.findall(
                r'RegistryEvent\.Register\s*<\s*\w+\s*>', content
            ))
            if registry_events == 0:
                self.warnings.append(
                    f"[{rel_path}] 发现 @SubscribeEvent 但可能缺少 RegistryEvent 注册"
                )
