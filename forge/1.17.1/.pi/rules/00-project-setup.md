...
- `gradle.properties` 必须包含：
  ...
  forge_version=37.1.68
  mappings_version=20210626.163703
  loader_version=37.1.68
  ```
  > 注意：
...
- Forge 1.17.1 推荐使用 **Parchment**
- `gradle.properties` 中的 `mappings_version` 对应 Parchment 版本（`Parchment 1.17.1-2021.09.06`）
- **禁止**在 `build.gradle` 中切换到 `yarn`（Fabric 专属）
- Parchment 映射将混淆的 Minecraft 方法映射为可读名称，并附带参数名
