...
IF 代码涉及输入（KeyMapping、MouseListener、KeyboardHandler）
  → 客户端
  → 放在 client/ 包下
  → 使用 @OnlyIn(Dist.CLIENT)
...
IF 只需在客户端显示额外信息（如 GUI 覆盖层）
  ...
  → 检查 OverlayMessage 来判断状态

IF 需要在客户端渲染自定义模型
  → 继承 BlockModel / ItemModel
  → 在 ClientSetup 中注册
...
public class ClientSetup {
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        // 1.17.1：客户端特有初始化 + 按键注册都在这里
        net.minecraftforge.fmlclient.registry.ClientRegistry.registerKeyBinding(TEST_KEY); // TEST_KEY 需定义在静态字段中
    }

    @SubscribeEvent
    public static void onRegisterRenderers(EntityRenderersEvent.RegisterRenderers event) {
        // 注册实体渲染器（通过事件 API 而非静态方法）
        event.registerEntityRenderer(ModEntities.MY_ENTITY.get(), MyEntityRenderer::new);

        // 注册方块实体渲染器
        event.registerBlockEntityRenderer(ModBlockEntities.MY_BLOCK_ENTITY.get(),
            MyBlockEntityRenderer::new);
    }
}
```

> **KeyMapping 注册方式**：1.17.1 **必须**在 `FMLClientSetupEvent`（`Bus.MOD` + `Dist.CLIENT`）中调用 `net.minecraftforge.fmlclient.registry.ClientRegistry.registerKeyBinding(KeyMapping)`（oracle：`query_loader_api --platform=forge --minecraftVersion=1.17.1 --className=net.minecraftforge.fmlclient.registry.ClientRegistry` → `void registerKeyBinding(KeyMapping)`）。`RegisterKeyMappingsEvent` 是 **1.19+** API，本档**不存在**（1.17.1 摘要命中 0）。

## 示例：KeyMapping 处理

```java
// client/input/ModKeyBindings.java
// KeyMapping 定义（静态字段；1.17.1 在 FMLClientSetupEvent 里注册）
...
public class ModKeyBindings {
    // 注册 KeyMapping（在 MOD 总线上，Forge 自动处理到主线程）
    @SubscribeEvent
    public static void onClientSetup(FMLClientSetupEvent event) {
        net.minecraftforge.fmlclient.registry.ClientRegistry.registerKeyBinding(TEST_KEY);
    }
    ...
    public static void onClientTick(ClientTickEvent event) {
        if (event.getPhase() == TickEvent.Phase.END) {
            while (TEST_KEY.consumeClick()) {
                // 按键按下时的逻辑（仅在游戏中有效，GUI 中不会触发）
                // 发送到服务端用网络包，直接操作用客户端逻辑
            }
        }
    }
}
...
> 注意：`InputEvent.Key` 在 Forge 1.17.1 中**已废弃**，应使用 `ClientTickEvent` + `consumeClick()`。
>
> **总线归属（别把两者塞进同一个类）**：按键**注册**走 `Bus.MOD`（`FMLClientSetupEvent` + `ClientRegistry.registerKeyBinding`），按键**检测**（`ClientTickEvent`）走 **FORGE 总线**——同一个类不能同时挂两条总线；检测逻辑请放到另一个不带 `bus = Bus.MOD` 的 `@Mod.EventBusSubscriber` 类里。
...
> 注意：
> - 1.17.1 的 BlockEntity 同步推荐使用 `getUpdateTag()` / `handleUpdateTag()`（无需手动 implement `getUpdatePacket`）
> - `setChanged()` 标记数据已修改
> - `level.sendBlockUpdated()` 将更新通知发送给客户端，客户端会自动调用 `getUpdateTag()` 同步数据
> - `Block.UPDATE_ALL` 标志确保所有相关客户端更新到位
