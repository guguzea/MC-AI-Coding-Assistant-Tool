...
new Item.Properties()
    ...
    .tab(CreativeModeTab)                       // 创意模式标签（1.17.1 可用）
```
...
IF 需要附魔书以外的附魔物品
  → 不支持，Forge 1.17.1 附魔只能通过附魔台/附魔桌
...
IF 自然方块 → CreativeModeTab.DECORATIONS
IF 工具 → CreativeModeTab.TOOLS
IF 武器 → CreativeModeTab.COMBAT
IF 食物 → CreativeModeTab.FOOD_AND_DRINK
...
IF 搜索 → CreativeModeTab.SEARCH（不推荐）
```
