...
> 注意：`ForgeRegistries.ATTRIBUTES` 是注册表的持有类，`DeferredRegister.create()` 需要传入 `IRegistry<Attribute>`。属性注册在 mod 构造函数中通过 modEventBus 完成，而不是在 `FMLCommonSetupEvent` 中。
...
> 注意：
> 1. `EntityRenderers` 在 Forge 1.17.1 中通过 `EntityRenderersEvent.RegisterRenderers` 事件注册（不是 `ClientSetup` 中直接调用）
