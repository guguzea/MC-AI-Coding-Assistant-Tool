...
**本档没有 `FluidType`。** `FluidType` / `SoundActions` 是 Forge **1.19+**。1.17.1 仍用 `FluidAttributes.builder`。
...
| 类型 | 注册表 | 备注 |
|------|--------|------|
| `Fluid`（Source / Flowing） | `ForgeRegistries.FLUIDS` | 不要 `FluidType` 注册表 |
...
贴图：`assets/{modid}/textures/block/my_fluid_still.png`（以及 flow）
...
- ❌ `FluidType` / `SoundActions` / `RegistryObject<BUCKET_ITEM>`
