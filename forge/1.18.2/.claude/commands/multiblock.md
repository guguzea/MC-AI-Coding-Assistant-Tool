...
# mc-multiblock（1.18.2）

> 一手来源：原语类名 `BlockPos` / `BlockEntity` / `Level` 经 `query_api`（version=1.18.2）核实存在。多方块无平台 API——**模式技能**。
...
- 原语核实：`query_api`（version=1.18.2）；反模式库：`forge/1.18.2/knowledge/antipatterns/`。
