...
→ IF 项目中存在 src/main/resources/META-INF/mods.toml
    → IF mods.toml 中 modLoader = "javafml"
        → IF build.gradle 中 minecraft = "1.18.2"
...
社区实务与署名：MCP `search_community_docs`；`community_knowledge/ATTRIBUTION.md`（含 MC百科耿悠博教程许可说明）。
...
## 世界高度（Caves & Cliffs 更新）

Forge 1.18.2 引入 Caves & Cliffs 大更新，世界高度发生变化：
...
1. **推荐使用 DeferredRegister**：`DeferredRegister` 是 Forge 官方推荐的注册方式，Forge 1.18.2 完全支持
