...
- 禁止重写 `onDestroyedByPlayer`、`onBlockActivated` 等旧版（MCP 时代）交互方法名；注意 `use` 是本档现行方块交互方法（返回 `InteractionResult`），应重写它（见下方 Decision Flow）
...
- BlockEntity 的 `saveAdditional`/`load` 方法处理 NBT 序列化（使用 `CompoundTag`）
...
Forge 1.18.2 的世界高度为 Y=-64 到 Y=320。在生成世界相关内容时需要注意此范围。
