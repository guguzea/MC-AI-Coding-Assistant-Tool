...
- 渲染器必须放在 `client` 包下
- 必须通过 `EntityRenderersEvent.RegisterRenderers` 注册
- 必须在 `FMLClientSetupEvent` 中订阅 `EntityRenderersEvent.RegisterRenderers`
- 纹理文件路径：`textures/entity/{entity_path}/{name}.png`

### 世界高度注意

Forge 1.18.2 的世界高度为 Y=-64 到 Y=320。在生成实体生成位置时需要注意此范围。
