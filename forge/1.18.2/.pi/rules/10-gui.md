---
description: Client-Server separation and GUI (Forge 1.18.2)
---

# Client-Server separation and GUI (Forge 1.18.2)
...
// In Menu constructor
super(MyMenuTypes.MY_MENU.get(), windowId); // A-43：AbstractContainerMenu 只有 (MenuType, int) 两个构造参数（原 5 参示例编译不过）
this.addDataSlots(dataSlots); // A-43：不接线则客户端收不到同步（原版承诺自动同步但没接线）
...
public class MyBlock extends Block {
    ...
    public MenuProvider getMenuProvider(BlockState state, Level level,
        ...
        return new SimpleMenuProvider(
            (id, inv, player) -> new MyMenu(id, inv),
...
- ❌ `MenuScreens.register()` called on server → `FMLClientSetupEvent` already prevents this
