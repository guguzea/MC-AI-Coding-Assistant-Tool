...
> 注意：Forge 1.18.2 中，方块实体的注册表名称是 `BLOCK_ENTITIES`（不是 `BLOCK_ENTITY_TYPES`）
...
IF 平台 = Fabric
  → 跳转 fabric/1.18.2/AGENTS.md（Fabric 使用 Registry.register() in onInitialize）
...
IF 注册 方块实体（BlockEntity）
  ...
  → 使用 DeferredRegister.create(ForgeRegistries.BLOCK_ENTITIES, MOD_ID)
