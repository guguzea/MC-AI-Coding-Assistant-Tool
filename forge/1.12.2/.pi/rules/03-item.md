...
new Item()
    ...
    .setRarity(EnumRarity rarity)               // 稀有度（COMMON, UNCOMMON, RARE, EPIC）
    .setPotionEffect(String effect)              // 药水效果
    .addToolClass(String class, int level)       // 工具类
```
...
IF 需要食物（可食用物品）
  → extends ItemFood
  → 或 extends Item 覆盖 onFoodEaten 方法
...
```
IF 自定义工具材料
  → 定义枚举实现 ToolMaterial 接口
  → 5 个必需方法全部实现
  → getRepairMaterial() 必须返回有效的 Ingredient
  → 在物品类中引用

IF 复用现有材料
  → ToolMaterial.WOOD / ToolMaterial.STONE / ToolMaterial.IRON / ToolMaterial.GOLD / ToolMaterial.DIAMOND（静态字段）
```
...
public class ItemFoodExample extends ItemFood {
    ...
    protected void onFoodEaten(ItemStack stack, World world, EntityPlayer player) {
        ...
        // 食物效果逻辑
        player.addPotionEffect(new PotionEffect(MobEffects.JUMP, 200, 1));
    }
}
```
...
public static final Item EXAMPLE_FOOD = new ItemFoodExample(4, 0.3f, false)
        ...
        .setTranslationKey(MODID + ".example_food");
```

> 注：`PotionEffect` 第一个参数是 `Potion`（如 `MobEffects.JUMP`），不是 `int` ID。
