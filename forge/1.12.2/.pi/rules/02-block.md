...
- 用 `new Block(Material)`，再链式 `setHardness` / `setResistance` / `setSoundType`（没有 `Block.Properties`）
- 禁止重写已废弃的方法
...
- 有状态或需要持久化的方块：继承 `BlockContainer`，或普通 `Block` 重写 `hasTileEntity(IBlockState)` + `createTileEntity(World, IBlockState)`
- `ITileEntityProvider.createNewTileEntity` 签名是 `(World, int meta)`，**不是** `(World, IBlockState)`
- NBT 在 **TileEntity** 上：`writeToNBT(NBTTagCompound)` **返回** NBTTagCompound；`readFromNBT(NBTTagCompound)`。不要写在 Block 上
- 无参构造 `TileEntity()`。每 tick 实现 `ITickable#update()`（不是 1.13 的 `tick()`）
...
IF 需要存储数据（箱子、熔炉等）
  ...
  → 需要 `GameRegistry.registerTileEntity`
...
IF 玻璃/冰/透明
  ...
  → 碰撞/完整方块靠重写 isFullCube / isOpaqueCube 等，不要抄 1.13+ Properties.notSolid()
...
IF 需要每 tick 逻辑（自动机、计时器）
  → TileEntity 实现 ITickable，方法是 update()
