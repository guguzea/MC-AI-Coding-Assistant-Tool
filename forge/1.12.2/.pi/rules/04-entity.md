...
- 实体必须通过 `RegistryEvent.Register<EntityEntry>>` 注册
- 必须在 `mcmod.info` 中添加 entity 声明

### EntityEntry.Builder 配置
...
EntityEntryBuilder.create()
    ...
    .name("my_entity")
    .egg(0xFFFFFF, 0xFFFFFF)  // 蛋的颜色
    .tracker(int trackingRange, int updateFrequency, boolean sendsVelocityUpdates)
```

### 实体属性（Attribute）

- 每个实体**必须**在 `onLivingUpdate()` 或 `createAttributes()` 中设置属性
- 属性在实体注册后设置

### AI Goal

- 使用 `Goal` 实现自主行为（行走、攻击、飞行等）
- 每个 Entity 实例创建对应的 `Goal` 如 `NearestAttackableTargetGoal` 等
...
public class EntityExample extends EntityLiving {
    ...
    protected void applyEntityAttributes() {
        ...
        this.getAttributeMap().registerAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.3);
    ...
    protected void initEntityAI() {
        ...
        this.tasks.addTask(1, new EntityAIMeleeAttack(this, 1.0, true));
...
```java
// mcmod.info 中的 entity 声明
"dependencies": [],
"requiredMods": [],
"entityTypes": [
  {
    "entityClassName": "com.example.mod.entities.EntityExample",
    "name": "example_entity",
    "id": 0
  }
]
...
public class ClientProxy extends CommonProxy {
    ...
    private static class RenderFactory {
        public static Render createRender(RenderManager manager) {
            return new RenderLiving(manager, new ModelExample(), 0.5f) {
                @Override
...
> 3. 实体注册需要在 `mcmod.info` 中声明 `entityTypes`
