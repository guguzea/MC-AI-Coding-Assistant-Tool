...
- 实体类继承 `Entity`（基础）、`EntityLiving`（可移动/有生命）或 `EntityCreature` / `EntityMob`（AI 任务）
- 近战/目标 AI（`EntityAIAttackMelee`、`EntityAIHurtByTarget`、`EntityAINearestAttackableTarget`）的构造参数是 **`EntityCreature`**（javadoc 14.23.5.2854），不要把这些任务塞进只继承 `EntityLiving` 的类
- 推荐：被动生物 `EntityCreature`，敌对 `EntityMob`；不要为了抄 1.14+ `Mob` 而改名
...
```
IF 实体需要近战/寻路/目标 AI
  → 继承 EntityCreature（被动）或 EntityMob（敌对）
  → EntityAIAttackMelee 等任务要求 EntityCreature

IF 实体需要移动、生命、但不挂 Creature AI
  → 继承 EntityLiving
...
IF 自定义模型
  → 继承 RenderLiving<YourEntity>
  → 用 ModelBase 子类（不是方块/物品 JSON 模型）
  → 贴图走 getEntityTexture → assets/{modid}/textures/entity/...
```
...
> 注意：
> 1. 渲染器用 **IRenderFactory** 重载，在 **preInit** 注册（javadoc：带 `Render` 实例的重载已 Deprecated）。
> 2. 纹理：`assets/{modid}/textures/entity/example_entity.png`
> 3. 不要在 `mcmod.info` 里编 `entityTypes` 字段（官方字段表没有）
> 4. 示例 AI 需要 `EntityCreature`；不要把 `EntityAIAttackMelee` 接到纯 `EntityLiving` 上
