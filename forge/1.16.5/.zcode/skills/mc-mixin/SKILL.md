---
name: mc-mixin
description: Minecraft Forge Mixin 注入。安全使用 @Mixin、@Inject、@At、@ModifyVariable。触发词：Mixin、@Inject、@At、mixins.json、AccessWidener
platform: forge
version: "1.16.5"
dependencies: []
mappings: parchment
...
public class MixinPlayerEntity {
    ...
    private void onAttack(LivingEntity target, float amount, CallbackInfo ci) {
        // 在原方法执行前运行
    ...
    private void afterAttack(LivingEntity target, float amount, CallbackInfo ci) {
        // 在原方法执行完毕后运行
    }
}
```
...
public net.minecraft.entity.Entity field_70179_y F
```
...
- Forge Access Transformer：https://docs.minecraftforge.net/en/1.16.5/
