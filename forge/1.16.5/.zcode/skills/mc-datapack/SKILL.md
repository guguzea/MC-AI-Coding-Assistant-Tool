...
> 一手来源：`forge/1.16.5/knowledge/common/datapack-format.md`（pack_format 表逐项对齐官方 MDK，sha256 钉在 `mcp-server/data/mdk-checksums.json`）。本技能不含 Java API——数据包是 JSON 面；Java 侧数据生成走 `mc-datagen`。
...
→ 版本 → 1.16.2–1.16.5 用 pack_format 6（下表）
...
```json
{ "pack": { "pack_format": 6, "description": "My Datapack" } }
```

| MC 版本 | pack_format（数据包 = 资源包共用） |
|---------|------------|
| **1.16.2–1.16.5** | **6**（官方 1.16.5-36.2.34 MDK，`_comment`「…texture changes from 1.16.2」；本包 scaffold `pack.mcmeta` 同为 6） |
...
- 校验：`validate_datapack_json`；完整速查：`forge/1.16.5/knowledge/common/datapack-format.md`。
