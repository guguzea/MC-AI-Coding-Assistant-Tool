...
IF interactable block with persistent data (machine, chest)
  → Container + ContainerType + ContainerScreen

IF just a simple interaction (no data)
  → IScreen / Screen directly (without Container)
...
public class ClientSetup {
    ...
    public static void init(FMLClientSetupEvent event) {
        event.enqueueWork(() ->
            ScreenManager.registerFactory(MyContainerTypes.MY_CONTAINER.get(), MyScreen::new)
        );
    }
}
```
        ...
        if (!world.isClientSide) {
            ...
            NetworkHooks.openGui(
                ...
                new SimpleNamedContainerProvider(
                    (id, inv, p) -> new MyContainer(id, inv, p),
                    new StringTextComponent("My Container")
                ),
                pos
            );
        }
...
- ❌ Modifying world state in `Container` constructor → too early, use `detectAndSendChanges()`
