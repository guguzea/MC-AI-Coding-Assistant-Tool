...
IF 需要工具（剑、镐、斧、铲）
  → extends PickaxeItem / AxeItem / ShovelItem / SwordItem
  → 镐斧铲父类是 MCP `ToolItem`（不要抄 Mojmap `DiggerItem`）
  → 需要定义 IItemTier
...
public enum MyArmorMaterial implements IArmorTier {
    MY_MATERIAL("my_material", 40, new int[]{4, 7, 9, 4}, 20,
        SoundEvents.ARMOR_EQUIP_DIAMOND, 3.0f, 0.1f, () -> Ingredient.fromItems(Items.DIAMOND));
