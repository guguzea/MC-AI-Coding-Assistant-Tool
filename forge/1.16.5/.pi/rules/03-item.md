...
new Item.Properties()
    ...
    .tab(ItemGroup group)                       // 创造模式标签（1.16.x 使用 tab）
```
...
public enum MyItemTier implements IItemTier {
    ...
    // getLevel(), getUses(), getSpeed(), getAttackDamageBonus(), getEnchantmentValue(), getRepairMaterial()
}
```
...
IF 需要工具（剑、镐、斧、铲）
  → extends DiggerItem / SwordItem
  → 需要定义 IItemTier
  → 需要在 constructor 传入：speed, damage, durability, tier 属性
...
IF 需要可耐久物品
  ...
  → 覆盖 onHurt() 处理使用逻辑
  → 覆盖 onItemRightClick() 处理方块交互（右键放置等）

IF 需要附魔书以外的附魔物品
  → 不支持，Forge 1.16.5 附魔只能通过附魔台/附魔桌
...
IF 复用现有材料
  → Tiers.STONE / Tiers.IRON / Tiers.DIAMOND / Tiers.GOLD（静态字段）
```
...
public class MyItem extends Item {
    ...
    @Override
    public ActionResult<ItemStack> useOnWorld(ItemUsageContext context) {
        // 右键点击方块时的逻辑
...
public enum MyTier implements IItemTier {
    ...
    MyTier(int level, int uses, float speed, float damage, int enchantability, Supplier<Ingredient> repairIngredient) {
        this.level = level;
        this.uses = uses;
        this.speed = speed;
        this.damage = damage;
...
public static final RegistryObject<Item> MY_SWORD = ITEMS.register("my_sword",
    () -> new SwordItem(MyTier.MY_MATERIAL, 3, 1.6f, new Item.Properties().tab(ItemGroup.TAB_COMBAT))
);
// 创造模式标签通过 .tab(ItemGroup.TAB_COMBAT) 注册
```
...
public class MyArmorItem extends ArmorItem {
    public MyArmorItem(IArmorTier material, EquipmentSlotType slot, Properties properties) {
