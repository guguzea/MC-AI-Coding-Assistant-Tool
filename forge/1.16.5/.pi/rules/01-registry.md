...
- 所有注册必须通过 **Forge 事件系统**（最终由 `RegistryEvent` 触发）
- mod ID 必须与 `mods.toml` 中的 `modId` 完全一致
- **推荐使用 `DeferredRegister`**：`DeferredRegister` 自 Forge 1.16.2 起可用于所有注册表，1.16.5 完全支持
- `RegistryEvent` 仍可用，但不推荐（`DeferredRegister` 封装了它，更安全）

### 注册时机

`DeferredRegister` 通过 modEventBus 在正确的 `RegistryEvent` 时机自动执行注册，无需手动监听。
...
IF 平台 = Fabric
  → 跳转 fabric/1.16.5/AGENTS.md（Fabric 使用 Registry.register() in onInitialize）
...
IF 注册 方块实体（TileEntity）
  ...
  → 用 TileEntityType.Builder.of(...).build(null) 注册（语料 tileentities_tileentity.md:26；Builder.create 是 1.15.2 旧形，1.16.x.md:170）
...
IF 注册 流体
  → 只注册 Fluid 到 ForgeRegistries.FLUIDS
  → 用 FluidAttributes.builder；不要 FluidType（1.19+）
...
IF 一个方块有 TileEntity
  → 必须先注册 Block，再注册 TileEntityType
  → Block 的 registry name 必须与 TileEntityType 关联的 Block 匹配
...
public class ExampleMod {
    ...
    public void onServerStarting(ServerStartingEvent event) {
        LOGGER.info("Server starting: {}", event.getServer().getWorldData().getWorldGenSettings());
    }
}
```
...
> - BlockItem 与方块使用相同 registry name，Forge 自动关联，无需手动设置
