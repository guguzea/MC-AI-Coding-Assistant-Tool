...
- TileEntity 用 `save(CompoundNBT)` / `load(BlockState, CompoundNBT)`（语料 tileentities_tileentity.md:38）。不要 `saveAdditional`（1.17+）
...
IF 需要可交互（右键打开 GUI、触发事件）
  ...
  → 或使用 IContainerProvider + ITickableTileEntity 实现 GUI（TODO(未核实)：原写 ContainerProvider/ITickableBlockEntity 本档语料 0 命中、属别的版本线，禁止照抄；ITickableTileEntity 见语料 tileentities_tileentity.md:55）
...
IF 需要每 tick 逻辑（自动机、计时器）
  → 必须使用 TileEntity + ITickableTileEntity（语料 tileentities_tileentity.md:55）
...
public class MyBlockEntity extends TileEntity {
