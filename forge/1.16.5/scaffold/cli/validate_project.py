...
class Validator:
    ...
    def _validate_java_file(self, path: Path):
        ...
        for match in re.finditer(
            r'(ITEMS|BLOCKS|ENTITIES)\s*\.\s*register\s*\(\s*"([^"]+)"', content
        ):
...
def main():
    parser = argparse.ArgumentParser(description="Validate Forge mod project")
    parser.add_argument("project_root", nargs="?", default=".")
    parser.add_argument("--json", action="store_true", help="JSON 格式输出")
    parser.add_argument("--include-crash", action="store_true", help="额外分析 crash-reports/")
