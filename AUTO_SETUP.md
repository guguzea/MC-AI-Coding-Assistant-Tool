...
| **MCP 工具** | 运行时查询文档 / API / 映射 / 诊断（**80** 个工具） | 宿主拉起 **stdio** 子进程 `node dist/index.js` | 各 IDE 自己的 MCP 配置文件 |
...
2. 按**当前宿主**生成并合并 MCP 配置，使 Agent 能调用 **80** 个工具。
...
可选：工具列表里应能看到本服务，数量 **80**（`tool-registry.ts` 45 + `wave/register.ts` 35）。对不上先 `npm run build`，不要改服务名。
