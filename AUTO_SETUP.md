...
| **MCP 工具** | 运行时查询文档 / API / 映射 / 诊断（**79** 个工具） | 宿主拉起 **stdio** 子进程 `node dist/index.js` | 各 IDE 自己的 MCP 配置文件 |
...
2. 按**当前宿主**生成并合并 MCP 配置，使 Agent 能调用 **79** 个工具。
...
本工具包是**人在环**的模组开发副驾驶，不是无人值守流水线。配置 MCP、写盘、装 Skill 都须用户确认；Gradle / 拷 jar / 上传发布同样在用户没有确定的情况下不代跑。
...
- **推荐默认：用户级 / 全局**，这样打开任意模组工程都能用这 79 个工具。`args` 与 `MC_SKILL_DATA` 仍指向 **本仓库**（`MC_skill`），不要改成模组工程路径。
...
| `MC_SKILL_CACHE` | 反编译 / MDK zip / loader jar 缓存根。**MCP 运行时（`resolveCacheRoot`）与仓库脚本（如 `decompile-loader-apis.mjs`）都读此变量。** 不设则可能分家：MCP 默认 `%APPDATA%/mc-skill-cache`（Unix `~/.config/mc-skill-cache`），脚本默认 `os.tmpdir()/mc-skill-cache`（Windows 即 `%TEMP%/mc-skill-cache`，仍可能与 MCP 分家）。请设成同一路径。不要改 `resolveCacheRoot` 去探测 D 盘。`download_official_mdk` 解压依赖 **unzip / 7z / bsdtar**（不要假定 GNU tar 能解 zip；Linux 请装 unzip）。 |
...
可选：工具列表里应能看到本服务，数量 **79**（`tool-registry.ts` 45 + `wave/register.ts` 34）。对不上先 `npm run build`，不要改服务名。
...
编码期请用 `activate_platform_pack`：`session` 把该档 AGENTS / 默认规则 00+01+09 / Skill **索引**送进当前对话（`topics` / `includeAllRules` 见根 README）；要工程内常驻再 `write`（`hosts` 必填；不要再用 `includeSkills`，改用 `writeSkillStubs`，默认 true，只写 stub；`includeSkillBodies` 才写知识库 Skill 全文）。**不要**把规则拷进知识库根。MCP 工具列表与 Skill 扫描是两套机制。重载 MCP **不会**让 Skill 面板出现条目。本仓库 Skill 在 `forge/<ver>/`、`fabric/<ver>/`、`neoforge/` 子树，**不要**提交到 `MC_skill` 仓库根 `.cursor/skills/`。
...
- 无 MCP 宿主时用 CLI，不要假装已经连上 79 个工具
