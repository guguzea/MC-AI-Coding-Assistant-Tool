...
| **MCP 工具** | 运行时查询文档 / API / 映射 / 诊断（**70** 个工具） | 宿主拉起 **stdio** 子进程 `node dist/index.js` | 各 IDE 自己的 MCP 配置文件 |
...
2. 按**当前宿主**生成并合并 MCP 配置，使 Agent 能调用 **70** 个工具。
...
- **推荐默认：用户级 / 全局**，这样打开任意模组工程都能用这 70 个工具。`args` 与 `MC_SKILL_DATA` 仍指向 **本仓库**（`MC_skill`），不要改成模组工程路径。
...
可选：工具列表里应能看到本服务，数量 **70**（`index.ts` 44 + `wave/register.ts` 26）。对不上先 `npm run build`，不要改服务名。
...
- 无 MCP 宿主时用 CLI，不要假装已经连上 70 个工具
