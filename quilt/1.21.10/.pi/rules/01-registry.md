...
> ⚠️ **离线数据**：`data/quilt_1.21.10/` 无 quilt-docs 树（本版本无已发布 QSL/QFAPI 构件，属上游真实状态）。`search_docs({platform:"quilt"})` 对本版 QSL 查询返回 `PLATFORM_DATA_MISSING`（**不**回退 Fabric）；需要同版 Fabric API 写法时改用 `search_docs({platform:"fabric", version:"1.21.10"})`。禁止补抓或从邻版克隆 quilt-docs。
