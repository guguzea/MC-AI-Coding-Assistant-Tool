...
> ⚠️ **QSL 停更（2025-12 说法 `// TODO(未核实)`：原引官方 FAQ 已 404，无法复核）**：本版本**无 QSL/QFAPI 正式版构件**（maven 只到 alpha；实证数据见 quilt/1.21.10/AGENTS.md 顶部横幅）。下表 API 是**源码树考据，非可编译 API**——禁止生成。事件一律走同版 Fabric API。
...
- 不清楚方法名 → `search_docs({platform:"quilt", version:"1.21.10"})`；实测本版检索**两条路径都有回退面**（逐字段见顶部「离线数据」条）：`fallback:"fabric"` + `sourcePlatform:"fabric"` 命中的是同版 Fabric 正文（本版 `total:15`），**不得**当 QSL 证据；`fallback:"quilt"` + `resolvedVersion:"1.21.11"` 命中的是同线已建档正文而非本版正文。QSL 方法名一律 `query_loader_api`（先 `ingest_loader_api` 入库用户自备 jar）
