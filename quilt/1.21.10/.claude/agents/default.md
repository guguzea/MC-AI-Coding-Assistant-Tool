# Quilt 1.21.10 — Agent 总纲

> 本规则集适用于 **Quilt 1.21.10**。检测 `quilt.mod.json` 或 `org.quiltmc.loom`（可同时有 `fabric.mod.json`）。
> **必须再读** `fabric/1.21.10/.cursor/rules/` 的 **02–04、07–08、10**（及缺的 09）。05/06 用本目录，不要 overlay Fabric 网络。
...
`query_api` 对 1.21.10 无 Vanilla 索引。平台 API：`search_docs({platform:"quilt", version:"1.21.10"})`；无本档 quilt-docs 时改口 `list_doc_versions` 已入库档或 `search_fabric_docs version=1.21.10`。QSL 专用查询禁止把 Fabric Registry 当命中。
