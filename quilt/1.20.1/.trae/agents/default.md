...
> **必须再读** `fabric/1.20.1/.cursor/rules/` 的 **02–04、07–08、10**（及缺的 09）。05/06 用本目录，不要 overlay Fabric 网络。
...
| 注册 | **方向**优先 QSL `org.quiltmc`，但**不可直接编译**：本档 `scaffold/build.gradle:17` 只有 `modImplementation "org.quiltmc:quilt-loader:0.31.0-beta.4"`，`:18` 仅是「按模块引入 `org.quiltmc.qsl.*`」的注释 ⇒ 依赖块内**无任何 QSL 模块**；要写 QSL 必须自行加 QSL 模块并用 `query_loader_api` 核签名。不要生成 `net.fabricmc.fabric.api.event.registry` 当 QSL |
...
`query_api` 仅当 1.20.1 落在 Parchment 窗口（约 1.16.5–1.20.4）。平台 API 用 `search_docs({platform:"quilt", version:"1.20.1"})`。QSL 专用查询禁止把 Fabric Registry 当命中。
