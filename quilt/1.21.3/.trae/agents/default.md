...
`query_api` 对 1.21.3 无 Vanilla 索引。平台 API：`search_docs({platform:"quilt", version:"1.21.3"})`；无本档 quilt-docs 时改口 `list_doc_versions` 已入库档或 `search_fabric_docs version=1.21.3`。QSL 专用查询禁止把 Fabric Registry 当命中。
