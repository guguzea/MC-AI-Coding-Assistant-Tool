...
简单 Item/Block 仍可用 Vanilla `Registry.register(Registries.*, id, value)`（不是 FAPI 专属）。
...
QFAPI：有 QSL 替代的 FAPI **弃用**；item groups 等仍可能走 QFAPI（wiki `qsl-qfapi`）。方法签名以 QSL README / 已打开源码为准。
