...
> **F115 读法警示（语料 ≠ 事实源）**：`search_docs({platform:"quilt", version:"1.21.1"})` 命中的是索引语料 `data/quilt_1.21.1/quilt-docs/1.21.1/processed/qsl-verified.md`，其 `:3` **仍原样带着**本版 knowledge 已撤回的两条说法（「QSL 已于 2025-12 停更」与版本号 `11.0.0-alpha.3`）。语料正文按仓规**上游原样、不改写**，所以这不是矛盾而是两处不同步：读到该页正文时**必须回读** `quilt/1.21.1/knowledge/common/qsl-verified.md` 的降级/撤回声明（停更说法原引 `quiltmc.org/en/faq/` 已 404 → `// TODO(未核实)`；构件实证以 `:9` 的 maven 计数为准），**禁止**把语料正文里的停更日期或版本号当可引用事实写进答案。
