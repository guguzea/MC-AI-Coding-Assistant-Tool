...
> **必须再读** `fabric/1.21.1/.cursor/rules/` 的 **02–10**（方块/物品/实体/网络/datagen/客户端/反模式/GUI）。
...
| 05 | `05-events.mdc` | 仅 QSL 分叉；否则指向 fabric/1.21.1 的 05 |
| 02–04, 06–10 | **不要本目录找** | 打开 `fabric/1.21.1/.cursor/rules/` |

`query_api` 仅当 1.21.1 落在 Parchment 窗口（约 1.16.5–1.20.4）。平台 API 用 `search_docs({platform:"quilt"})`。QSL 专用查询禁止把 Fabric Registry 当命中。
