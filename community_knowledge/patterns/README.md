...
与 `knowledge/patterns/` 的分工（2026-08 审计澄清）：**模式正文与验收以 `knowledge/patterns/` 为准**；本目录管 Wave D 框架约定。⚠️（2026-09-19 口径对齐）**`patterns/` 不进 community 搜索索引**——`mcp-server/scripts/_lib/community-index-core.mjs` 显式排除（避免 `sourceKind=unknown`）；要检索模式请直接读本目录文件，不要用 `search_community_docs` 找它们。两套 `examples/` 文件名并行、互为独立文（如本目录 `mod-ci-github-actions.md` ≠ knowledge 侧 `ci-github-actions.md`），不互为镜像、禁止双向覆盖。
