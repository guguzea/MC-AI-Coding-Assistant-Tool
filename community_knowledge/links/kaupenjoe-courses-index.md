...
- 组织主页：https://github.com/Tutorials-By-Kaupenjoe
- 作者：Nico Kaupenjoe（YouTube 同名系列；仓库 MIT 许可，视频课程另见 kaupenjoe.net）
- 形态：每个仓库 = 一个课程；**每个分支 = 一集**（如 `34-oreGen`），可按分支 diff 学习单主题。

## 仓库清单（本库已核实存在）

| 平台 × 版本 | 仓库 |
|-------------|------|
| NeoForge 26.X | [NeoForge-Course-26.X](https://github.com/Tutorials-By-Kaupenjoe/NeoForge-Course-26.X)（102 分支，含 networking/data-attachment/hud/configs/dimensions 等） |
| Fabric 26.X | [Fabric-Course-26.X](https://github.com/Tutorials-By-Kaupenjoe/Fabric-Course-26.X) |
...
- 写代码前先看目标分支对应工程版本是否一致；跨版本 API 以本档规则 + `search_*_docs` 为准。
- 本库已从其中提炼的自写短文：`authored/custom-commands-brigadier`、`authored/ore-generation-worldgen`、`authored/saveddata-world-persistence`（SavedData 部分另核对 NeoForge 官方文档）、`authored/neoforge-payload-networking`、`authored/data-attachments-vs-alternatives`。

API 细节请用 `search_neoforge_docs` / `search_fabric_docs` / `search_forge_docs`。
