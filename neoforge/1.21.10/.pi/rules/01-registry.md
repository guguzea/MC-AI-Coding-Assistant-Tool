---
description: 01 — 注册（NeoForge 1.21.10）
globs:
alwaysApply: true
status: ready
---

# 01 — 注册（NeoForge 1.21.10）

来源：search_neoforge_docs `concepts/registries` / `items` / `blocks`（version=1.21.10）。
...
→ 核不到 → search_neoforge_docs version=1.21.10
```
