---
description: 06 — 网络（NeoForge 1.21.10）
---

# 06 — 网络（NeoForge 1.21.10）
...
发送：客户端 **`ClientPacketDistributor.sendToServer`**（不是本档的 `PacketDistributor.sendToServer`）。服务端仍用 `PacketDistributor.sendToPlayer` / `sendToPlayersTrackingChunk` / `sendToAllPlayers`。
