...
必须 search_neoforge_docs query=networking version=1.21.10。核不到的类名写成「以该版文档为准，禁止默写」，不填伪造签名。

网络：search_neoforge_docs query=networking version=1.21.10。本档核实 RegisterPayloadHandlersEvent + PayloadRegistrar（networking/payload）。禁止 SimpleChannel。禁止把 1.20.4 单数 RegisterPayloadHandlerEvent 当本档。
