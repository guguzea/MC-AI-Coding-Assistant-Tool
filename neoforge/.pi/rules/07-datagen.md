...
- **禁止**在运行时修改数据生成器输出
- 生成的 JSON 文件放在 `src/main/data/`（数据包）或 `src/main/assets/`（资源包）
...
└── {package}/
    └── datagen/
        ...
        ├── ModBlockLootSubProvider.java # 战利品子提供器
        └── ModTags.java                 # 标签
```
...
- 熔炉配方使用 `SimpleCookingRecipeBuilder.smelting()`（在 `buildRecipes()` 内）
...
IF 生成战利品表
  → 使用 LootTableProvider + BlockLootSubProvider
  → LootPool.Builder
...
> 注意：标签需要正确的命名空间。在 NeoForge 1.20.4 中，`#minecraft:xxx` 是原版标签，`#forge:xxx` 是 Forge 通用标签，`#{modid}:xxx` 是 mod 专属标签。
