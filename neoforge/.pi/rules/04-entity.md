...
- 渲染器必须放在 `client` 包下
- 必须通过 `EntityRenderersEvent.RegisterRenderers` 注册（不是直接调用 `EntityRenderers.register()`）
- 必须在 `FMLClientSetupEvent` 中订阅 `EntityRenderersEvent.RegisterRenderers`（不是 `FMLCommonSetupEvent`）
...
public class ModAttributes {
    ...
    public static final DeferredRegister<Attribute> ATTRIBUTES =
        DeferredRegister.create(NeoForgeRegistries.Keys.ATTRIBUTES, ExampleMod.MOD_ID);
...
> 注意：`NeoForgeRegistries.Keys.ATTRIBUTES` 是注册表的键，`DeferredRegister.create()` 需要传入 `ResourceKey<IRegistry<Attribute>>` 而非直接的 `IRegistry`。属性注册在 mod 构造函数中通过 modEventBus 完成，而不是在 `FMLCommonSetupEvent` 中。
...
public class ClientSetup {
    ...
    public static void onClientSetup(FMLClientSetupEvent event) {
        // 渲染器通过 EntityRenderersEvent.RegisterRenderers 注册
        // 参见 ClientSetup 中的 subscribe
    }
...
public class MyEntityRenderer extends LivingEntityRenderer<MyEntity, MyEntityModel<MyEntity>> {
    public MyEntityRenderer(EntityRendererProvider.Context context) {
        ...
        this.addLayer(new ItemInHandLayer<>(this, context.getItemRenderer(), context.getBlockRenderer()));
    }
...
> 注意：
> 1. `EntityRenderers` 在 NeoForge 1.20.4 中通过 `EntityRenderersEvent.RegisterRenderers` 事件注册（不是 `ClientSetup` 中直接调用）
> 2. `ItemInHandLayer` 需要 `EntityRendererProvider.Context` 中的 `ItemRenderer` 和 `BlockRenderer`
> 3. 纹理文件路径：`textures/entity/my_entity.png`（对应 `assets/{modid}/textures/entity/my_entity.png`）
> 4. `EntityModelLayer` 用于定义层标识，通过 `EntityRenderersEvent.RegisterLayerDefinitions` 注册，渲染器中用 `context.bakeLayer()` 获取
