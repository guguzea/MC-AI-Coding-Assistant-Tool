...
- 所有注册必须通过 **NeoForge 事件系统**（最终由 `RegisterEvent` 触发）
- mod ID 必须与 `neoforge.mods.toml` 中的 `modId` 完全一致
- **推荐使用 `DeferredRegister`**：`DeferredRegister` 是 NeoForge 官方推荐的注册方式，自 Forge 1.18 起即可用于所有注册表，1.20.4 完全支持
...
|----------|---------------------------|--------|
| 方块 | `DeferredRegister.create(NeoForgeRegistries.BLOCKS, MOD_ID)` | modEventBus |
| 物品 | `DeferredRegister.create(NeoForgeRegistries.ITEMS, MOD_ID)` | modEventBus |
| 方块实体 | `DeferredRegister.create(NeoForgeRegistries.BLOCK_ENTITY_TYPES, MOD_ID)` | modEventBus |
| 实体类型 | `DeferredRegister.create(NeoForgeRegistries.ENTITY_TYPES, MOD_ID)` | modEventBus |
| 附魔 | `DeferredRegister.create(NeoForgeRegistries.ENCHANTMENTS, MOD_ID)` | modEventBus |
| 流体 | `DeferredRegister.create(NeoForgeRegistries.FLUIDS, MOD_ID)` + `FluidType` | modEventBus |
| 粒子类型 | `DeferredRegister.create(NeoForgeRegistries.PARTICLE_TYPES, MOD_ID)` | modEventBus |
| 声音事件 | `DeferredRegister.create(NeoForgeRegistries.SOUND_EVENTS, MOD_ID)` | modEventBus |

### mod ID 规范

```java
// ✅ 正确：在 neoforge.mods.toml 中声明的 mod ID
...
```java
// ✅ 正确：全小写，下划线分隔
BLOCKS.register("my_awesome_block", () -> new Block(...));

// ❌ 错误：驼峰命名
BLOCKS.register("myAwesomeBlock", () -> new Block(...));

// ❌ 错误：大写开头
BLOCKS.register("MyAwesomeBlock", () -> new Block(...));

// ❌ 错误：使用横杠
BLOCKS.register("my-awesome-block", () -> new Block(...));
```
...
IF 平台 = Fabric
  → 跳转 fabric/1.20.4/AGENTS.md（Fabric 使用 Registry.register() in onInitialize）

IF 平台 = Forge (1.20.1)
  → 跳转 forge/1.20.1/AGENTS.md（Forge 使用 @Mod 注解模式）
```
...
```
IF 注册 方块
  → 使用 DeferredRegister.create(NeoForgeRegistries.BLOCKS, MOD_ID)
  → 在 BLOCKS.register() 中注册
  → 如需物品形态，在 ITEMS.register() 中注册同名 BlockItem

IF 注册 物品
  → 使用 DeferredRegister.create(NeoForgeRegistries.ITEMS, MOD_ID)
  → 在 ITEMS.register() 中注册

IF 注册 方块实体（BlockEntity）
  → 方块实现 EntityBlock 接口，重写 newBlockEntity()
  → 使用 DeferredRegister.create(NeoForgeRegistries.BLOCK_ENTITY_TYPES, MOD_ID)
  → 在 BLOCK_ENTITY_TYPES.register() 中注册 BlockEntityType

IF 注册 实体（Entity）
  → 使用 DeferredRegister.create(NeoForgeRegistries.ENTITY_TYPES, MOD_ID)
  → 在 ENTITY_TYPES.register() 中注册 EntityType
  → 必须在 neoforge.mods.toml 中声明 entity 字段
...
IF 报错 "Registry name must be in namespace:name format"
  → DeferredRegister 自动前缀 modId，无需手动拼接
...
public class ExampleMod {
    ...
    // 注册方块对应的 ItemBlock（同名，NeoForge 自动关联）
    ...
    public ExampleMod(NeoForgeAddonPlugin context) {
        IEventBus modEventBus = context.getBootstrapContext().getEventBus(ExampleMod.class);
...
public class ModBlocks {
    ...
    public static final DeferredRegister<Block> BLOCKS =
        DeferredRegister.create(NeoForgeRegistries.BLOCKS, ExampleMod.MOD_ID);
...
public class ModItems {
    ...
    public static final DeferredRegister<Item> ITEMS =
        DeferredRegister.create(NeoForgeRegistries.ITEMS, ExampleMod.MOD_ID);
...
> 注意：
> - DeferredRegister 在 modEventBus 上注册后，会在正确的 RegistryEvent 时机自动执行所有 register() 调用
> - BlockItem 与方块使用相同 registry name，NeoForge 自动关联，无需手动设置
> - `RegistryObject.get()` 可在 lambda 内部安全使用，此时对象已完成注册

---

## NeoForge 1.20.4 注册方式对比
...
| 适用场景 | 所有标准 NeoForge 注册 | 高级自定义或遗留代码迁移 |
