...
- **必须使用 Java 17**（NeoForge 1.20.4 要求；Java 21 仅在 Minecraft 1.20.6+ 才需要）
...
- ❌ 不要修改 `build.gradle` 中的 `minecraft` 和 `neoforge` 版本号（这两个版本号必须与项目目标版本匹配）
- ❌ 不要删除或注释 `minecraft` 和 `neoforge` 配置块中的任何必需行
- ❌ 不要混用 Fabric 的 `loom` 插件和 NeoForge 的 `neogradle` 插件
...
- `gradle.properties` 必须包含：
  ...
  mappings_version=20240527.185332
  loader_version=20.4
  ```
  > 注意：
  > - `build.gradle` 中引用时用 `mapping_version`（不带 s），属性名必须与 `build.gradle` 中的 `${property名}` 完全一致
  > - `loader_version` 填 NeoForge 版本号，与 neoforge.mods.toml 中的 `loaderVersion` 字段对应
- **禁止在 `build.gradle` 中直接写版本号**，必须引用 `${minecraft_version}` 等属性

### Mappings 约束

- NeoForge 1.20.4 默认使用 **MCP**
- `gradle.properties` 中的 `mappings_version` 对应 MCP 版本
- **禁止**在 `build.gradle` 中切换到 `yarn` 或 `parchment`（除非用户明确要求且项目已适配）
...
src/main/resources/
├── META-INF/
│   └── neoforge.mods.toml  # NeoForge 元数据（必需）
...
IF 这是新项目（没有 build.gradle）
  → 使用 neoforge/scaffold/ 中的模板生成项目骨架
  → README_AI.md 包含每个文件的职责说明

IF 这是已有项目
  → 读取 gradle.properties 获取版本配置
  → 读取 build.gradle 确认 mappings 和插件配置
  → 读取 neoforge.mods.toml 确认 modId

IF 构建失败且报错涉及版本号
  → 检查 gradle.properties 中的版本是否匹配
  → 典型错误：「net.neoforged.fml.loading.FMLError」通常是 neoforge_version 不兼容
```

### Decision: 遇到 Gradle 构建报错时的处理顺序

```
IF 报错包含 "Could not resolve net.neoforged"
  → 检查网络连接，或配置 Maven 镜像
  → 检查 gradle.properties 中 neoforge_version 是否有效

IF 报错包含 "Incompatible Java version"
  → 确认使用 Java 17，不兼容 Java 8 或 Java 21（仅 1.20.6+ 才需要 21）

IF 报错包含 "Could not find net.neoforged:neoforge"
  → 确认 build.gradle 中 minecraft_version 和 neoforge_version 版本匹配
  → NeoForge 版本对应关系：1.20.4 → 20.4.x
...
## 示例：正确的 build.gradle 结构（NeoForge 1.20.4）
...
## neoforge.mods.toml 字段说明

生成或修改 `neoforge.mods.toml` 时，关注以下字段：
...
modId = "neoforge"           # 必需
mandatory = true              # 必需
versionRange = "[20.4,)"      # 必需：NeoForge 版本范围
...
- `loaderVersion` 的格式：`"[20.4,)"` 表示 NeoForge 20.4 及以上
