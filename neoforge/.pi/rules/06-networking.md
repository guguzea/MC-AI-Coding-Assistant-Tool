...
// sendToServer() 是 SimpleChannel 的快捷方法，等同于 send(PacketDistributor.TRACKING_ENTITY.with(...), msg)
```
...
// 注册（5 参数，最后一项是 BiConsumer<MSG, Supplier<NetworkEvent.Context>>）
...
## NeoForge 1.20.4 网络包注意事项

NeoForge 1.20.4 的网络包包名为 `net.neoforged.neoforge.network`，与 Forge 的 `net.minecraftforge.network` 不同：
