...
└── src/main/
    ...
    └── resources/
        ├── META-INF/
        │   └── neoforge.mods.toml   # NeoForge 元数据（modId / 版本 / 依赖声明）
...
```properties
# ---- Minecraft / NeoForge 版本（必须与 NeoForge 版本匹配）----
...
mapping_version=20240527.185332
...
> **AI 注意**：修改 `mod_id` 后，必须同步修改 `neoforge.mods.toml`、`ExampleMod.java` 中的 `MOD_ID` 常量和所有 `DeferredHolder` 注册名。
...
> - `BLOCKS.register()` 在 modEventBus 注册后才执行，不要在 lambda 之外使用 `EXAMPLE_BLOCK`
...
### 禁止在 lambda 表达式之外引用 DeferredHolder
...
public static final DeferredHolder<Item, BlockItem> MY_BLOCK_ITEM = ITEMS.register("my_block",
...
NeoGradle 的插件仓库配置由 Gradle 管理，修改会导致构建失败。
...
> **AI 注意**：`neoforge.mods.toml` 中的 `modId`（如 `${mod_id}`）会在 Gradle 构建时替换为 `gradle.properties` 中的值。不要手动修改 `neoforge.mods.toml` 中的 `modId`，修改 `gradle.properties` 即可。
