...
## 错误：在 lambda 外部引用 DeferredHolder

**症状：** `NullPointerException` 或 `DeferredHolder.get() returns null`
...
**原因：** `DeferredHolder` 在静态初始化时可能尚未解析，只有在 `register(modEventBus)` 之后才有值。

**正确方案：**
```java
// ✅ 在 DeferredHolder 的 lambda 内部引用（延迟到注册完成后）
...
## 错误：mod ID 与 neoforge.mods.toml 不一致
...
**说明：** 在 1.18+ 的 NeoForge 中，实体注册有两个时期：
