...
> 本节是机制路线：vanilla（含 NeoForge 基础）**没有**开箱即用的流式音频 API、自定义声道或混音广播接口。具体类与方法签名一律以 `search_neoforge_docs` 为准，不要凭记忆补签名。

**先澄清事实**：sounds.json 的 `"stream": true` 只决定**加载方式**（流式读盘 vs 整文件进内存），它不是流式播放 API，也不提供声道/混音控制。声音最终由客户端声音引擎播放；服务端只负责「告诉客户端放什么」——广播性声音仍走 `Level#playSound`（服务端，向附近玩家同步，本文件上方已核实）；客户端触发的本地声音走客户端侧本地播放入口（Forge 1.20.1 语义对应 `Level#playLocalSound` / `ClientLevel`，NeoForge 侧入口名以 `search_neoforge_docs` 复核）。

### 路线 1：SoundInstance 级细粒度控制（以 search_neoforge_docs 为准）

- 机制：向客户端声音系统提供自定义 `SoundInstance` 实例，自行掌控播放、循环、停止与衰减；它消费的仍是 sounds.json 里已注册的 sound 条目。
- 适用：持续型/可打断的环境音、循环音。
- 启动点在客户端：服务端触发需经 `mc-networking` 发包。

### 路线 2：外界音频库集成（以 search_neoforge_docs 为准）
...
- ❌ 自定义声道/混音找不到 API 就硬编码音量 → 先 `search_neoforge_docs`
- ❌ 客户端逻辑依赖「驻留的 SoundEvent 引用」判断条目可用 → 只要 sounds.json 里有条目，客户端可直接引用，不必驻留 SoundEvent（Forge 1.20.1 官方注；NeoForge 以 `search_neoforge_docs` 复核）
- ✅ 能用普通 `sounds.json` + `Level#playSound` 表达的，先走常规路径，再考虑高级方案
