# =============================================================================
# MC Skill Sync Script — neoforge/1.20.4
#
...
function Normalize-Content {
    param([string]$text)
    # Replace: `neoforge/1.20.4/.cursor/rules/XX-name.mdc` → `XX-name.mdc`
    $text = $text -replace '`neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)`', '`$1`'
    # Replace: neoforge/1.20.4/.cursor/rules/XX-name.mdc (no backticks)
    $text = $text -replace 'neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '$1'
    # Replace: 参考 neoforge/1.20.4/.cursor/rules/XX-name.mdc → 参考 `XX-name.mdc`
    $text = $text -replace '参考 neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '参考 `$1`'
    # Replace: 参见 `neoforge/1.20.4/.cursor/rules/XX-name.mdc` → 参见 `XX-name.mdc`
    $text = $text -replace '参见\s*`?neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)`?', '参见 `$1`'
    # Replace: 参考 neoforge/.../XX-name.mdc (no backticks, no space) → 参考 `XX-name.mdc`
    $text = $text -replace '参考\s+neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '参考 `$1`'
    # Replace:  → 参考 neoforge/1.20.4/.cursor/rules/XX-name.mdc  (in code blocks)
    $text = $text -replace '→\s+参考\s+neoforge/1\.20\.4/\.cursor/rules/(\d{2}-[a-z-]+\.mdc)', '→ 参考 `$1`'
    # Replace forge references
    $text = $text -replace 'Forge', 'NeoForge'
    $text = $text -replace 'forge', 'neoforge'
    $text = $text -replace '1\.20\.1', '1.20.4'
    return $text
}
