...
- **必须使用 Java 8**（Minecraft 1.16.5 运行在 Java 8；Java 16 从 Minecraft 1.17 起才要求）
- `build.gradle` 中声明 `sourceCompatibility = JavaVersion.VERSION_1_8`（或 `options.release = 8`）
...
IF 这是新项目（没有 build.gradle）
  → 使用 fabric/1.16.5/scaffold/ 中的模板生成项目骨架
...
IF 报错包含 "Incompatible Java version"
  → 确认使用 Java 8（1.16.5 游戏运行时是 Java 8；用 Java 16/17 编译的模组无法在 1.16.5 加载）

IF 报错包含 "Could not find net.fabricmc:yarn"
  → 确认 gradle.properties 中 yarn_mappings 版本格式正确（如 1.16.5+build.10）
...
## 示例：正确的 build.gradle 结构（Fabric 1.16.5）

```groovy
plugins {
    id 'fabric-loom' version '0.10.31'  // ✅ 1.16.5 专用版本
    id 'maven-publish'
    id 'eclipse'
    id 'idea'
}
