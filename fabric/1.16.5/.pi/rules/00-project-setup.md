...
- **必须使用 Java 16**（Fabric 1.16.5 要求）
- `build.gradle` 中声明 `sourceCompatibility = JavaVersion.VERSION_16`
...
- `gradle.properties` 必须包含：
  ...
  fabric_api_version=0.16.24+build.4-1.16.5
  ```
...
IF 报错包含 "Incompatible Java version"
  → 确认使用 Java 16，不兼容 Java 8 或 Java 17
