...
- 容器屏通过 `ScreenRegistry.register（或原版 HandledScreens.register）` 注册
...
public class MyScreen extends Screen {
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        this.textRenderer.drawWithShadow(matrices, new LiteralText("Hello Fabric!"),
            width / 2 - 50, height / 2 - 20, 0xFFFFFF);
...
public class ExampleModClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        // 1.16+ 原版也可以 HandledScreens.register；Fabric 文档常用 ScreenRegistry
    }
}
```
...
Yarn 移位是 `transferSlot(PlayerEntity, int)`（[yarn 1.16.5 ScreenHandler.mapping](https://github.com/FabricMC/yarn/blob/1.16.5/mappings/net/minecraft/screen/ScreenHandler.mapping)）。wiki 现页 `tutorial:screenhandler` 写 `quickMove`，那是 **1.19.4+** Yarn 名，不要抄进本档。
容器屏：FAPI `ScreenRegistry.register`（本档 loader-api 已核）或原版 `HandledScreens.register`。`ScreenEvents.AFTER_INIT` 是 FAPI 静态字段（[fabric 1.16 ScreenEvents.java](https://github.com/FabricMC/fabric/blob/1.16/fabric-screen-api-v1/src/main/java/net/fabricmc/fabric/api/client/screen/v1/ScreenEvents.java)）。
...
public class MyScreenHandlerFactory implements NamedScreenHandlerFactory {
    ...
    public Text getDisplayName() {
        return new LiteralText("My Block");
    }
}
...
需要同步坐标等到客户端时，实现 Fabric `ExtendedScreenHandlerFactory`，在 `writeScreenOpeningData` 里写 `PacketByteBuf`。

```java
public class MyPosScreenFactory implements ExtendedScreenHandlerFactory {
