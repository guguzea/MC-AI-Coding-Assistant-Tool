...
Fabric 1.16.5 的事件系统与 1.20.x 不同，优先使用 Fabric API 提供的事件模块：

```groovy
modImplementation "net.fabricmc.fabric-api:fabric-events-attack-v0:${project.fabric_api_version}"
modImplementation "net.fabricmc.fabric-api:fabric-events-interaction-v0:${project.fabric_api_version}"
```

```java
// Fabric Events API 扩展（1.16.5 版本）
// 事件模块 API 在 1.16.5 较为有限，优先使用内置事件系统
```
