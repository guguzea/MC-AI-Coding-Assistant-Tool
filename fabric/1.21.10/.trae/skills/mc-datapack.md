...
> 一手来源（2026-09-19 核实）：Minecraft Wiki《Pack format》数据包格式表（https://minecraft.wiki/w/Pack_format ，1.21.9–1.21.10 = 88.0）与《Data pack》目录改名记载（https://minecraft.wiki/w/Data_pack ，1.21 起目录改单数）。本技能不含 Java API——数据包是 JSON 面；Java 侧数据生成走 `mc-datagen`。
...
→ 版本 → 1.21.9–1.21.10 数据包 pack_format = 88.0（资源包为 69.0）
→ JSON 写完 → validate_datapack_json 校验，再进游戏 /reload
```

## pack.mcmeta（已核实，出处见上）

1.21.9 起 **pack_format 引入小版本号**（非破坏性改动递减小版本）：1.21.9–1.21.10 数据包格式是 **88.0**、资源包 **69.0**（wiki 原话「Pack format now includes minor versions, which are incremented instead of the major version when non-breaking changes are made」）。写 `pack.mcmeta` 时以本版 vanilla `pack.mcmeta` 的实际写法为准。
...
- `pack_format` 填邻版数字（1.21.9–1.21.10 数据包是 **88.0**，不是 81/61；且注意小版本号新制）。
- 写 1.20.x 时代的复数目录（`recipes/`、`loot_tables/`、`tags/items/`）——1.21 起必须单数。
- 把 Java 类名问题带到本技能：数据包没有类名，类名核实走 `search_fabric_docs(version=1.21.10)`。
