...
> 本档正文的类名/签名只来自 `data/fabric_1.21.10` 本档语料：页面 `fabric-docs/1.21.10/processed/develop_items_potions.md`，代码 `reference/1.21.10/src/main/java/com/example/docs/potion/ExampleModPotions.java`。页内没有的签名一律留 `TODO(未核实)`，禁止用邻版（1.21.8 / 1.21.11）补全。
...
- `FabricPotionBrewingBuilder` 与 `PotionBrewing` 在本档参考代码 import 面里**都不存在**，按页面 prose 写 import 会编译失败。
- 盘上唯一真实的 Fabric API 入口：`net.fabricmc.fabric.api.registry.FabricBrewingRecipeRegistryBuilder`。
- 逐签名核对走 `query_loader_api(platform=fabric, minecraftVersion=1.21.10)`。

## 本档参考类的命名家族

本档语料的参考类是 `ExampleMod*` 家族：入口 `com.example.docs.ExampleMod`（提供 `MOD_ID`）、效果 `com.example.docs.effect.ExampleModEffects`、药水 `com.example.docs.potion.ExampleModPotions`。别档语料用的是 `FabricDocsReference*` 家族——那是别的版本档的文件名，不要拿本档 import 去凑。

## 注册 Potion

注册进 `BuiltInRegistries.POTION`；本档 `Registry.register(...)` 返回裸 `Potion`：
...
- 本档 `Potion` 构造形态是**双参**：`new Potion(String, MobEffectInstance...)`，示例第一参数 `"tater"`。页面只说「declaring a field to store your `Potion` instance」，未提其他重载 ⇒ `// TODO(未核实)`。
- 注意本档**没有**用 `Registry.registerForHolder`（那是 1.21.11 档页面的写法），本档字段类型就是 `Potion`，Holder 在酿造那一步现取。
- 标识符工厂：`ResourceLocation.fromNamespaceAndPath(ExampleMod.MOD_ID, "tater")`。
...
本档参考资源命名空间是 `example-mod`（`reference/1.21.10/src/main/resources/assets/example-mod/...`），但该目录下**没有 lang 文件**，页面也未给任何药水语言键 ⇒ `// TODO(未核实)`。显示名请 `search_fabric_docs(version="1.21.10")` 或反编译原版资源核实后再写，禁止抄别档的键。药水模型/纹理本档参考资源同样没有。
...
- `Registry.registerForHolder` 与 `Holder<Potion>` 字段形态：**本档页面与代码都没出现过**（别档才用），本档不写。
...
- 附魔效果面：同档另有 `develop_items_custom-enchantment-effects.md`，与本主题不同面，未在此引。
...
- 反模式：`09-anti-patterns.mdc`；本档核实表：`fabric/1.21.10/knowledge/common/verified-api-1.21.10.md`
- 全文核对：`get_fabric_doc_full(version="1.21.10", id="develop_items_potions")`
