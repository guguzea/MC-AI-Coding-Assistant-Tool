...
> ⚠️ **1.17.x 食物 API**：`FoodComponent.Builder` 使用 `.hunger(int)` 和 `.saturation(float)` 方法，没有 `.saturationModifier()` 和 `.snack()` 方法（这些是 1.20+ 新增的）。
...
```java
// 剑
private static final Item COPPER_SWORD =
    Registry.register(Registry.ITEM, new Identifier(MOD_ID, "copper_sword"),
        new SwordItem(MyToolMaterial.COPPER, 3, 1.6f,
            new Item.Settings().maxDamage(250)));

// 镐
private static final Item COPPER_PICKAXE =
    Registry.register(Registry.ITEM, new Identifier(MOD_ID, "copper_pickaxe"),
