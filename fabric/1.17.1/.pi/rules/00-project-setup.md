...
- `build.gradle` 中声明 `java.toolchain.languageVersion = JavaLanguageVersion.of(16)` 或 `sourceCompatibility = JavaVersion.VERSION_16`（A-43 更正：Minecraft 1.17.1 是 **Java 16** 时代，原写 17 与上一条自相矛盾）
- Gradle Wrapper 版本不低于 **Gradle 7.3**（适配 Loom 0.10.31 —— A-43 更正：原写 0.11-SNAPSHOT，与 scaffold 实测钉值不符且 SNAPSHOT 不可复现）
...
IF 这是新项目（没有 build.gradle）
  → 使用 fabric/1.17.1/scaffold/ 中的模板生成项目骨架
...
## 示例：正确的 build.gradle 结构（Fabric 1.17.1）
