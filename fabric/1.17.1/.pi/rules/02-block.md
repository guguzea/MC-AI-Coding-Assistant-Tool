...
> 适用版本：Fabric 1.17.1
> FAPI object.builder 有 `copyOf` / `of(Material)` / `breakByTool` / `breakByHand`。**没有** `create()`。颜色用 `materialColor(MapColor.STONE_GRAY)`（Yarn 无 `MapColor.STONE` / `MaterialColor.STONE`）。
...
IF 简单静态方块
  ...
  → 或 FabricBlockSettings.of(Material.STONE)
