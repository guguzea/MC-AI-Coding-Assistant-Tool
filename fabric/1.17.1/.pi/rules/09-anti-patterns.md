...
```groovy
// ❌ 错误：1.17.x 使用了 1.20.x 的 Loom 版本
id 'fabric-loom' version '1.4-SNAPSHOT'  // ❌ 不兼容

// ✅ 正确：1.17.x 使用 Loom 0.11-SNAPSHOT
...
| `Could not find net.fabricmc:yarn` | yarn 版本号格式错误 | 格式：`1.17.1+build.65` |
| `FabricLoader not found` | 依赖缺失 | 检查 `fabricloader` 在 depends 中 |
| `No such registry` | Registry 类型错误 | 确认 `Registry.XXX` 正确（非 Registries） |
