...
# GUI 开发（Fabric 1.17.1）

Yarn 移位是 `transferSlot`。不要抄 wiki 现页 `quickMove`。容器屏：`ScreenRegistry.register` 或 `HandledScreens.register`。
...
// 注册 ScreenHandler
// ⚠️ 1.17.x 使用 Registry.SCREEN_HANDLER，不是 Registries.MENU
