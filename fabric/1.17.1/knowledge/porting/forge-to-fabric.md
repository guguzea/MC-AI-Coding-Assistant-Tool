...
> 本指南帮助将 Forge 模组移植到 Fabric 平台（Fabric 1.17.1）。
...
public class ExampleMod implements ModInitializer {
    ...
    public void onInitialize() {
        // 所有注册在此执行
        // 注意：1.17.x 使用 Registry.ITEM（静态字段），不是 Registries.ITEM
...
// ✅ Fabric（1.17.x 使用 Registry.BLOCK 静态字段，不用 Registries 类）
...
> ⚠️ **1.17.x 特殊注意**：`Registries` 类在 1.17.x 中不存在！应使用 `Registry.BLOCK`、`Registry.ITEM` 等静态字段。
