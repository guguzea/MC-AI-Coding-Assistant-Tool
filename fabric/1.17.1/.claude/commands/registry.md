...
本文件描述 Fabric 1.17.1 平台上进行注册系统（Registry）开发时所需掌握的核心 API 和常用命令。
...
- `registry`：要注册到的注册表（如 `Registry.BLOCK`、`Registry.ITEM`）
...
// 注册方块
Registry.register(Registry.BLOCK, new Identifier(MOD_ID, "my_block"), new Block(FabricBlockSettings.create()));

// 注册实体类型
Registry.register(Registry.ENTITY_TYPE, new Identifier(MOD_ID, "my_entity"), EntityType.Builder.create(...).build());
```
...
Registry.BLOCK           // 方块
Registry.ITEM           // 物品
Registry.ENTITY_TYPE     // 实体类型
Registry.PARTICLE_TYPE   // 粒子类型
Registry.SOUND_EVENT    // 声音事件
Registry.MOB_EFFECT     // 状态效果
Registry.BLOCK_ENTITY_TYPE  // 方块实体类型
Registry.ENCHANTMENT    // 附魔
Registry.ITEM_GROUP     // 物品栏分组
Registry.POTION         // 药水
Registry.SCREEN_HANDLER // 屏幕处理器
Registry.FLUID         // 流体
Registry.FLUID_TYPE    // 流体类型
```
...
private static final RegistrySupplier<BlockEntityType<MyBlockEntity>> MY_BLOCK_ENTITY = 
    Registry.register(Registry.BLOCK_ENTITY_TYPE, new Identifier(MOD_ID, "my_block"),
...
private static final RegistrySupplier<Item> ITEM = 
    Registry.register(Registry.ITEM, id, new Item(...));

// Fabric API 提供（需要添加 fabric-api 依赖）
private static final RegistryObject<Item> ITEM = 
    RegistryObject.create(id, Registry.ITEM, MOD_ID);
```
...
public void onInitialize() {
    ...
    // 检查注册是否成功
    Item registered = Registry.ITEM.get(new Identifier(MOD_ID, "my_item"));
    if (registered != null) {
