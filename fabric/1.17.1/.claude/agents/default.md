...
```
.claude/rules/00-project-setup.mdc    → 项目结构与构建（Loom 0.11-SNAPSHOT）
.claude/rules/01-registry.mdc         → 注册系统（Registry.XXX 静态字段）
...
.claude/rules/07-datagen.mdc          → 数据生成（推荐手写 JSON）
