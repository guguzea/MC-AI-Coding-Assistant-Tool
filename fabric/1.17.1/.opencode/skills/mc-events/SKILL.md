---
name: mc-events
description: Fabric 1.17.1 事件系统。Event 回调、onInitialize 注册、本档版本差异。触发词：事件、Event、Callback、TickEvents、onInitialize
platform: fabric
version: "1.17.1"
dependencies: []
mappings: yarn
...
- 实体交互：`AttackEntityCallback.EVENT`；击杀回调是击杀者侧 `ServerEntityCombatEvents.AFTER_KILLED_OTHER_ENTITY`（**无 AFTER_DEATH**）；本档**没有** `ServerLivingEntityEvents`（约 1.19.4 才加入）。
- 方块状态判断用 `state.isOf(Blocks.X)`（1.17 起 API 形态，不是 `getBlock() ==`）。
