...
- `build.gradle` 中声明 `java.toolchain.languageVersion = JavaLanguageVersion.of(21)` 或 `sourceCompatibility = JavaVersion.VERSION_21`
- Gradle Wrapper 版本不低于 **Gradle 8.5**
...
IF 这是新项目（没有 build.gradle）
  → 使用 fabric/1.21.3/scaffold/ 中的模板生成项目骨架
...
IF 报错包含 "Incompatible Java version"
  → 确认使用 Java 21，1.21.x 不支持 Java 17

IF 报错包含 "Could not find net.fabricmc:yarn"
  → 确认 gradle.properties 中 yarn_mappings 版本格式正确（如 1.21.3+build.2）
...
## 示例：正确的 build.gradle 结构（Fabric 1.21.3）

```groovy
plugins {
    id 'fabric-loom' version '1.8'   // 与本档 scaffold/build.gradle:2 同值；旧示例写 1.3-SNAPSHOT 属 1.20.1 模板残留（见下方「Loom 版本口径」）
    id 'maven-publish'
    id 'eclipse'
    id 'idea'
}
...
tasks.named('jar').configure {
    manifest {
        attributes([
            "Fabric-ModuleType": "Lib",  // 装饰性 jar manifest 属性，Loader 不读；模组元数据一律看 fabric.mod.json（本档官方模板血统的 1.21.11/scaffold/build.gradle 无此属性）
...
> **Loom 版本口径（F185，读值日期 2026-09-13）**：本档原先三处互不相同 —— 本节示例与 `knowledge/antipatterns/gradle.md:41` / `:113`、`.cursor/skills/mc-kotlin.md:23` 写 `1.3-SNAPSHOT`，而唯一真机构建过的 `scaffold/build.gradle:2` 钉 `1.8`。现统一取 **本档 scaffold 实钉值 `1.8`**；`1.3-SNAPSHOT` 的上游最后一次发布是 2024-01-04（`https://maven.fabricmc.net/net/fabricmc/fabric-loom/maven-metadata.xml` + 该目录 `Last-Modified`），属 1.20.x 时代，不是 1.21.3 的可用组合。**未核实**：上游 1.8 线只有 `1.8.0-alpha.*` 与 `1.8.1`–`1.8.13`，探测不到裸 `1.8` 的 pom（`net/fabricmc/fabric-loom/1.8/fabric-loom-1.8.pom` → HTTP 404）；scaffold 裸 `1.8` 是否可解析归 S25a 处理，本代理不改钉值。
