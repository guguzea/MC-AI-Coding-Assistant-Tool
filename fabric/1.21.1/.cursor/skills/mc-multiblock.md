...
# mc-multiblock（Fabric 1.21.1）

多方块结构**没有平台 API** —— Fabric API、vanilla 都没有「多方块注册表 / 成型状态 / 自动结构校验」这种东西。这是一件**模式技能**：下面只写「用本档已核实的 vanilla 名把这个模式落下来」，不替你发明 API。

## 本档出处口径

类名有两条合法证据：本档 `data/fabric_1.21.1/**` 语料逐字命中，或 `data/fabric_1.21.1/mappings/yarn-mappings.sqlite` 命中。映射库只证**存在**，不证签名与用法。

自查结果（`assert-skill-yarn-attest --pack=fabric_1.21.1`）：
...
- ⚠️ 本档 `data/fabric_1.21.1/reference/**` 是 **Mojang 官方名**源码树：`Registry.register(BuiltInRegistries.BLOCK_ENTITY_TYPE, id, BlockEntityType.Builder.<T>of(...).build())`（`block/entity/ModBlockEntities.java:26`）——**注册链禁止照抄**，`of(...)` 与 `BuiltInRegistries` 不是本档 Yarn 名。同一棵树里 `block/custom/CounterBlock.java:47` 逐字出现 `world.getBlockEntity(pos)`，该方法名两套映射同名，可当本档出处。

本档唯一带页面逐字出处的 Yarn 签名（`data/fabric_1.21.1/fabric-wiki/1.21.1/processed/tutorial_blocks.md:373-375`）就是「整体外形」这一段：
...
→ 控制器宿主 → BlockEntity 经 BlockEntityType 注册，注册入口走 Registries
   ...
   // TODO(未核实)：reference 树的 of(...) / BuiltInRegistries 是官方名，禁止当 Yarn 签名照抄
   // TODO(未核实)：须 search_fabric_docs(version=1.21.1) 或 get_minecraft_source 逐个核实
→ 结构探测 → 以控制器 BlockPos 为原点，按 Direction 逐格比 BlockState
   // 本档页面命中：world.getBlockEntity(pos)（reference 1.21.1 CounterBlock.java:47）
   // TODO(未核实)：getBlockEntity / getBlockState 的返回类型与可空性
...
- **没有 formed-state API**：「是否成型」只是控制器自己的一份字段加你自己写的序列化。本档 `writeNbt` / `readNbt` 可用，`toTag` 自查**无出处**（那是 1.14.4 / 1.16.5 一代的名字）；序列化链的确切签名本档无页面出处，须核实。
...
- **没有跨档通用名**：`Level` / `ServerLevel` / `BlockGetter` / `ResourceLocation` 是 mojmap 名，本档自查全部 `映射·`（语料命中只来自官方名 reference 树，不是 Yarn 名）⇒ 本档写法对应 `World` / `ServerWorld` / `BlockView` / `Identifier`。本档 `Identifier` 走 `Identifier.of(...)`（构造器 private；见本档 `AGENTS.md` 命名规范与 `.cursor/rules/01-registry.mdc`），不要写 `new Identifier(...)`。
...
- 拿 Forge / NeoForge 教程或本档官方名 reference 树的调用链当本档 Yarn 写法。
...
- 核实路径：`search_fabric_docs(version=1.21.1)` → `get_fabric_doc_full`；vanilla 签名 `get_minecraft_source`（需 JDK 17+）或工程 `genSources`
