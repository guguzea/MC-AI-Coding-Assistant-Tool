...
# GUI 开发（Fabric 1.21.1）

Yarn 移位是 `quickMove`。客户端只用 `HandledScreens.register`（loader-api 无 `ScreenRegistry`）。额外数据：`ExtendedScreenHandlerFactory#getScreenOpeningData`，不是 `writeScreenOpeningData`。
...
- ❌抄 `ScreenRegistry.register` 或 `writeScreenOpeningData` — 本档无 ScreenRegistry；额外数据是 `getScreenOpeningData`
