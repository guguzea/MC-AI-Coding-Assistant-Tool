...
Yarn 1.21.1 第二参是 `SpawnLocation`：用 `SpawnLocationTypes.ON_GROUND`（[yarn SpawnLocationTypes.mapping](https://github.com/FabricMC/yarn/blob/1.21.1/mappings/net/minecraft/entity/SpawnLocationTypes.mapping)），不要抄 1.20 的 `SpawnRestriction.Location`。
属性字段仍是 `GENERIC_MAX_HEALTH` / `GENERIC_MOVEMENT_SPEED`（[yarn 1.21.1+build.3 javadoc](https://maven.fabricmc.net/docs/yarn-1.21.1%2Bbuild.3/net/minecraft/entity/attribute/EntityAttributes.html)）。不要把 1.21.3+ 去掉 GENERIC_ 前缀的名字抄进本档。
...
- 把 26.1 文档的 `MobCategory` / `Identifier.fromNamespaceAndPath` / `BuiltInRegistries` 抄进 Yarn 1.21.1
- 抄 `SpawnRestriction.Location` — 本档是 `SpawnLocationTypes.ON_GROUND`
