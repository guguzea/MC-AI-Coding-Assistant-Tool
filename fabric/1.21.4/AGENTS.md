# Fabric 1.21.4 — Agent 总纲

> 只适用于 **Fabric 1.21.4**。文档：search_fabric_docs version=1.21.4（先 list_fabric_versions）。**禁止**复制 `fabric/1.21.11` 或邻版 wiki。
...
| 注册 | Registry.register（search_fabric_docs query=registry version=1.21.4） |
...
- 需要构建：以**官方 develop 页**（https://fabricmc.net/develop/）与 `search_fabric_docs`（`version=1.21.4`）为准。
