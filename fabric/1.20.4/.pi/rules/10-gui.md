...
Yarn 移位是 `quickMove`（与 1.20.1 mapping 同名）。不要抄 `transferSlot`。
容器屏：`ScreenRegistry.register`（本档 loader-api 仍有）或 `HandledScreens.register`。额外数据：`writeScreenOpeningData`（1.20.4 loader-api 已核；1.21.1 才改成 `getScreenOpeningData`）。
