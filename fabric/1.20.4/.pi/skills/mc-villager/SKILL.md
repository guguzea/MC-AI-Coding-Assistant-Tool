...
> Wave D 技能骨架（fabric 1.20.4）。详细规则见对应 `.cursor/rules/` 与 MCP `search_fabric_docs` / 专题工具。
...
- 上表只证**类名存在与包路径**，**不证**方法名/参数/返回值。逐签名以本档语料为准：`search_fabric_docs version=1.20.4`，或 `get_minecraft_source`（需 JDK 17+）/ IDE `./gradlew genSources`。
