...
- 实体类继承 `Entity` 或其子类
- 使用 `MobCategory`（1.21 使用此枚举）
- 使用 `EntityDimensions.changing()` 表示可变化尺寸的实体
- 属性注册使用 `DefaultAttributeRegistry`
...
// 3. 在 onInitialize() 中注册属性
@Override
public void onInitialize() {
    DefaultAttributeRegistry.register(MY_PIG.get(),
...
public void onInitialize() {
    // ...属性注册后
    SpawnRestriction.register(MY_PIG.get(),
...
- ❌忘记注册属性 — 实体使用默认 20 HP，可能不符合预期
- ❌混用 `MobCategory.CREATURE` 和 `EntityCategory.CREATURE` — 1.21 使用 `MobCategory`
