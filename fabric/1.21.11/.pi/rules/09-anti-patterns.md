...
```java
// ❌ 错误：static {} 在没有任何 entrypoint 调用时就会跑，专用服务端/类加载顺序不可控
...
// ✅ 正确：onInitialize 触发静态注册（官方常见 ModItems.initialize()）
...
// ❌ 错误：依赖传递性混淆
dependencies {
    modImplementation("net.fabricmc.fabric-api:fabric-api:0.141.6+1.21.11")  // 不传递依赖
}

// ✅ 正确：API 需要传递，实现不需要
dependencies {
    modApi("net.fabricmc.fabric-api:fabric-api:0.141.6+1.21.11")  // 传递依赖
...
- `ScreenHandlerRegistry` / `ScreenRegistry` — 用 `Registry.register(Registries.SCREEN_HANDLER, …)` + `HandledScreens.register`；`ScreenHandlerType` 构造依赖 fabric-api access widener
- `FabricItemSettings` — 用原版 `Item.Settings`
- `FuelRegistry`（旧命令式 API）— 用本档 datagen / 燃料数据包，不要抄邻版 `FuelRegistry.INSTANCE.add`

```java
// ❌ 错误：这些类已从本档 Fabric API 移除
...
public class ExampleMod implements ModInitializer {
    @Override
    public void onInitialize() {
        MinecraftClient client = MinecraftClient.getInstance();  // 服务端崩溃！
    }
}
...
| `No such registry` | Registry 类型错误 | 确认 `Registries.XXX` 正确 |
