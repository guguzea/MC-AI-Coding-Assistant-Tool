...
- **所有注册**必须经由主入口触发：在 `onInitialize()`（或它调用的 `ModItems.initialize()` 等）里执行 `Registry.register()`。允许静态字段由 initialize 触发；**禁止**无 entrypoint 的类加载副作用注册
...
| 附魔 | DataGen `bootstrap()` + `register()`（`FabricDynamicRegistryProvider`）。`Registries.ENCHANTMENT` 只作 tag provider 的 `registryKey`。**不要**在 `onInitialize()` 里 `Registry.register(Registries.ENCHANTMENT, ...)` | DataGen |
...
IF 注册 附魔
  → 1.21.x 数据驱动：DataGen 生成器的 `bootstrap()` 里 `register()`，见 07-datagen
  → `Registries.ENCHANTMENT` 只给 `FabricTagProvider<Enchantment>` 当 registryKey
...
IF 附加数据（Attachment）
  → `AttachmentRegistry.create` / `createPersistent` / `createDefaulted` / `builder` + `AttachmentSyncPredicate.all / targetOnly / allButTarget`（FQCN `net.fabricmc.fabric.api.attachment.v1.*`，见 `1.21.11-fabric-api.json`：6 类，**无** `GlobalAttachments`）
  → 该页未写模块 id，`fabric-attachment-api-v1` 本档 **未核实**；依赖按 `fabric-api` 实测，不要照抄
  → 摘要首参为 intermediary `class_2960`，Yarn 名以该版映射为准，禁止默写
  → 不要用 NeoForge `NeoForgeRegistries.ATTACHMENT_TYPES` / `getData` / `setData`；不要用 Forge Capability
```
