...
IF 附加数据（Attachment）
  → `fabric-attachment-api-v1`：`AttachmentRegistry.create`（见 AGENTS「1.21.x 网络与 Attachment」与 `mc-capability` Skill）
  → 不要用 NeoForge `Registries.ATTACHMENT_TYPE` / `getData` / `setData`；不要用 Forge Capability
```
