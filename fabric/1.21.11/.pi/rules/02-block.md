...
- ❌ Yarn `SwordItem` / `PickaxeItem` / `MiningToolItem` / `DiggerItem` / `ToolItem`（1.21.11 Yarn 命名里**没有这些类**，也不存在「镐斧铲的父类是 X」这条叙述；工具改走 `ToolMaterial` 实例 + `Item.Settings` 工具组件，见 `03-item`）
