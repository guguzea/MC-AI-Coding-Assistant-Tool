...
本文件描述 Fabric 1.21.11 平台上进行实体（Entity）开发时所需掌握的核心 API 和常用命令。
...
`MobEntity.createMobAttributes()` 提供生物默认属性配置：`DefaultAttributeRegistry.register(MY_ENTITY, MobEntity.createMobAttributes().add(GENERIC_MAX_HEALTH, 30.0).add(GENERIC_MOVEMENT_SPEED, 0.25).add(GENERIC_ATTACK_DAMAGE, 2.0))`。

## 生成限制命令

### SpawnRestriction.register()

控制实体在世界中的自然生成需要设置生成限制。基本命令：`SpawnRestriction.register(entityType, location, heightmap, predicate)`。`location` 指定允许生成的位置：`ON_GROUND`（地面上）、`IN_AIR`（空中）、`IN_WATER`（水中）、`ON_GROUND_RESTRICED`（受限地面）。`heightmap` 指定用于检查的 `Heightmap.Type`：`MOTION_BLOCKING`（阻挡移动，包括液体）、`MOTION_BLOCKING_NO_LEAVES`（阻挡移动，不包括树叶）、`WORLD_SURFACE`（世界表面）。`predicate` 是 `EntityType.IEntityPredicate` 类型，检查实体是否可以生成。
