...
> **查 API 分工（勿混用）**：本档 **1.21.11 无 `query_api` Parchment 索引**（`query_api --version=1.21.11` → `DATA_UNAVAILABLE`；**勿当 Yarn 签名工具**）。Vanilla 源码/签名 → `get_minecraft_source` 或 IDE `genSources`；Yarn 名互转 → `convert_mapping`；Fabric API → `query_loader_api`；教程/用法 → `search_fabric_docs`（version=1.21.11）。
...
| `query_loader_api` | **Fabric API** 类摘要（`platform=fabric` + `minecraftVersion=1.21.11`） |
