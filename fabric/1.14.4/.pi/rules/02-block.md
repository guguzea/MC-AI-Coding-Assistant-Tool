...
IF 简单静态方块
  ...
  → 或 FabricBlockSettings.of(Material.STONE).build() / FabricBlockSettings.copy(Blocks.STONE).build()
...
FAPI `net.fabricmc.fabric.api.block.FabricBlockSettings`（loader-api 已核）：`of`、`copy(Block)`、`hardness(float)`、`resistance(float)`、`strength(float, float)`（**无**单参 `strength`）、`breakByTool(Tag)` / `breakByTool(Tag, miningLevel)`、`breakByHand(boolean)`、`materialColor`、`dropsLike`、`noCollision`、`friction` / `slipperiness`。本类 **没有** `requiresTool`、`solidBlock`、`suffocates`、`mapColor`（用地图色请用 `materialColor`）。**注意它是独立 builder、不是 `Block.Settings` 的子类**：交给 `new Block(...)` 前必须 `.build()`（返回 `AbstractBlock.Settings`）——本档 scaffold 真机构建实测 `incompatible types: FabricBlockSettings cannot be converted to Settings`。
