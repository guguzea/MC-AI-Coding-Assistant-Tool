...
```
IF 静态实体（不移动、不交互）
  → EntityType.Builder.create(EntityCategory.MISC)

IF 有行为实体（动物、怪物）
  → EntityType.Builder.create(EntityCategory.CREATURE)
...
public static final RegistryObject<EntityType<MyPigEntity>> MY_PIG = Registry.register(
    Registry.ENTITY_TYPE,
    new Identifier(MOD_ID, "my_pig"),
    EntityType.Builder.<MyPigEntity>create(
        MyPigEntity::new,
        EntityCategory.CREATURE
    )
    .size(0.9f, 1.4f)  // 宽, 高
    .build(new Identifier(MOD_ID, "my_pig").toString())
);
```
...
public void onInitialize() {
    // 注册属性
    ((LivingEntityAttributeRegistry) (Object) this).register(MY_PIG,
        LivingEntityAttributes.GENERIC_MAX_HEALTH,
        20.0
    );
    ((LivingEntityAttributeRegistry) (Object) this).register(MY_PIG,
        LivingEntityAttributes.GENERIC_MOVEMENT_SPEED,
        0.25
    );
    ((LivingEntityAttributeRegistry) (Object) this).register(MY_PIG,
        LivingEntityAttributes.GENERIC_ATTACK_DAMAGE,
        3.0
    );

    // 注册实体...
    Registry.register(Registry.ENTITY_TYPE, new Identifier(MOD_ID, "my_pig"), MY_PIG.get());
}
```
...
```java
// 使用 World spawn biomes 配置（1.14.4 方式）
// 在 EntityType 构造后，Minecraft 会自动处理基本生成
```
...
public class ExampleModClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        EntityRendererRegistry.register(MY_PIG.get(), (dispatcher, context) ->
...
- ❌混淆 `EntityCategory` 和 `MobCategory`（后者是 1.17+ 的名称）
