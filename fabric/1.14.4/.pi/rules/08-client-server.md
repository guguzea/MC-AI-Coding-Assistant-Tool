...
IF 判断当前环境是否为客户端
  ...
  → World.isClient（Yarn 字段；[yarn 1.14.4 World.mapping](https://github.com/FabricMC/yarn/blob/1.14.4/mappings/net/minecraft/world/World.mapping)）。不要用 MCP/Forge 的 `isRemote`
...
- ❌ 在共享代码中使用 `Minecraft.getInstance()` — 服务端无 `Minecraft` 类
- ❌ 混淆 `world.isRemote` 和 `world.isClient` — Yarn 1.14.4 用 **`world.isClient`**，不要抄 MCP `isRemote`
