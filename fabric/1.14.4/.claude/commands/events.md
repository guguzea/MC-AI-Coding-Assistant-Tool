...
- 实体交互：`AttackEntityCallback.EVENT`；实体加载 `ServerEntityEvents.ENTITY_LOAD`（1.14.4 时点存在）；本档**没有** `ServerLivingEntityEvents` / `ServerEntityCombatEvents`，死亡/受伤用实体方法或 Mixin。
