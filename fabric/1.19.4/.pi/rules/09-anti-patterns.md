...
// ❌ 错误：依赖传递性混淆
dependencies {
    modImplementation("net.fabricmc.fabric-api:fabric-api:0.87.2+1.19.4")  // 不传递依赖
}

// ✅ 正确：API 需要传递，实现不需要
dependencies {
    modApi("net.fabricmc.fabric-api:fabric-api:0.87.2+1.19.4")  // 传递依赖
...
| `Could not find net.fabricmc:yarn` | yarn 版本号格式错误 | 格式：`1.19.4+build.2` |
