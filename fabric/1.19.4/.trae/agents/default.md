...
> **查 API 分工（勿混用）**：`query_api` / `get_method_params` 查 **Vanilla 游戏类**（Parchment/Mojang 名 + Javadoc），**不是 Yarn 索引**，**不含** `net.fabricmc.fabric.api.*`。本档 **1.19.4** 在 Parchment 索引内（约 1.16.5–1.20.4）。多数 Vanilla 类名与 Yarn 相同，但请以工程 Yarn 为准；不一致时用 `convert_mapping`。Fabric API → `query_loader_api`；教程 → `search_fabric_docs`。
