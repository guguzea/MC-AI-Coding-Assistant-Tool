...
本文件描述 Fabric 1.19.4 平台上进行实体（Entity）开发时所需掌握的核心 API 和常用命令。
...
创建实体类型使用 `EntityType.Builder` 构建器。基本命令：`EntityType.Builder.create(EntityType.EntityFactory<T> factory, SpawnGroup spawnGroup).dimensions(EntityDimensions).build()`。第一个参数是实体类的构造函数引用（`EntityType.EntityFactory<T>`），第二个参数 `SpawnGroup` 指定实体的生成分类：`MONSTER`（怪物）、`CREATURE`（动物）、`AMBIENT`（环境生物如蝙蝠）、`WATER_CREATURE`（水中生物）、`WATER_AMBIENT`（水下生物）、`MISC`（杂项，不会在世界生成）。`dimensions()` 设置实体碰撞箱大小，`EntityDimensions.fixed(width, height)` 用于固定大小的实体。
