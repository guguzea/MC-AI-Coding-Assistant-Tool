...
# mc-multiblock（Fabric 1.19.4）

多方块结构**没有平台 API** —— Fabric API、vanilla 都没有「多方块注册表 / 成型状态 / 自动结构校验」这种东西。这是一件**模式技能**：下面只写「用本档已核实的 vanilla 名把这个模式落下来」，不替你发明 API。

## 本档出处口径

类名有两条合法证据：本档 `data/fabric_1.19.4/**` 语料逐字命中，或 `data/fabric_1.19.4/mappings/yarn-mappings.sqlite` 命中。映射库只证**存在**，不证签名与用法。

自查结果（`assert-skill-yarn-attest --pack=fabric_1.19.4`）：
...
本档唯一带页面逐字出处的签名（`data/fabric_1.19.4/fabric-wiki/1.19.4/processed/tutorial_blocks.md:373-375`）就是「整体外形」这一段：
...
→ 控制器宿主 → BlockEntity 经 BlockEntityType 注册，注册入口走 Registries
   ...
   // TODO(未核实)：须 search_fabric_docs(version=1.19.4) 或 get_minecraft_source 逐个核实
...
- 核实路径：`search_fabric_docs(version=1.19.4)` → `get_fabric_doc_full`；vanilla 签名 `get_minecraft_source`（需 JDK 17+）或工程 `genSources`
