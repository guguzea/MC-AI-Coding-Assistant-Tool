...
> 本档正文的类名/签名只来自 `data/fabric_1.21.8` 本档语料：页面 `fabric-docs/1.21.8/processed/develop_items_potions.md`，代码 `reference/1.21.8/src/main/java/com/example/docs/potion/FabricDocsReferencePotions.java`。页内没有的签名一律留 `TODO(未核实)`，禁止用邻版（1.21.4 / 1.21.10 / 1.21.11）补全。

## 页面 prose 与盘上代码不一致（必读）

页面写「用 `FabricPotionBrewingBuilder.BUILD` 事件、调用 `PotionBrewing.addMix` 方法」，但同页转引的参考代码盘上真实写法是 `FabricBrewingRecipeRegistryBuilder.BUILD` + `builder.addMix(...)`：

- `FabricPotionBrewingBuilder`、`PotionBrewing` 两个名字在本档参考代码 import 面里**都不存在**，照页面 prose 写 import 会编译失败。
- 盘上唯一真实的 Fabric API 入口：`net.fabricmc.fabric.api.registry.FabricBrewingRecipeRegistryBuilder`；`addMix` 是 BUILD 回调形参上的方法。
- 逐签名核对走 `query_loader_api(platform=fabric, minecraftVersion=1.21.8)`。

## 注册 Potion

药水跟物品、方块一样要注册，注册进 `BuiltInRegistries.POTION`；本档 `Registry.register(...)` 返回裸 `Potion`：
...
进注册表只是有这瓶药水；要能在酿造台出成品，得在 `onInitialize()` 挂 `FabricBrewingRecipeRegistryBuilder.BUILD`：
...
- 裸 `Potion` → Holder：本档真实写法 `BuiltInRegistries.POTION.wrapAsHolder(TATER_POTION)`。
- `BUILD.register(...)` 的 lambda 形参类型页面未写 ⇒ `// TODO(未核实)`，不要自拟 `BrewingRecipeRegistry.Builder` 一类类型名。
- 注册后用马铃薯即可酿出 Tater 药水（页内配图 `/assets/develop/tater-potion.png`）。

## 资源与本地化

本档参考资源命名空间是 `fabric-docs-reference`（`reference/1.21.8/src/main/resources/assets/fabric-docs-reference/...`），且该目录下**没有 lang 文件**，页面也未给任何药水语言键 ⇒ `// TODO(未核实)`。显示名请 `search_fabric_docs(version="1.21.8")` 或反编译原版资源核实后再写，禁止抄别的版本档的键。药水模型/纹理本档参考资源同样没有。
...
- 可饮用的自定义药水物品（`PotionItem` / `use`）：本档药水页全程只注册 `Potion`。`use` 的返回类型（`TypedActionResult` / `ActionResult` 家族）出现在同档 `develop_items_custom-item-interactions.md`、`develop_events.md`、`develop_debugging.md`，要写必须先读那些页。
...
- 附魔效果面：同档另有 `develop_items_custom-enchantment-effects.md`，与药水/酿造不同面，未在此引。
...
- 反模式：`09-anti-patterns.mdc`；本档核实表：`fabric/1.21.8/knowledge/common/verified-api-1.21.8.md`
- 全文核对：`get_fabric_doc_full(version="1.21.8", id="develop_items_potions")`
