...
> 一手来源（2026-09-19 核实）：Minecraft Wiki《Pack format》数据包格式表（https://minecraft.wiki/w/Pack_format ，1.21.7–1.21.8 = 81）与《Data pack》目录改名记载（https://minecraft.wiki/w/Data_pack ，1.21 起目录改单数）。本技能不含 Java API——数据包是 JSON 面；Java 侧数据生成走 `mc-datagen`。
...
→ 版本 → 1.21.7–1.21.8 数据包 pack_format = 81（资源包为 64）
...
```json
{ "pack": { "pack_format": 81, "description": "My Datapack" } }
```
...
- `pack_format` 填邻版数字（1.21.7–1.21.8 数据包是 **81**，不是 61/88）。
- 写 1.20.x 时代的复数目录（`recipes/`、`loot_tables/`、`tags/items/`）——1.21 起必须单数。
- 把 Java 类名问题带到本技能：数据包没有类名，类名核实走 `search_fabric_docs(version=1.21.8)`。
