...
> Wave D 技能骨架（fabric 1.21.8）。详细规则见对应 `.cursor/rules/` 与 MCP `search_fabric_docs` / 专题工具。
...
- 需要该主题的签名/流程时：`search_fabric_docs version=1.21.8 query=…`；0 命中就留 `// TODO(未核实)`，**禁止**用邻版或另一套映射的名顶上。
