...
- **允许**在静态字段上直接 `Registry.register`（类加载即注册）；`onInitialize()` 里必须引用该类以触发加载
- **禁止** `new Item` 却不 `Registry.register`；禁止先 `new` 再在 `static {}` 里 register
...
| 注册内容 | API | 代码位置 |
|---------|-----|---------|
| 方块 | `Registry.register(Registries.BLOCK, id, block)` | 静态字段；`onInitialize()` 引用该类 |
| 物品 | `Registry.register(Registries.ITEM, id, item)` | 静态字段；`onInitialize()` 引用该类 |
| 方块实体 | `Registry.register(Registries.BLOCK_ENTITY_TYPE, id, type)` | 静态字段；`onInitialize()` 引用该类 |
| 实体类型 | `Registry.register(Registries.ENTITY_TYPE, id, type)` | 静态字段；`onInitialize()` 引用该类 |
| 粒子类型 | `Registry.register(Registries.PARTICLE_TYPE, id, type)` | 静态字段；`onInitialize()` 引用该类 |
| 声音事件 | `Registry.register(Registries.SOUND_EVENT, id, soundEvent)` | 静态字段；`onInitialize()` 引用该类 |
| 附魔 | `Registry.register(Registries.ENCHANTMENT, id, enchantment)` | 静态字段；`onInitialize()` 引用该类 |
| 流体 | `Registry.register(Registries.FLUID, id, fluid)` | 静态字段；`onInitialize()` 引用该类 |
...
IF 注册 方块 / 物品 / 实体 / 方块实体 / 附魔 / 粒子 / 声音等标准内容
  → 静态字段上 Registry.register()；onInitialize() 引用该类以触发加载
...
IF 平台 = Forge
  → 跳转 ../forge/1.20.1/AGENTS.md（Forge 使用 DeferredRegister）
```
