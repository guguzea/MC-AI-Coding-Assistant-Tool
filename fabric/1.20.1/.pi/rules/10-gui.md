...
Yarn 移位是 `quickMove(PlayerEntity, int)`（[yarn 1.20.1 ScreenHandler.mapping](https://github.com/FabricMC/yarn/blob/1.20.1/mappings/net/minecraft/screen/ScreenHandler.mapping)）。不要抄 `transferSlot`。
容器屏：`ScreenRegistry.register` 或 wiki 的 `HandledScreens.register`。额外数据：`writeScreenOpeningData`。
