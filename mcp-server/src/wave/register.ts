...
export const waveToolSchemas: Array<{ name: string; description: string; inputSchema: z.ZodTypeAny }> = [
  { name: "query_registry", description: "查询 Vanilla 注册表资源 ID（minecraft:stone）。nameLayer=registry_id；类/方法映射请用 convert_mapping。", inputSchema: queryRegistrySchema },
  ...
  { name: "validate_datapack_json", description: "recipe / loot_table / advancement / tag 的精简 JSON 校验（1.20.1 / 1.21.1）。", inputSchema: validateDatapackJsonSchema },
  ...
  { name: "generate_model", description: "Generate block model JSON templates", inputSchema: generateModelSchema },
  { name: "generate_lang", description: "Generate en_us + zh_cn lang JSON", inputSchema: generateLangSchema },
  { name: "generate_network_packet", description: "Generate network packet skeleton", inputSchema: generateNetworkPacketSchema },
  { name: "generate_capability", description: "Generate Capability / DataAttachment skeleton", inputSchema: generateCapabilitySchema },
  { name: "generate_config", description: "Generate config spec skeleton", inputSchema: generateConfigSchema },
  { name: "generate_entity_renderer", description: "Generate entity renderer skeleton", inputSchema: generateEntityRendererSchema },
