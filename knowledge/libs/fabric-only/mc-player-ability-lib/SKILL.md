...
→ 简单标记/开关（1.20.5+）→ Data Components 或 FAPI Attachment（见 `mc-cca`）
→ 飞行/缩放/鞘翅等"能力"，要注册+兼容修正 → PAL
→ platform = forge / neoforge → 本 skill 不适用（无 Forge 构建）
→ 已选 PAL：
   ├─ 依赖：官方只 `modImplementation` PlayerAbilityLib（CCA 非硬依赖）
...
- 坐标/maven：Ladysnake 系仓库与坐标照官方 README 抄，`modImplementation` 声明 PlayerAbilityLib
- `fabric.mod.json`：`depends` 写 PAL（modId 以官方为准）；软依赖用门闩（见 `authored/soft-deps-modlist`）
- 版本核对：PAL 版本与 MC 对齐
...
- 漏声明 PAL 依赖：运行时 `NoClassDefFoundError`
