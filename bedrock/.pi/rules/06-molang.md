...
→ 不确定的 query → 打开 intro / syntax；不要把 syntax 当查询清单
```
...
`stable/molang-syntax` **不是**查询清单（该页指向 Learn「Query Functions」文，本库未入库该页）。intro 页示例（`query.last_hit_by_player`、`query.life_time`、`Math.Random`）可用，不要用「syntax 页没有」误杀它们。其余 query 打开 intro/syntax；未入库的 Query Functions 禁止臆造。
