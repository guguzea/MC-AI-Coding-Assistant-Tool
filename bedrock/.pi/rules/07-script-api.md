...
→ ≥ 1.19.80：世界实验 = level.dat 里 `experiments` compound，每个功能一个 **byte = 1**（关 = 删掉该 byte tag）；「Beta APIs」对应键名 **`gametest`**。出处：wiki.bedrock.dev/nbt/enabling-experiments（2026-09-02 取）= **社区权威，非 Microsoft Learn 官方**，键名只用于手工改 level.dat 分诊，禁止当官方 API 写进 pack 文档。pack JSON 仍**打不开**该开关；Minecraft Education / BDS 无 GUI，只能手改 level.dat
