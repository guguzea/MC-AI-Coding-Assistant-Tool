...
- `format_version`：资源/行为/世界模板用 **2**。**3** 是 Preview（semver 字符串版本、自定义 pack 设置）；未点名 Preview 不要写 3。皮肤包可用 1。`TODO(未核实)`：各内容类型的 `format_version` 具体取值与演进以 Learn pack-manifest 现页为准（本仓语料只到该页快照）。
