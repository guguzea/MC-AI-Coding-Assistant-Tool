...
- `format_version`：资源/行为/世界模板用 **2**。**3** 是 Preview（semver 字符串版本、自定义 pack 设置）；未点名 Preview 不要写 3。皮肤包可用 1。`TODO(未核实)`：各内容类型的 `format_version` 具体取值与演进以 Learn pack-manifest 现页为准（本仓语料只到该页快照）。**工具接受面（W5-2 裁定 2026-09-19 的 semver 部分维持；原写「整数形与 beta 串」的后半句按实现更正）**：`validate_addon_manifest` 的 `format_version` **只收正整数**——校验分支是 `typeof !== "number" || !Number.isInteger || < 1` ⇒ 报「format_version 必须是正整数」（`mcp-server/src/bedrock/index.ts` 的 manifest 校验段），**任何字符串形态都被拒，`"beta"` 也不例外**；`generate_addon_manifest` 恒输出 `format_version: 2`。它接收的 `beta` 是**布尔开关**，作用是把 script 模块的 `dependencies[].version` 从 `stableVer` 换成字面量 `"beta"` 并追加 Beta APIs 警示，**不改 `format_version`、也不改 `min_engine_version`**（后者只跟 `minEngineVersion` 参数走）。⇒ 旧文案里那个「beta 串」是 `dependencies.version` 的形态，被错记成了 `format_version` 的一种取值。semver 形（如 `"3.0.1"`）仍属**有意拒绝**——官方逐字语义未核实前不放行。
