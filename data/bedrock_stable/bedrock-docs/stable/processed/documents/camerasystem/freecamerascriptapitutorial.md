> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/camerasystem/freecamerascriptapitutorial?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:57.958Z
> 警告：此文档可能滞后于当前正式版
...
- Open https://mctools.dev in your browser.

- Scroll down to Start From a Template and find Code Starter (TypeScript) . Click Create New .

- In the dialog box, enter a title like "CameraAnimation" and your creator name.

- Open the Advanced Options and give the project a folder name and a description.

- Select Save to My Computer and choose an empty folder to save to. (We created one named mc_camera for the tutorial.)

- Click Create Project . After Creator Tools finishes, it'll show you the dashboard for your project, and the files will be saved in the folder you selected in step 5.

- In Windows Terminal or PowerShell, change to the directory you created in Step 5, and install the required Node tools and libraries: ```powershell npm i ```

Tip

You can edit your files directly in Minecraft Creator Tools, or edit them in Visual Studio Code (or another text editor).

You can also download the TypeScript Code Starter sample pack from the Scripting Samples repository , either by downloading a Zip file or cloning the repository with Git. If you take this approach, you should copy the ts-starter folder to a new work directory (like mc_camera in the example above).
...
function initialize() {
 ...
 if (initRetries initialize(), INIT_RETRY_TICKS);
 } else {
...
The flyover is defined as a Catmull-Rom spline , which constructs a smooth, curved path defined by a handful of control points. You set the control points, and Minecraft uses the Catmull-Rom spline formula to plot a curve that passes through all of them. In the code above, we use five control points that describe a square around the player's location when they press the button, rising into the air and then descending again as it finishes by moving directly over the player. The curve that the spline function creates from these control points will be a counter-clockwise spiral.

Where did the name "Catmull-Rom" come from, you wonder? Edwin Catmull was its co-inventor—and also the co-founder of Pixar.

After that, we set up a free camera , which we start over the player. Then we start the main event: the `playAnimation()` function, which takes the spline we've defined as its first parameter, then a few options in a key/value object:

- animation.progressKeyFrames defines an easing function and alpha value at the start and end of the animation
...
While you can define spline control points the way we did in this example, there's an alternative which can be easier, especially the more control points you have. The Camera Tool offers a GUI-based, interactive way to define the control points for splines. After you define the spline to make the moves you want, you can export the code from the Editor to use in your own add-on.

## Attach the camera to an entity

While the flyover doesn't demonstrate the ability to attach a camera to an entity, you can find an example of that in the 1.26.10 Update Video , which features a similar flyover script that gives you a "tour" of a zoo. After the tour, it lets you look through the eyes of a sheepomelon for three seconds.
...
There are enhancements and improvements you can make to this script on your own. You might want to change the path or timing of the flyover, or even use a fade transition at its start or end (or both).
