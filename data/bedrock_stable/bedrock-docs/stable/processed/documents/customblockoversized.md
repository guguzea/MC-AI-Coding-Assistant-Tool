> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/customblockoversized?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:50:18.070Z
> 警告：此文档可能滞后于当前正式版
...
First, we will cover how to create an over-sized block, then we will use one of the examples from another custom block tutorial (the Sushi Block one) to demonstrate culling.
...
- Custom Blocks Part Three: Render and Lighting Options
...
- Your block is limited to 30x30x30 pixels in size.
...
- At least 1 pixel of your block on each axis must be contained by the base 16x16x16 block.
...
- Check back on your previous blocks Did you forget a file?
...
- Open the chat dialogue box (press 'T' or 'Enter' on Windows 10 OS).

- Type the following command: /give @p demo:giantumbrella
...
Here is a link to the already-completed custom block sample packs .
...
To use culling, you will need to add a few things to both the behavior pack and the resource pack. A reference of block culling rules is here .
...
- On the main level of the resource pack, add a directory called: block_culling .

- Create a file in there and name it sushi_cull.json , then add these contents:
...
- Save the file, and test the culled block.
