> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/featurestutorial_1?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:52.411Z
> 警告：此文档可能滞后于当前正式版

# Features Tutorial - a Simple Feature

This tutorial explains how to create and test a features behavior pack using the hot_air_balloon sample feature from the Minecraft Custom Features Project sample pack .

## More Information

If this is your first time working with Features, take a look at the Introduction to Features reference page for a more in-depth overview of the parts before you continue.

We also have an Introduction to Structure Blocks article to get you started with everything you need if you are new to the world of creating, exporting, and importing structures.
...
- Expand/unzip the folder (it should be called minecraft-samples-main ).

- Take a look at all the delightful Minecraft sample projects, then find and open custom_features .

- Open the example_feature_set folder, then open the behavior_packs folder inside it.

- Inside behavior_packs , find the second folder named example_feature_set where you'll find the folders and files of the project we are using as a starting point for our tutorial. Tip The only file we will actually need to copy and paste from this sample is hot_air_balloon.mcstructure .

## hot_air_balloon - Behavior Pack Structure

- Create the main folder for your behavior pack and call it something like hot_air_balloon_BP .

- Create a manifest.json file in there, like the one below.
...
- Next, create the folders and subfolders, according to the pack structure. The finished behavior pack should have the following structure: hot_air_balloon_BP (main behavior pack folder) feature_rules (folder) hot_air_balloon_feature_rules.json

- features (folder) hot_air_balloon_feature.json

- structures (folder) example (folder) hot_air_balloon.mcstructure
...
Take a look at the `/locate` command page for the latest list of biomes. You can also try using the `locate` command if you can't find your hot air balloons.
...
`"hot_air_balloon.mcstructure"` is the only file we actually needed from the sample pack. If you don't have it, you can make your own structure and export it to this folder using structure blocks .
