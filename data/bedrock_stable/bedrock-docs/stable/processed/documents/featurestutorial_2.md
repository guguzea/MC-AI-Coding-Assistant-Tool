> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/featurestutorial_2?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:53.215Z
> 警告：此文档可能滞后于当前正式版
...
For a more in-depth explanation about features, take a look at the Introduction to Features reference page.

## Create a custom feature project

If you already did the simple feature tutorial , this next part should seem very familiar.

- Go to the minecraft-samples page and download the .zip file of the code.

- Unzip the folder, then expand the folder called mineraft-samples-main .

- Find and open the folder called custom_features .

- Inside custom_features, find the example_feature_set folder, we want both the behavior_packs folder and the resource_packs folder right now.

## Behavior pack structure

- Create a behavior pack folder and call it something like oak_apple_BP .

- Create a manifest.json file inside your new behavior pack like the one below: ``` { "format_version": 2, "header": { "description": "Feature: Oak trees can grow apples! (BP)", "name": "Oak Apple", "uuid":" ", "version": [1, 0, 0], "min_engine_version": [1, 20, 20] }, "modules": [ { "description": "Features Samples", "type": "data", "uuid": " ", "version": [1, 0, 0] } ], "dependencies": [ { "uuid": " ", "version": [ 1, 0, 0 ] } ] } ``` Important Remember that the UUID in the dependencies section should match the UUID for the resource pack if you want them to load each other automatically. For more information, review Creating a Behavior Pack from Scratch .

- The final behavior pack structure should be: oak_apple_BP (main behavior pack folder) blocks (folder) apple_block.json

- feature_rules find_valid_apples_feature_rules.json

- oak_tree_then_apples_feature_rules.json

- features (folder) apple_feature.json
...
This feature places a single block—the apple_block—that adheres to the specifications outlined in the feature rules that call this file. The may_replace line makes sure that apple_blocks can only replace minecraft:air , so it cannot "dig" into wood or leaves when it's placed.
...
If you want to learn more about how to make your feature's loot drop more varied, read Loot Table Conditions .
...
- oak_apple_RP (main resource pack folder) models (folder) blocks (folder) apple_block_geo.json

- texts (folder) en_US.lang

- textures (folder) blocks (folder) apple_block.png
...
This is the geometry file for the custom apple block itself. Check out our Custom Die Block tutorial to learn more about Minecraft custom block geometry.
...
This is a forest with apple blocks attached to the trees. After you activate the packs and find your apple trees, take a moment to break an apple block and munch on some apples—you earned it!
