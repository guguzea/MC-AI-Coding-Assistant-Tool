> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/advancedcustomblocks?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:29.227Z
> 警告：此文档可能滞后于当前正式版
...
If you haven't completed the How to Add a Simple Custom Block tutorial yet, it's recommended you do that before starting this one.
...
We will use Blockbench in these tutorials to create custom geometries for the more advanced blocks we'll create for sushi, fish in a bubble, and a giant umbrella. If this is your first time using Blockbench, we recommend taking a look at the Entity Wizard tutorial .
...
- In File Explorer, navigate to the folder custom_block_resource_pack , located in the development_resource_packs folder.

- Inside the custom_block_resource_pack folder, create a folder and name it models .

- Open the models folder.

- Inside the models folder, create a folder and name it blocks .

- Open the blocks folder.

- Inside the blocks folder, copy the downloaded geometry files from Blockbench.
...
- In the resource pack, navigate to blocks.json .

- Add the following sound information below the demo:die section.
...
- In File Explorer, navigate to the folder custom_block_behavior_pack , located in the development_behavior_packs folder.

- Open the blocks folder and create a text document named salmon_roll.json .

- Double-click salmon_roll.json to open it in a text editor.
...
- Open up the chat dialogue box.

- Type the following command: /give @s demo:california_roll
