> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/lootoverloads?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:50:05.110Z
> 警告：此文档可能滞后于当前正式版
...
`kill [tool|mainhand|offhand]` - Pulls items from the specified entity's loot table, as if it were killed it with the specified tool, or no tool if none is specified.
...
`mine [tool|mainhand|offhand]` - Obtains item resource drops from the target block's loot table, taking into account the tool, or no tool if none is specified. Enchanted tools held in the mainhand when `mainhand` is specified will have their enchantment evaluated for dropping loot.
...
`/loot replace block slot.container [ ] [ |mainhand|offhand]`
...
`/loot replace entity [ ] [ |mainhand|offhand]`
...
`spawn ` - Spawns an item into the world at the given position.
...
`loot [tool|mainhand|offhand]` - Pulls items from the specified loot table as if you used the specified tool to obtain them or no tool if none is specified.
...
`/loot replace block slot.container [ ] loot "loot_table" [ |mainhand|offhand]`
...
`/loot replace block slot.container [ ] kill [ |mainhand|offhand]`
...
`/loot replace block slot.container [ ] mine [ |mainhand|offhand]`
