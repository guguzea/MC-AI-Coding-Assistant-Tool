> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/custom_projectiles?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:50:15.053Z
> 警告：此文档可能滞后于当前正式版
...
You can find the samples for this tutorial in the Minecraft Samples repository .
...
- minecraft:projectile : defines an item as a projectile that can be shot from dispensers or spawned/used as ammunition. The entity defined in its projectile_entity property must have the minecraft:projectile entity component.

- minecraft:throwable : makes an item, like a snowball, throwable by the player. This defines behaviors like swinging, draw duration, and launch power. The item requires the minecraft:projectile item component to specify the target entity to be thrown.

- minecraft:shooter : makes an item, like a bow, shoot projectiles. The item must have the minecraft:use_modifiers item component to function properly, and the items defined in its ammunition property must also have the minecraft:projectile item component.

### Entity components

- minecraft:projectile : defines core projectile behavior including trajectory, physics, damage, sounds, particles, and on hit effects.

- minecraft:shooter : defines ranged attack behavior. This is required for the minecraft:behavior.ranged_attack goal to determine which projectiles to shoot. The entities defined in its projectiles property must have the minecraft:projectile entity component. If you use the minecraft:behavior.fire_at_target goal instead, the shooter component is unnecessary, as that goal defines the target projectile in its projectile_def property.
...
- The custom_snowball.json file goes into the behavior_pack/entities/ folder.
...
- Next, we'll need to add resource_pack/animations/custom.animation.json to ensure the snowball, which is just a 2D sprite, always faces the camera. (This is called "billboarding.")
...
- In resource_pack/render_controllers/custom_snowball.­render_controllers.json , we'll set the is_hurt_color to 0 to prevent the red flash whenever the snowball is destroyed.
...
- Next, the snowball needs a texture. Add this image to resource_pack/textures/items :

- Last but not least, its resource definition uses the render controller, animation, and texture we just added. Add resource_pack/entity/custom_snowball.entity.json :
...
- First, since the snowball is a 2D sprite, we can use its texture again as its own icon by defining it in resource_pack/textures/item_texture.json :
...
- Now, we can define the item itself in behavior_pack/items/custom_snowball.json :
...
- Put this in behavior_pack/entities/custom_arrow.json :
...
- Add this to resource_pack/animations/custom.animation.json :
...
- The render controller again sets is_hurt_color to 0. In resource_pack/render_controllers/custom_arrow.render_controllers.json :
...
- Now, here's where we stop following the snowball's path: the arrow is a true 3D model, not a sprite, so we'll need to define a geometry for it ( resource_pack/models/entity/custom_arrow.geo.json ):
...
- Now the arrow needs a texture saved as resource_pack/textures/entity/custom_arrows.png :

- And, finally, the resource definition brings everything together ( resource_pack/entity/custom_arrow.entity.json ):
...
- Unlike the snowball, the arrow needs a separate texture for its icon. Save this image as resource_pack/textures/items/custom_arrow.png :

- We'll refer to it in resource_pack/textures/item_texture.json :
...
- Add the item definition JSON ( custom_projectiles_behavior_pack/items/custom_arrow.json ):
...
We're not finished yet, though—the custom arrow needs its custom bow! We'll go through the item definition more quickly this time.

- Save this image to resource_pack/textures/items/custom_bow_standby.png (the name will be explained in a minute):

- Add the texture to resource_pack/textures/item_texture.json :
...
- As with the arrow, we want the bow to render as a 3D object, so we need to add a bow model. Actually, we need to add four bow models: one at standby (rest), and three for different states of charging (being pulled back) before releasing the arrow. We'll define the geometry in resource_pack/models/entity/custom_bow.geo.json :
...
- Next, we'll define an attachable for the bow. Ensure you've saved the following images to your textures/items folder for the different charging state textures.
...
- Add resource_pack/attachables/custom_bow.json :
...
- Bring it all together in resource_pack/render_controllers/custom_bow.render_controllers.json :
...
This only shows what's changed, not the full animation controller!
...
```json
{
 ...
 {
 // BEFORE:
 // "third_person_bow_equipped" : "query.get_equipped_item_name == 'bow' && (variable.item_use_normalized > 0 && variable.item_use_normalized 0 && variable.item_use_normalized
