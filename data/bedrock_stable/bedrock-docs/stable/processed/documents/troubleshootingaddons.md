> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/troubleshootingaddons?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:50:37.176Z
> 警告：此文档可能滞后于当前正式版
...
The Content Log should be one of your go-to debugging tools; enable it in Minecraft's creator settings. Check out the Content Error Log article for full details on how to use it!

### Set up your IDE for debugging

In Visual Studio Code, you should install Minecraft-specific extensions like the Minecraft Debugger and Blockception's development pack.
...
- Missing movement components: ```json "components": { "minecraft:movement": { "value": 0.25 }, "minecraft:movement.basic": {}, "minecraft:navigation.walk": { "can_path_over_water": true } } ```

- No AI goals: Add behavior components: ```json "minecraft:behavior.random_stroll": { "priority": 6, "speed_multiplier": 1.0 } ```

- Movement speed is zero: Check the movement value is greater than 0.
...
- Verify texture path in client entity file matches actual file location

- Check behavior and resource pack identifiers match exactly

- Ensure texture is a valid PNG (not just a renamed file)

- Verify render controller references correct texture
...
- Animation controller exists and is referenced in client entity

- Animation files have correct identifiers

- State transitions use correct query conditions

- Animation is referenced with correct name
...
- terrain_texture.json has an entry for your texture

- Texture name in block JSON matches the terrain_texture entry

- The texture file exists at specified path

- Texture dimensions are power-of-2 (16x16, 32x32, etc.)
...
- Geometry file is in models/blocks/ folder

- Geometry identifier matches block JSON reference

- Block JSON uses correct syntax: ```json "minecraft:geometry": { "identifier": "geometry.my_custom_block" } ```
...
- State values not defined in description

- Condition query syntax errors

- Event not properly setting state
...
- item_texture.json exists in textures folder

- Texture name in item JSON matches item_texture entry

- Texture file exists at correct path
...
- Script module in manifest.json has "type": "script"

- Entry path points to correct file: "entry": "scripts/main.js"

- Dependencies use module_name , not uuid : ```json "dependencies": [ { "module_name": "@minecraft/server", "version": "1.17.0" } ] ```

- Script file has valid JavaScript syntax
...
- Check you're using the correct event (beforeEvents vs afterEvents)

- Verify entity/block types match exactly (with namespace)

- Ensure you're not canceling events unintentionally

See Debugging Scripts for comprehensive debugging techniques.
...
- Recipe file in recipes/ folder

- Recipe type matches crafting method

- Item identifiers include namespace

- Pattern and keys are correct
...
- Loot table file exists in loot_tables/ folder

- Entity/block references correct loot table path

- Loot table syntax is valid
...
See Improving Performance and Resource Usage .
