> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/controlschemes?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:56.215Z
> 警告：此文档可能滞后于当前正式版
...
The control scheme can be manually set by using the `/controlscheme` command. It can also be set with camera presets .
...
This scheme is experimental, and requires the Experimental Creator Camera setting to be enabled.
 ...
 Touch: Push the virtual joystick or D-pad to move the player without turning (strafe). Drag on the screen to rotate the player to look toward the point being touched. Note: This control scheme will force the visible joystick option on.
...
When you use camera presets , you can use the `control_scheme` key in the camera.json file to associate a control scheme with a preset.
...
To switch schemes from the command line, use the `/controlscheme` command .
