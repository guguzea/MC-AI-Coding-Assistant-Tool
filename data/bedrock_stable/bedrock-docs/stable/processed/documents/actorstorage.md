> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/actorstorage?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:39.168Z
> 警告：此文档可能滞后于当前正式版
...
On the left of the diagram, we can see the chunk key space. These keys take the legacy chunk key form of ` `.
...
In the middle, we have the digest key space. Each digest key takes the form `digp `.
...
- is the same key string used by the chunk the data is associated with.

 Actor Key Space

On the right, we have the actor key space. Each actor key takes the form `actorprefix `.
...
- is a unique ID that is generated for each actor when it is added to the level. This ID is consistent between play sessions and is only unique to this world. Other actors in other worlds may have the same ID, but no actor in the same world will have the same ID.
