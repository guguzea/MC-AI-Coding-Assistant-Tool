> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/scripting/developer-tools?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:43.752Z
> 警告：此文档可能滞后于当前正式版
...
For Minecraft Preview or legacy UWP builds, see GDK Migration on Windows for folder locations.
...
For more details, consult the `debug-utilities` reference page .
...
For more details, consult the `diagnostics` reference page .
...
- Verify VS Code is in "listening" mode (you should see "Waiting for debugger to connect..." in the Debug Console)

- Ensure you ran the loopback exemption command as the administrator.

- Check that port 19144 isn't blocked by firewall or antivirus software.

- Verify launch.json has the correct port number.

- Try restarting both Minecraft and VS Code.

### Breakpoints not triggering

If you've set breakpoints but they don't seem to be doing anything, check the following:

- If you're using TypeScript, verify sourceMapRoot and generatedSourceRoot paths in launch.json are correct.

- Ensure you're setting breakpoints in the source file, not the compiled output.

- Add a console.warn() debugging statement to verify the code path is actually being run.

- If you have multiple behavior packs with scripts, specify a targetModuleUuid in launch.json .

- Reload needed : Run /reload after connecting the debugger if scripts were already running

### Profiler not generating output

If `/script profiler stop` doesn't create a .cpuprofile file, check the following:

- Ensure you have write permissions to the logs folder.

- Wait a few seconds after stopping before looking for the file.

- Check both the logs folder and the root of your Minecraft data folder.

- Verify there's a script actually running (profiles are empty without script activity).

### Console messages not generating output

If your `console.warn()` calls aren't showing up anywhere, try the following:

- Enable Content Log File in Settings > Creator.

- Enable Content Log GUI to see messages in-game.

- Check the log file in the logs folder (not just content_log.txt ).

- Ensure your script is actually loading (check for errors at the top of the log).
...
- Run /reload to reload scripts without leaving the world.

- Exit and re-enter the world for manifest changes.

- Clear development_behavior_packs and redeploy for stubborn issues.

- Verify your build process completed successfully (check for TypeScript errors).
...
For more debugging strategies and common error patterns, see Debugging Scripts in Minecraft .
