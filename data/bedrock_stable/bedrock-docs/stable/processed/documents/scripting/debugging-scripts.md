> 来源：https://learn.microsoft.com/en-us/minecraft/creator/documents/scripting/debugging-scripts?view=minecraft-bedrock-stable
> 抓取时间：2026-09-21T11:49:47.569Z
> 警告：此文档可能滞后于当前正式版
...
There are several approaches to debugging scripts in Minecraft, each with its own strengths:
 ...
 Console logging
 Quick checks, simple issues
 Easy

 Content log
 JSON errors, pack loading issues
 Easy

 VS Code debugger
 Complex logic, variable inspection
 Medium

 Custom tooling
 Variable inspection over time
 Medium

 Visual debug utilities
 Spatial issues, entity tracking
 Advanced
...
The simplest debugging technique is printing messages to the console. To do that in Minecraft, you'll need to enable the content log .
...
- Navigate to your Minecraft logs folder: GDK (1.21.120+) : %APPDATA%\Minecraft Bedrock\logs\

- Preview : %APPDATA%\Minecraft Bedrock Preview\logs\

- Legacy UWP : See GDK Migration on Windows for older paths

- Open the most recent .log file.

- Search for your debug messages.
...
For complex issues, the Visual Studio Code debugger lets you pause execution, inspect variables, and step through code line by line. You can read detailed documentation for the debugger in its Developer Tools section .

### Quick setup checklist

- Install the debugger extension: Minecraft Bedrock Edition Debugger

- Enable loopback (run once as Administrator): ```powershell # For Minecraft retail CheckNetIsolation.exe LoopbackExempt -a -p=S-1-15-2-1958404141-86561845-1752920682-3514627264-368642714-62675701-733520436 # For Minecraft Preview CheckNetIsolation.exe LoopbackExempt -a -p=S-1-15-2-424268864-5579737-879501358-346833251-474568803-887069379-4040235476 ```

- Create launch.json in your .vscode folder: ```json { "version": "0.3.0", "configurations": [ { "type": "minecraft-js", "request": "attach", "name": "Debug with Minecraft", "mode": "connect", "localRoot": "${workspaceFolder}/", "port": 19144 } ] } ```

- Start debugging in VS Code (F5), then connect from Minecraft: ```text /script debugger connect ```
...
- Start simple: Begin with one breakpoint at the suspected problem area.

- Step through slowly: Use F10 (Step Over) to execute one line at a time.

- Check assumptions: Use the Watch panel to verify your assumptions about variable values.

- Look at the call stack: The Call Stack panel shows how you got to the current line.
...
For a guide to this approach, read our article on Building Debug Tools .

## Visual debug utilities

The `@minecraft/debug-utilities` module lets you render visual debugging information directly into the game world. With debug drawing utilities, you can build specialized tools that display targeted information.
...
- Start profiling in-game: ```text /script profiler start ```

- Play through your experience normally for a few minutes.

- Stop profiling: ```text /script profiler stop ```

- Open the generated .cpuprofile file in VS Code.
...
That also shows another useful trick: the `getEntities` call is relatively time-consuming; you want to fetch its value outside your loop, and use the cached value. If you know a value won't change (such as the size of the Overworld), it can be moved entirely out of the `runInterval` call:
