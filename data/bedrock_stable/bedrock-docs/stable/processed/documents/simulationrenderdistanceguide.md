...
Ticking areas have their own separate article that goes into substantially greater depth: Introduction to Ticking Areas .

There are certain limits to ticking areas: for instance, while mobs may despawn in ticking areas, they won't spawn in them unless a player is present within simulation distance. If something active inside a ticking area moves outside it—a mob, a mine cart, an automatically-fired projectile—it will stop moving until it's within simulation distance.
...
Basically, actions and updates are confined to simulation distance and to ticking areas.

### Tweaking render and simulation distance

In practice, render distance and simulation distance are set by players client-side, and by server operators . For any given client, it will be the lesser of those two values: if the server has a `view-distance` of 32 and the client's render distance is set to 24, then the client will only render out to 24 chunks; if the server's `view-distance` is 16, though, the client will only render out to 16 chunks.

Inside your add-ons, you can manipulate fog distance to increase apparent render distance, but the engine won't render chunks in the world past the render distance no matter what values you've set fog to.

As a creator, the most important thing to think about is render and simulation distances on low-end devices, not just your own development machine. This is particularly important when you're thinking about cut scenes or other animations that might be displayed on less powerful hardware.
...
You only have 10 ticking areas per world and the areas themselves are limited in scale—and you don't want to have bigger or more numerous ticking areas than you need for performance reasons. So, plan your ticking areas to have minimal performance hits:
...
- Minimize count : Use as few ticking areas as possible

- Optimize size : Make them only as large as necessary

- Strategic placement : Position them where they provide maximum benefit

- Regular cleanup : Remove unused ticking areas
...
- Custom blocks : Use appropriate render methods for optimal performance

- Particle effects : Limit particle density in high-render-distance scenarios

- Entity limits : Be aware that render distance affects visible entity counts

### Simulation Distance Effects

- Mob behavior : AI only functions within simulation distance

- Block updates : Crops, water flow, and redstone stop beyond this limit

- Spawning mechanics : New entities only spawn within simulation distance

## Related documentation

- Introduction to Ticking Areas - Basic ticking area commands

- Custom Block Render Distance - How custom block rendering interacts with render distance

- Improving Performance and Resource Usage - General optimization

- Entity Despawn Component - Entity persistence options
