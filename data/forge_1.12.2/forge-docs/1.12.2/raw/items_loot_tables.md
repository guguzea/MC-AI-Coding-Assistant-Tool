...
source: "https://docs.minecraftforge.net/en/1.12.x/items/loot_tables/"
sourceType: mkdocs
---
# Loot Tables
...
> **Note**: Note Users are allowed by vanilla to place their own loot tables in the world save directory to override the game’s (and mods’) own tables. These are considered config files and thus cannot be modified by the methods described below, by design.
...
> **Note**: Note You must perform all of your desired changes to the table during that table’s LootTableLoadEvent, any changes afterward are disallowed by safety checks or will cause undefined behavior if the safety checks are bypassed.
