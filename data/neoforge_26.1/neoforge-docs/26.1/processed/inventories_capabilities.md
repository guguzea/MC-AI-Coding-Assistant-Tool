...
<ol>
<li class="">Do I only care about **what** a block, entity or item stack can do, but not about **how** it does it?</li>
<li class="">Is the **what**, the behavior, only available for some blocks, entities, or item stacks, but not all of them?</li>
<li class="">Is the **how**, the implementation of that behavior, dependent on the specific block, entity or item stack?</li>
</ol>
...
`EnergyHandler` exposes an interface for handling energy containers. It is based on the RedstoneFlux API by TeamCoFH. The capabilities of type [EnergyHandler](/docs/inventories/transactions#energy-handler) are:
...
<li class="">`EntityCapability`: capabilities for entities: behavior depends on the specific `EntityType`.<!-- -->

- The capability commonly specifies a `Direction` context for different resources depending on the side.

</li>
<li class="">`ItemCapability`: capabilities for item stacks: behavior depends on the specific `Item`.<!-- -->

- The capability commonly specifies an [ItemAccess](/docs/inventories/transactions#item-access) context for the holding item resource.
...
<li class="">The behavior type that is being queried. This is the `T` type parameter.</li>
...
<ol>
<li class="">Fetch block entity and block state if they were not supplied.</li>
<li class="">Fetch registered capability providers. (More on this below).</li>
<li class="">Iterate the providers and ask them if they can provide the capability.</li>
<li class="">One of the providers will return a capability instance, potentially allocating a new object.</li>
</ol>
...
<li class="">An invalidation listener, that is called when the capability changes.<!-- -->
...
For more information, refer to the javadoc of [IBlockCapabilityProvider](https://github.com/neoforged/NeoForge/blob/26.1.x/src/main/java/net/neoforged/neoforge/capabilities/IBlockCapabilityProvider.java).
...
If the capability previously returned by a block or block entity provider is no longer valid, *you must invalidate the caches** by calling `level.invalidateCapabilities(pos)`. Refer to the [invalidation section](#block-capability-invalidation) above for more information.
...
See [CapabilityHooks](https://github.com/neoforged/NeoForge/blob/26.1.x/src/main/java/net/neoforged/neoforge/capabilities/CapabilityHooks.java) for a list of the providers registered by NeoForge itself.
