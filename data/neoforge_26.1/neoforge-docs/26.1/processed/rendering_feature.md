...
Feature submission is typically handled by the underlying subsystem responsible for those objects: [EntityRenderer for entities](/docs/entities/renderer#entity-renderers), [BlockEntityRenderer for block entities](/docs/blockentities/ber#blockentityrenderer), [ParticleGroupRenderState for particles](#TODO), etc. Each provides their own `submit` method, usually taking in some general render state of the object. The necessary elements are then submitted through the `SubmitNodeCollector` and stored in a `SubmitNodeCollection` tree map for rendering.
...
| submitMovingBlock | A list of BlockStateModelParts with dynamic lighting. |
