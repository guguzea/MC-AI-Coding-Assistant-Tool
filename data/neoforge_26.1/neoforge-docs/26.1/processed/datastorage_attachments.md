...
*To store additional level data, you can use [SavedData](/docs/datastorage/saveddata).*

> **Note**
> note

Data attachments for item stacks have been superceeded by vanilla's [data components](/docs/items/datacomponents).
...
There are a few ways to provide an attachment serializer: directly implementing `IAttachmentSerializer`, implementing [ValueIOSerializable](/docs/datastorage/valueio#valueioserializable) and using the static `AttachmentType#serializable` method to create the builder, or providing a map codec to the builder.
...
The two other overloads which delegate to `AttachmentSyncHandler` take in a [StreamCodec](/docs/networking/streamcodecs) for `read` and `write`, and an optional predicate for `sendToPlayer`.
...
By default, [entity](/docs/entities/) data attachments are not copied on player death. To automatically copy an attachment on player death, set `copyOnDeath` in the attachment builder.
