...
Particles are visual effects that polish the game and add immersion. Being mostly visual in nature, critical parts exist only on the physical (and logical) client [side](/docs/concepts/sides).

This article covers the rendering-specific aspects of the particle. For more information on particles types, which are typically used to spawn particles; and particle descriptions, which can specify a particle's sprites, see the companion [particle types](/docs/resources/client/particles) article.
...
Then, during the [feature submission process](/docs/rendering/feature), the `SingleQuadParticle.Layer` determines what atlas to use along with the pipeline used to draw the quad to the screen. Vanilla provides six layers by default:
...
Particles whose `SingleQuadParticle.Layer` uses `TextureAtlas#LOCATION_PARTICLES` must have an associated [particle description](/docs/resources/client/particles). Otherwise, the textures required by the particle will not be added to the atlas.
...
Your particle provider must then be associated with the particle type in the [client-side](/docs/concepts/sides) [mod bus](/docs/concepts/events#event-buses) [event](/docs/concepts/events) `RegisterParticleProvidersEvent`:
...
If `registerSpriteSet` is used, then the particle type must also have an associated [particle description](/docs/resources/client/particles). Otherwise, an exception will be thrown stating it 'Failed to load description'.
