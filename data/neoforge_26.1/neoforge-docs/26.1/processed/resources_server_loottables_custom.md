...
Due to the complexity of the loot table system, there are several [registries](/docs/concepts/registries#methods-for-registering) at work, all of which can be used by a modder to add more behavior.

All loot table related registries follow a similar pattern. To add a new registry entry, you generally extend some class or implement some interface that holds your functionality. Then, you define a [codec](/docs/datastorage/codecs) for serialization, and register that codec to the corresponding registry, using `DeferredRegister` like normal. This goes along with the "one base object, many instances" approach most registries (for example also blocks/blockstates and items/item stacks) use.

## Custom Loot Entries

To create a custom loot entry, extend `LootPoolEntryContainer` or one of its two direct subclasses, `LootPoolSingletonContainer` or `CompositeEntryBase`. For the sake of example, we want to create a loot entry that returns the drops of a [entity](/docs/entities/) - this is purely for example purposes, in practice it would be more ideal to directly reference the other loot table. Let's start by creating our loot entry class:
...
We then use the map codec in [registration](/docs/concepts/registries#methods-for-registering):
...
Like with custom loot entries, we then use this codec in [registration](/docs/concepts/registries#methods-for-registering):
...
And again, we then use the codec in [registration](/docs/concepts/registries#methods-for-registering):
...
We can [register](/docs/concepts/registries#methods-for-registering) the map codec to the registry:
...
We can then [register](/docs/concepts/registries#methods-for-registering) the map codec to the registry:
