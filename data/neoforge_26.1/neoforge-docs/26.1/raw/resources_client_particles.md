...
url: "https://docs.neoforged.net/docs/resources/client/particles/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:13.068Z"
---
# Particles

Particles are visual effects commonly spawned using their associated particle type. They can be spawned both client and server [side](/docs/concepts/sides), but being mostly visual in nature, critical parts exist only on the physical (and logical) client side.

This article covers the construction and use of particle types and particle descriptions. For more rendering-specific information, see the companion [client particles](/docs/rendering/particles) article.
...
To allow a [particle](/docs/rendering/particles) to make use of its description, the `ParticleType` must be associated with a [ParticleProvider](/docs/rendering/particles#particleprovider) that takes in the `SpriteSet` using the [client-side](/docs/concepts/sides) [mod bus](/docs/concepts/events#event-buses) [event](/docs/concepts/events) `RegisterParticleProvidersEvent`:
...
Particle definition files can also be [datagenned](/docs/resources/#data-generation) by extending `ParticleDescriptionProvider` and overriding the `#addDescriptions()` method:
