...
Vanilla Minecraft provides numerous different types of enchantment effect components for use in [enchantment](/docs/resources/server/enchantments/) definitions. This article will explain each, including their usage and in-code definition.
...
The object within the `value` block is a [LevelBasedValue](/docs/resources/server/loottables/#number-provider), which can be used to have a value effect component that changes the intensity of its effect by level.
...
- `minecraft:apply_mob_effect`: Applies a [mob effect](/docs/items/mobeffects) to the affected mob.
- `minecraft:attribute`: Applies an [attribute modifier](/docs/entities/attributes#attribute-modifiers) to the wielder of the enchantment.
