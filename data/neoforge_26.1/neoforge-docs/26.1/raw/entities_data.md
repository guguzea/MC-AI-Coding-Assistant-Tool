...
url: "https://docs.neoforged.net/docs/entities/data/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:57:44.929Z"
---
# Data and Networking
...
- `EntityDataSerializer`s are basically wrappers around a [StreamCodec](/docs/networking/streamcodecs).

Minecraft uses a hard-coded map of serializers. NeoForge transforms this map into a registry, meaning that if you want to add new `EntityDataSerializer`s, they must be added by [registration](/docs/concepts/registries#methods-for-registering).
...
These two methods are used to read and write data to disk. They work by loading/saving your values from/to a [value I/O](/docs/datastorage/valueio), like so:
...
Please refer to the [Networking articles](/docs/networking/) for more information on custom network packets.

## Data Attachments

Entities have been patched to extend `AttachmentHolder` and as such support data storage via [data attachments](/docs/datastorage/attachments). Its main use is to define custom data on entities that are not your own, i.e., entities added by Minecraft or other mods. Please see the linked article for more information.

## Custom Network Messages

For syncing, you can also always opt to use a custom packet to send additional information when needed. Please refer to the [Networking articles](/docs/networking/) for more information.
