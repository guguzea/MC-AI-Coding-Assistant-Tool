...
url: "https://docs.neoforged.net/docs/resources/client/models/modelsystem/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:58:08.835Z"
---
# Understanding the Model System

Models within Minecraft are simply a list of quads with attached textures. Each part of the modeling process has their own separate implementation, with the underlying model JSON deserialized into an `UnbakedModel`. In the end, each part of the pipelines takes in some `List` and properties necessary for their own pipelines. Some [block entity renderers](/docs/blockentities/ber) also make use of these models. There is no limit to how complex a model may be.

Models are stored in the `ModelManager`, which can be accessed through `Minecraft.getInstance().getModelManager()`. For the item pipeline, you can get the associated [ItemModel](/docs/resources/client/models/items#manually-rendering-an-item) via `ModelManager#getItemModel` by passing in a [Identifier](/docs/misc/identifier). For the block state pipeline, you can get the associated `BlockStateModel` via `ModelManager.getBlockStateModelSet().get()` by passing in a `BlockState`. Mods will basically always reuse a model that was previously automatically loaded and baked.
...
- A `BlockState`: The [blockstate](/docs/blocks/states) being rendered. May be null, indicating that an item is being rendered.
- A `RandomSource`: A client-bound random source you can use for randomization.
- A `List`: The list that should receive the parts to render.
...
The [client item](/docs/resources/client/models/items) JSON (in `assets//items`) is compiled and baked into an `ItemModel` for a given `Item` to be used by the `ItemStack`. The process of creating the `ItemModel` goes like so:
...
Information about item rendering can be found in the [Manually Rendering an Item](/docs/resources/client/models/items#manually-rendering-an-item) section.
...
NeoForge allows the `ItemDisplayContext` to be [extended](/docs/advanced/extensibleenums#creating-an-enum-entry) for use in custom render calls. Modded `ItemDisplayContext`s may specify a fallback transform to use if none is specified in the model. Otherwise, behavior will be the same as vanilla.
...
After writing your model wrapper class, you must apply the wrappers to the models it should affect. Do so in a [client-side](/docs/concepts/sides) [event handler](/docs/concepts/events) for `ModelEvent.ModifyBakingResult` on the [mod event bus](/docs/concepts/events#event-buses):
...
It is generally encouraged to use a [custom model loader](/docs/resources/client/models/modelloaders) over wrapping baked models in `ModelEvent.ModifyBakingResult` when possible. Custom model loaders can also use delegate models if needed.
