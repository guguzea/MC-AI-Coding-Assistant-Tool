...
url: "https://docs.neoforged.net/docs/items/consumables/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:49.472Z"
---
# Consumables

Consumables are [items](/docs/items/) which can be used over a period of time, 'consuming' them in the process. Anything that can be eaten or drunk in Minecraft is a consumable of some kind.

## The Consumable Data Component

Any item that can be consumed has the [DataComponents#CONSUMABLE component](/docs/items/datacomponents). The backing record `Consumable` defines how the item is consumed and what effects to apply after consumption.
...
- `sound` - Sets the [SoundEvent](/docs/resources/client/sounds#creating-soundevents) to play while consuming the item. This must be a `Holder` instance. Defaults to `SoundEvents#GENERIC_EAT`.
...
<li class="">`soundAfterConsume` - Sets the [SoundEvent](/docs/resources/client/sounds#creating-soundevents) to player once the item has finished being consumed. This delegates to the [PlaySoundConsumeEffect](#consumeeffect).</li>
<li class="">`hasConsumeParticles` - When `true`, spawns item [particles](/docs/resources/client/particles) every four ticks and once the item is fully consumed. Defauts to `true`.</li>
...
Vanilla provides some consumables within their `Consumables` class, such as `#defaultFood` for [food](#food) items and `#defaultDrink` for [potions](/docs/items/mobeffects#potions) and milk buckets.
...
A `ConsumeEffect` can be created by implementing the interface and [registering](/docs/concepts/registries#methods-for-registering) the `ConsumeEffect.Type` with the associated `MapCodec` and `StreamCodec` to `BuiltInRegistries#CONSUME_EFFECT_TYPE`:
...
First, let's create a new `ItemUseAnimation`. This is done using the [extensible enum](/docs/advanced/extensibleenums) system:
...
From there, we can then start applying the transforms. To do this, we must create a new `IClientItemExtensions`, implement our desired methods, and register it via `RegisterClientExtensionsEvent` on the [mod event bus](/docs/concepts/events#event-buses):
...
Sometimes, an entity may want to play a different sound while consuming an item. In those instances, the [LivingEntity](/docs/entities/livingentity) instance can implement `Consumable.OverrideConsumeSound` and have `getConsumeSound` return the `SoundEvent` they want their entity to play.
...
Adding your own `ConsumableListener` is simply [registering a new data component](/docs/items/datacomponents#creating-custom-data-components) and implementing `ConsumableListener`.
