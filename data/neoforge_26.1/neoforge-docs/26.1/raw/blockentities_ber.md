...
url: "https://docs.neoforged.net/docs/blockentities/ber/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:32.546Z"
---
# BlockEntityRenderer

A `BlockEntityRenderer`, often abbreviated as BER, is used to 'render' [blocks](/docs/blocks/) in a way that cannot be represented with a [static baked model](/docs/resources/client/models/) (JSON, OBJ, others). For example, this could be used to dynamically render container contents of a chest-like block. A block entity renderer requires the block to have a [BlockEntity](/docs/blockentities/), even if the block does not store any data otherwise.

BERs directly implements the `BlockEntityRenderer`, which submits its [features](/docs/rendering/feature) for rendering:
...
Now that we have our BER, we also need to register and connect it to its owning block entity. This is done in [EntityRenderersEvent.RegisterRenderers](/docs/concepts/events#registering-an-event-handler) like so:
...
As not all block entities with renderers can be represented by static item models, a special renderer can be created to more dynamically control the process. This is done using [SpecialModelRenderers](/docs/resources/client/models/items#special-models). In these cases, both a special model renderer must be created to submit the desired [features](/docs/rendering/feature), and a corresponding registered special block model renderer for scenarios when the block itself is being submitted for rendering rather than an item variant (e.g., enderman carrying a block).

Please refer to the [client item documentation](/docs/resources/client/models/items#special-models) for more information.
