...
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:37.591Z"
---
# Tags
...
To reference tags in code, you must create a `TagKey<T>`, where `T` is the type of tag (`Block`, `Item`, `EntityType<?>`, etc.), using a [registry key](/docs/misc/identifier#resourcekeys) and an [identifier](/docs/misc/identifier):
...
A custom tag provider, whether for an existing or custom [registry](/docs/concepts/registries), can be created by simply extending `TagsProvider<T>`, where `T` is the registry object you are generating a tag for.
