...
url: "https://docs.neoforged.net/docs/resources/server/datamaps/builtin/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:22.954Z"
---
# Built-In Data Maps

NeoForge provides various built-in [data maps](/docs/resources/server/datamaps/) for common use cases, replacing hardcoded vanilla fields. Vanilla values are shipped by data map files in NeoForge, so there is no functional difference to the player.
...
NeoForge additionally adds the `IItemExtension#getBurnTime` method to be overridden in custom items, overruling this data map. `#getBurnTime` should only be used in scenarios where the datamap does not suffice, for example [data component](/docs/items/datacomponents)-dependent burn times.
