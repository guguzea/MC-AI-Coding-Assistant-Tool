...
url: "https://docs.neoforged.net/docs/resources/server/advancements/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:28.546Z"
---
# Advancements
...
`icon`: A [JSON representation of an item stack](/docs/items/#json-representation).
- `title`: A [text component](/docs/resources/client/i18n#components) to use as the advancement's title.
- `description`: A [text component](/docs/resources/client/i18n#components) to use as the advancement's description.
...
- `recipes`: A list of [recipe](/docs/resources/server/recipes/) IDs to unlock.
- `loot`: A list of [loot tables](/docs/resources/server/loottables/) to roll and give to the player.
...
<li class="">`neoforge:conditions`: NeoForge-added. A list of [conditions](/docs/resources/server/conditions) that must be passed for the advancement to be loaded. Optional.</li>
</ul>
...
The `SimpleCriterionTrigger<T>` implementation has two purposes: supplying a method to check trigger instances and run attached listeners on success, and specifying a [codec](/docs/datastorage/codecs) to serialize the trigger instance (`T`).
...
Triggers must be registered to the `Registries.TRIGGER_TYPE` [registry](/docs/concepts/registries#methods-for-registering):
...
And then, triggers must define a [codec](/docs/datastorage/codecs) to serialize and deserialize the trigger instance by overriding `#codec`. This codec is typically created as a constant within the instance implementation.
...
Advancements can be [datagenned](/docs/resources/#data-generation) using an `AdvancementProvider`. An `AdvancementProvider` accepts a list of `AdvancementSubProviders`s, which actually generate the advancements using `Advancement.Builder`.
