...
url: "https://docs.neoforged.net/docs/resources/server/recipes/builtin/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:33.340Z"
---
...
- The key associates the characters used in the pattern with [ingredients](/docs/resources/server/recipes/ingredients). In the above example, all `X`s in the pattern must be iron ingots, and all `#`s must be sticks.

</li>
<li class="">`result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).</li>
<li class="">Not shown in the example is the `group` key. This optional string property creates a group in the recipe book. Recipes in the same group will be displayed as one in the recipe book.</li>
<li class="">Not shown in the example is `show_notification`. This optional boolean, when false, disables the toast shown on the top right hand corner on first use or unlock.</li>
</ul>
...
- `ingredients`: A list of [ingredients](/docs/resources/server/recipes/ingredients). The list order is preserved in code for recipe viewing purposes, but the recipe itself accepts the ingredients in any order.
- `result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).
...
- `source`: The [ingredient](/docs/resources/server/recipes/ingredients) that contains the potion contents to imbue.
- `material`: The [ingredient](/docs/resources/server/recipes/ingredients) used to imbue the source.
- `result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).
...
- `category`: This optional field defines the category in the crafting book.
- `group`: This optional string property creates a group in the recipe book. Recipes in the same group will be displayed as one in the recipe book, which typically makes sense for transmuted recipes.
- `input`: The [ingredient](/docs/resources/server/recipes/ingredients) to transmute.
- `material`: The [ingredient](/docs/resources/server/recipes/ingredients) that transforms the stack into its result.
...
Dye recipes are a special type of single item crafting recipes where the dye stacks' `DataComponents#DYE` are combined into a single `DataComponents#DYED_COLOR` and applied to the target stack to construct the result stack. All other components from the target stack are copied to the result stack, similar to [transmute crafting](#transmute-crafting). For example:
...
- `target`: The [ingredient](/docs/resources/server/recipes/ingredients) to apply the dyes to.
- `dye`: The [ingredient](/docs/resources/server/recipes/ingredients) dyes that are used to dye the stack. They must all have the `DataComponents#DYE` [data component](/docs/items/datacomponents).
- `result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).
...
- `ingredient`: The input [ingredient](/docs/resources/server/recipes/ingredients) of the recipe.
- `result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).
...
The `type` defines the recipe serializer (`minecraft:stonecutting`). The ingredient is an [ingredient](/docs/resources/server/recipes/ingredients), and the result is a basic [item stack template JSON](/docs/items/#json-representation). Like crafting recipes, they can also optionally specify a `group` for grouping in the recipe book.
...
- `base`: The base [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. Usually, this is some piece of equipment.
- `template`: The template [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. Usually, this is a smithing template.
- `addition`: The addition [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. Usually, this is some sort of material, for example a netherite ingot.
- `result`: The result of the recipe. This is [an item stack template's JSON representation](/docs/items/#json-representation).
...
- `base`: The base [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use the `minecraft:trimmable_armor` tag here.
- `template`: The template [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use a smithing trim template here.
- `addition`: The addition [ingredient](/docs/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use the `minecraft:trim_materials` tag here.
