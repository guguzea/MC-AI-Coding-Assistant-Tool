...
url: "https://docs.neoforged.net/docs/resources/client/models/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:03.931Z"
---
# Models
...
Items use the associated model(s) defined by its [client item](/docs/resources/client/models/items) while blocks use the associated model in the [blockstate file](#blockstate-files). These locations are relative to the `models` directory, so a model referenced by the name `examplemod:item/example_model` would be defined by the JSON at `assets/examplemod/models/item/example_model.json`.
...
- `loader`: NeoForge-added. Sets a custom model loader. See [Model Loaders](/docs/resources/client/models/modelloaders#model-loaders) for more information.
- `parent`: Sets a parent model, in the form of a [resource location](/docs/misc/identifier) relative to the `models` folder. All parent properties will be applied and then overridden by the properties set in the declaring model. Common parents include:
...
<li class="">`textures`: A sub-object that maps names (known as material variables) to `Material`s. Material variables can then be used in [elements](#elements). They can also be specified in elements, but left unspecified in order for child models to specify them.<!-- -->

- `sprite`: The [location of the texture](/docs/resources/client/textures).
...
- `elements`: A list of cuboid [elements](#elements).
- `display`: A sub-object that holds the different display options for different [perspectives](/docs/resources/client/models/modelsystem#perspectives), see linked article for possible keys. Only effective on item models, but often specified in block models so that item models can inherit the display options. Every perspective is an optional sub-object that may contain the following options, which are applied in that order:
...
`type`: NeoForge-added. Sets a custom block state model loader. See [Block State Model Loaders](/docs/resources/client/models/modelloaders#block-state-model-loaders) for more information.
...
Finally, a `neoforge:definition_type` can specify a custom model loader to register the block state file See [Block State Definition Loaders](/docs/resources/client/models/modelloaders#block-state-definition-loaders) for more information.

## Client Items

[Client items](/docs/resources/client/models/items) are used by the game to assign a model or multiple models to the states of the `ItemStack`. While there are some item-specific fields in the model JSON, client items consume models to render based on context, so most of their information has been moved to their own separate [section](/docs/resources/client/models/items).
...
For item tinting, please see the [relevant section in the client items article](/docs/resources/client/models/items#tinting).

## Registering Standalone Models

Models that are not associated with a block or item in some way, but are still required in other contexts (e.g. [block entity renderers](/docs/blockentities/ber)), can be registered through `ModelEvent.RegisterStandalone`:
