...
Now that we have our `ToolMaterial`, we can use it for [registering](/docs/concepts/registries#methods-for-registering) tools. All `tool` delegates have the same three parameters:
...
Custom tools can be created by adding a `Tool` [data component](/docs/items/datacomponents) (via `DataComponents#TOOL`) to the list of default components on your item via `Item.Properties#component`.
...
For shields, you can apply the [DataComponents#EQUIPPABLE](/docs/items/armor#equippable) data component for the offhand and `DataComponents#BLOCKS_ATTACKS` for reducing damage to the held entity when active.
