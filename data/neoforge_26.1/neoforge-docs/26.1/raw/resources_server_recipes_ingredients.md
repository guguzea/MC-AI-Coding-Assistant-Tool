...
url: "https://docs.neoforged.net/docs/resources/server/recipes/ingredients/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:36.589Z"
---
# Ingredients

`Ingredient`s are used in [recipes](/docs/resources/server/recipes/) to check whether a given [ItemStack](/docs/items/#itemstacks) is a valid input for the recipe. For this purpose, `Ingredient` implements `Predicate<ItemStack>`, and `#test` can be called to confirm if a given `ItemStack` matches the ingredient.
...
- `Ingredient.of(Blocks.IRON_BLOCK, Items.GOLD_BLOCK)` returns an ingredient that accepts either an iron or a gold block. The parameter is a vararg of [ItemLikes](/docs/items/#itemlike), which means that any amount of both blocks and items may be used.
...
- `Ingredient.of(BuiltInRegistries.ITEM.getOrThrow(ItemTags.WOODEN_SLABS))` returns an ingredient that accepts any item from the specified [tag](/docs/resources/server/tags), for example any wooden slab.
...
- `CustomDisplayIngredient.of(Ingredient.of(Items.DIRT), SlotDisplay.Empty.INSTANCE)` returns an ingredient with a custom [SlotDisplay](/docs/resources/server/recipes/#slot-displays) you provide to determine how the slot gets consumed for rendering on the client.
...
Custom ingredients are a [registry](/docs/concepts/registries), so we must register our ingredient. We do so using the `IngredientType` class provided by NeoForge, which is basically a wrapper around a [MapCodec](/docs/datastorage/codecs) and optionally a [StreamCodec](/docs/networking/streamcodecs).
