...
url: "https://docs.neoforged.net/docs/1.21.3/resources/server/loottables/custom/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:58:52.047Z"
---
# Custom Loot Objects

Due to the complexity of the loot table system, there are several [registries](/docs/1.21.3/concepts/registries#methods-for-registering) at work, all of which can be used by a modder to add more behavior.

All loot table related registries follow a similar pattern. To add a new registry entry, you generally extend some class or implement some interface that holds your functionality. Then, you define a [codec](/docs/1.21.3/datastorage/codecs) for serialization, and register that codec to the corresponding registry, using `DeferredRegister` like normal. This goes along with the "one base object, many instances" approach most registries (for example also blocks/blockstates and items/item stacks) use.
...
We then use this codec in [registration](/docs/1.21.3/concepts/registries#methods-for-registering):
...
Like with custom loot entry types, we then use this codec in [registration](/docs/1.21.3/concepts/registries#methods-for-registering):
...
And again, we then use the codec in [registration](/docs/1.21.3/concepts/registries#methods-for-registering), though this time directly:
...
We can [register](/docs/1.21.3/concepts/registries#methods-for-registering) the condition type to the registry using the condition's codec:
...
We can then [register](/docs/1.21.3/concepts/registries#methods-for-registering) the function type to the registry using the function's codec:
