...
version: "1.21.3"
pageId: "misc/config"
url: "https://docs.neoforged.net/docs/1.21.3/misc/config/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:53:02.382Z"
---
# Configuration
...
Operations that occur whenever a config is loaded or reloaded can be done using the `ModConfigEvent.Loading` and `ModConfigEvent.Reloading` events. The events must be [registered](/docs/1.21.3/concepts/events#registering-an-event-handler) to the mod event bus.
...
A configuration screen can be registered for a mod by registering a `IConfigScreenFactory` extension point during mod construction on the [client](/docs/1.21.3/concepts/sides#mod):
