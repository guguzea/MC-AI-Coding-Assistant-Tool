...
url: "https://docs.neoforged.net/docs/1.21.3/networking/streamcodecs/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:52:56.077Z"
---
# Stream Codecs

Stream codecs are a serialization tool used to describe how an object should be stored and read from a stream, such as buffers. Stream codecs are primarly used by Vanilla's [networking system](/docs/1.21.3/networking/payload) to sync data.

> **Info**
> info

As stream codecs are roughly analagous to [codecs](/docs/1.21.3/datastorage/codecs), this page has been formatted in the same way to show the similarities.
...
`TRUSTED_TAG` and `TRUSTED_COMPOUND_TAG` are variants of `TAG` and `COMPOUND_TAG`, respectively, that have an unlimited heap to decode the tag to, compared to the 2MiB limit of `TAG` and `COMPOUND_TAG`. Trusted tag stream codecs should ideally only be used in clientbound packets, such as what Vanilla does for [block entity data packet](/docs/1.21.3/blockentities/#synchronizing-on-block-update) and [entity data serializers](/docs/1.21.3/networking/entities#dynamic-data-parameters).
...
When constructing a stream codec, `B` should be the least-specific buffer type. For example, a `ResourceLocation` is sent as a string. As strings are supported by a regular `ByteBuf`, its type should be `StreamCodec`. `FriendlyByteBuf` contains methods for writing a `ChunkPos`, so its type should be `StreamCodec`. An `Item` needs access to the registry, so its type should be `StreamCodec`.
...
The `map` method transforms the value using two functions: one to transform the current type into the new type, and one to transform the new type back into the current type. This is analagous to [codec transformers](/docs/1.21.3/datastorage/codecs#transformers).
