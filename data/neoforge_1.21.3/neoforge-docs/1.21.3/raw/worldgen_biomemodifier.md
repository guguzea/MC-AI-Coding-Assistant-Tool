...
url: "https://docs.neoforged.net/docs/1.21.3/worldgen/biomemodifier/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:04:33.573Z"
---
# Biome Modifiers
...
To have NeoForge load a biome modifier JSON file into the game, the file will need to be under `data/<modid>/neoforge/biome_modifier/<path>.json` folder in the mod's resources, or in a [Datapack](/docs/1.21.3/resources/#data). Then, once NeoForge loads the biome modifier, it will read its instructions and apply the described modifications to all target biomes when the world is loaded up. Pre-existing biome modifiers from mods can be overridden by datapacks having a new JSON file at the exact same location and name.
...
- The [datapack registered](/docs/1.21.3/concepts/registries#datapack-registries) `BiomeModifier` used to modify the biome builder.
- The [statically registered](/docs/1.21.3/concepts/registries#methods-for-registering) `MapCodec` that encodes and decodes the modifiers.
...
All `BiomeModifier`s contain a `type` key that references the id of the `MapCodec` used for the `BiomeModifier`. The `codec` takes in the `MapCodec` that encodes and decodes the modifiers. This `MapCodec` is [statically registered](/docs/1.21.3/concepts/registries#methods-for-registering), with its id used as the `type` of the `BiomeModifier`.
...
A `BiomeModifier` JSON can be created through [data generation](/docs/1.21.3/resources/#data-generation) by passing a `RegistrySetBuilder` to `DatapackBuiltinEntriesProvider`. The JSON will be placed at `data/<modid>/neoforge/biome_modifier/<path>.json`.

For more information on how `RegistrySetBuilder` and `DatapackBuiltinEntriesProvider` work, please see the article on [Data Generation for Datapack Registries](/docs/1.21.3/concepts/registries#data-generation-for-datapack-registries).
