...
url: "https://docs.neoforged.net/docs/1.21.3/inventories/container/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:52:46.856Z"
---
# Containers

Many systems in Minecraft, such as [block entities](/docs/1.21.3/blockentities/) or entities, store items of some kind. To store items on something, Minecraft uses `Container` implementations.
...
Due to this, `Container`s can not only be implemented on block entities, but any other class as well. Notable examples include entity inventories, as well as common modded [items](/docs/1.21.3/items/) such as backpacks.
...
- `Nameable` defines a few methods related to setting (custom) names and, aside from many block entities, is implemented by classes such as `Entity`. This uses the [Component system](/docs/1.21.3/resources/client/i18n#components).
- `MenuProvider`, on the other hand, defines the `#createMenu` method, which allows an [AbstractContainerMenu](/docs/1.21.3/gui/menus) to be constructed from the container. This means that using this class is not desirable if you want a container without an associated GUI, for example in jukeboxes.
...
Until now, we mainly discussed `Container`s on `BlockEntity`s. However, they can also be applied to [ItemStacks](/docs/1.21.3/items/#itemstacks) using the `minecraft:container` [data component](/docs/1.21.3/items/datacomponents):
