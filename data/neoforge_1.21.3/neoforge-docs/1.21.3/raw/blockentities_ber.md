...
url: "https://docs.neoforged.net/docs/1.21.3/blockentities/ber/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:03:21.317Z"
---
# BlockEntityRenderer

A `BlockEntityRenderer`, often abbreviated as BER, is used to render [blocks](/docs/1.21.3/blocks/) in a way that cannot be represented with a [static baked model](/docs/1.21.3/resources/client/models/) (JSON, OBJ, others). For example, this could be used to dynamically render container contents of a chest-like block. A block entity renderer requires the block to have a [BlockEntity](/docs/1.21.3/blockentities/), even if the block does not store any data otherwise.
...
When you have created your BER, you must also register it to `EntityRenderersEvent.RegisterRenderers`, an [event](/docs/1.21.3/concepts/events#registering-an-event-handler) fired on the [mod event bus](/docs/1.21.3/concepts/events#event-buses):
...
`BlockEntityWithoutLevelRenderer`, colloquially known as BEWLR, is an adaptation of the regular `BlockEntityRenderer` for special [item](/docs/1.21.3/items/) rendering (hence "without level", as items do not have level context). Its overall purpose is the same: do special rendering for cases where static models aren't enough.
...
Finally, the item has to know that it should use the BEWLR for its rendering. This is done by having the final [`BakedModel`][bakedmodel] return true for `#isCustomRenderer`. The easiest way to do this is to have the [item model JSON](/docs/1.21.3/resources/client/models/) with a `parent` of `minecraft:builtin/entity`:
