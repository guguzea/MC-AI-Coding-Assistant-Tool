...
- A **RecipeSerializer** is basically a wrapper around a [MapCodec](/docs/1.21.3/datastorage/codecs) and a [StreamCodec](/docs/1.21.3/networking/streamcodecs), both used for serialization.
...
The contents of recipe files vary greatly depending on the selected type. Common to all recipe files are the `type` and [neoforge:conditions](/docs/1.21.3/resources/server/conditions) properties:
...
A full list of types provided by Minecraft can be found in the [Built-In Recipe Types article](/docs/1.21.3/resources/server/recipes/builtin). Mods can also [define their own recipe types](/docs/1.21.3/resources/server/recipes/custom).
...
Some mechanisms in vanilla are generally considered recipes, but are implemented differently in code. This is generally either due to legacy reasons, or because the "recipes" are constructed from other data (e.g. [tags](/docs/1.21.3/resources/server/tags)).
...
Anvils have two input slots and one output slot. The only vanilla use cases are tool repairing, combining and renaming, and since each of these use cases needs special handling, no recipe files are provided. However, the system can be built upon using `AnvilUpdateEvent`. This [event](/docs/1.21.3/concepts/events) allows getting the input (left input slot) and material (right input slot) and allows setting an output item stack, as well as the experience cost and the number of materials to consume. The process can also be prevented as a whole by [canceling](/docs/1.21.3/concepts/events#cancellable-events) the event.
...
See [the Brewing chapter in the Mob Effects & Potions article](/docs/1.21.3/items/mobeffects#brewing).
...
Of note is the `RecipeOutput` parameter. Minecraft uses this object to automatically generate a recipe advancement for you. On top of that, NeoForge injects [conditions](/docs/1.21.3/resources/server/conditions) support into `RecipeOutput`, which can be called on via `#withConditions`.

Recipes themselves are commonly added through subclasses of `RecipeBuilder`. Listing all vanilla recipe builders is beyond the scope of this article (they are explained in the [Built-In Recipe Types article](/docs/1.21.3/resources/server/recipes/builtin)), however creating your own builder is explained [in the custom recipes page](/docs/1.21.3/resources/server/recipes/custom#data-generation).
