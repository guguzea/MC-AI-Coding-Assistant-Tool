...
A few places in NeoForge also use [Maven version ranges](https://maven.apache.org/enforcer/enforcer-rules/versionRanges.html), for example the Minecraft and NeoForge version ranges in the [neoforge.mods.toml](/docs/1.21.3/gettingstarted/modfiles#neoforgemodstoml) file. These are mostly, but not fully compatible with semver (the `pre`-tag is not considered by it, for example).
