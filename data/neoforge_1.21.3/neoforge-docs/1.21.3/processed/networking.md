...
There is a technique provided by NeoForge to facilitate communication mostly built on top of [netty](https://netty.io). This technique can be used by listening for the `RegisterPayloadHandlersEvent` event, and then registering a specific type of [payloads](/docs/1.21.3/networking/payload), its reader, and its handler function to the registrar.
