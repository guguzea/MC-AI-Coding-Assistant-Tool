...
Then, with that, we can create a new `SlotDisplay`. The `SlotDisplay.Type` must be [registered](/docs/1.21.3/concepts/registries#methods-for-registering):
...
Next up, our recipe type. This is fairly straightforward because there's no data other than a name associated with a recipe type. They are one of two [registered](/docs/1.21.3/concepts/registries#methods-for-registering) parts of the recipe system, so like with all other registries, we create a `DeferredRegister` and register to it:
...
A recipe serializer provides two codecs, one map codec and one stream codec, for serialization from/to JSON and from/to network, respectively. This section will not go in depth about how the codecs work, please see [Map Codecs](/docs/1.21.3/datastorage/codecs) and [Stream Codecs](/docs/1.21.3/networking/streamcodecs) for more information.
...
And now, during [datagen](/docs/1.21.3/resources/server/recipes/#data-generation), you can call on your recipe builder like any other:
