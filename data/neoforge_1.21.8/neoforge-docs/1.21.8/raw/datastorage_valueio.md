...
url: "https://docs.neoforged.net/docs/1.21.8/datastorage/valueio/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:00:24.466Z"
---
# Value I/O

The Value I/O system is a standardized serialization method to manipulate data of some backing object, such as [CompoundTags for NBT](/docs/1.21.8/datastorage/nbt).
...
`ValueIOSerializable` is a NeoForge-added interface for objects that can be serialized and deserialized using value I/Os. NeoForge uses this API to handle [data attachments](/docs/1.21.8/datastorage/attachments). The interface provides two methods: `serialize` to write the object to a `ValueOutput`, and `deserialize` to read the object from a `ValueInput`.
...
Value I/O for [NBTs](/docs/1.21.8/datastorage/nbt) is handled via `TagValueOutput` and `TagValueInput`.
