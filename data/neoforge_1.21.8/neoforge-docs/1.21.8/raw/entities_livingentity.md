...
url: "https://docs.neoforged.net/docs/1.21.8/entities/livingentity/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:06:02.590Z"
---
# Living Entities, Mobs & Players

Living entities are a big subgroup of [entities](/docs/1.21.8/entities/) that all inherit from the common `LivingEntity` superclass. These include mobs (through the `Mob` subclass), players (through the `Player` subclass) and armor stands (through the `ArmorStand` subclass).

Living entities have a number of additional properties that regular entities do not have. These include [attributes](/docs/1.21.8/entities/attributes), [mob effects](/docs/1.21.8/items/mobeffects), damage tracking and more.

## Health, Damage and Healing

*See also: [Attributes](/docs/1.21.8/entities/attributes).*
...
By default, max health is determined by the `minecraft:generic.max_health` [attribute](/docs/1.21.8/entities/attributes), and the current health is set to the same value when [spawning](/docs/1.21.8/entities/#spawning-entities). When the entity is damaged by calling [Entity#hurtServer](/docs/1.21.8/entities/#damaging-entities) on it, the current health is decreased according to the damage calculations. Many entities, such as zombies, will by default then remain at that reduced health value, while some, such as players, can heal these lost hit points again.
...
When [taking damage](/docs/1.21.8/entities/#damaging-entities), living entities will apply some additional calculations, such as considering the `minecraft:generic.armor` attribute (except for [damage types](/docs/1.21.8/resources/server/damagetypes) that are in the `minecraft:bypasses_armor` [tag](/docs/1.21.8/resources/server/tags)) as well as the `minecraft:generic.absorption` attribute. Living entities can also override `#onDamageTaken` to perform post-attack behavior; it is only called if the final damage value is greater than zero.
...
Common to all these events is the `DamageContainer`. A new `DamageContainer` is instantiated at the start of the attack, and discarded after the attack has finished. It contains the original [DamageSource](/docs/1.21.8/resources/server/damagetypes#creating-and-using-damage-sources), the original damage amount, and a list of all individual modifications - armor, absorption, [enchantments](/docs/1.21.8/resources/server/enchantments/), [mob effects](/docs/1.21.8/items/mobeffects), etc. The `DamageContainer` is passed to all events listed below, and you can check what modifications have already been done to make your own changes as necessary.
...
Callbacks are applied in the order they are added. This means that callbacks added in an event handler with higher [priority](/docs/1.21.8/concepts/events#priority) will be run first.
...
*See [Mob Effects & Potions](/docs/1.21.8/items/mobeffects).*

## Equipment

*See [Containers on Entities](/docs/1.21.8/inventories/container).*
...
- `AbstractClientPlayer`: This class is used as a base for the two client players, both used to represent players on the [logical client](/docs/1.21.8/concepts/sides#the-logical-side).
...
- `ServerPlayer`: This class is used to represent players on the [logical server](/docs/1.21.8/concepts/sides#the-logical-side).
...
In addition to the [regular ways of spawning](/docs/1.21.8/entities/#spawning-entities) - that is, the `/summon` command and the in-code way via `EntityType#spawn` or `Level#addFreshEntity` -, `Mob`s can also be spawned through some other means. `ArmorStand`s can be spawned through regular means, and `Player`s should not be instantiated yourself, except for `FakePlayer`s.

### Spawn Eggs

It is common (though not required) to [register](/docs/1.21.8/concepts/registries) a spawn egg for mobs. This is done through the `SpawnEggItem` class, which has been patched by NeoForge to do some extra setup, such as registering the color handler and adding the spawn egg to the internal `SpawnEggItem` -> `EntityType` map.
...
As an item like any other, the item should be added to a [creative tab](/docs/1.21.8/items/#creative-tabs), and a [client item](/docs/1.21.8/resources/client/models/items), [model](/docs/1.21.8/resources/client/models/) and [translation](/docs/1.21.8/resources/client/i18n) should be added.

### Natural Spawning

*See also [Entities/MobCategory](/docs/1.21.8/entities/#mobcategory), [Worldgen/Biome Modifers/Add Spawns](/docs/1.21.8/worldgen/biomemodifier#add-spawns), [Worldgen/Biome Modifers/Add Spawn Costs](/docs/1.21.8/worldgen/biomemodifier#add-spawn-costs); and [Spawn Cycle](https://minecraft.wiki/w/Mob_spawning#Spawn_cycle) on the [Minecraft Wiki](https://minecraft.wiki).*
