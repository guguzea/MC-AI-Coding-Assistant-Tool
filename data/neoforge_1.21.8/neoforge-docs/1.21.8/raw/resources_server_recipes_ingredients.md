...
- `CustomDisplayIngredient.of(Ingredient.of(Items.DIRT), SlotDisplay.Empty.INSTANCE)` returns an ingredient with a custom [SlotDisplay](/docs/1.21.8/resources/server/recipes/#slot-displays) you provide to determine how the slot gets consumed for rendering on the client.
...
Custom ingredients are a [registry](/docs/1.21.8/concepts/registries), so we must register our ingredient. We do so using the `IngredientType` class provided by NeoForge, which is basically a wrapper around a [MapCodec](/docs/1.21.8/datastorage/codecs) and optionally a [StreamCodec](/docs/1.21.8/networking/streamcodecs).
