...
url: "https://docs.neoforged.net/docs/1.21.8/blockentities/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:51:16.992Z"
---
# Block Entities

Block entities allow the storage of data on [blocks](/docs/1.21.8/blocks/) in cases where [block states](/docs/1.21.8/blocks/states) are not suitable. This is especially the case for data with a non-finite amount of options, such as inventories. Block entities are stationary and bound to a block, but otherwise share many similarities with [entities](/docs/1.21.8/entities/), hence the name.

> **Note**
> note

If you have a finite and reasonably small amount (= a few hundred at most) of possible states for your block, you might want to consider using [block states](/docs/1.21.8/blocks/states) instead.
...
Remember that the `DeferredRegister` must be registered to the [mod event bus](/docs/1.21.8/concepts/events#event-buses)!
...
And then, you of course need to use this class as the type in your [block registration](/docs/1.21.8/blocks/#basic-blocks):
...
Data can be read from and written to [value I/O](/docs/1.21.8/datastorage/valueio) using the `#loadAdditional` and `#saveAdditional` methods, respectively. These methods are called when the block entity is synced to disk or over the network.
