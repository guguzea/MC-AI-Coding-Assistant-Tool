...
*See also [Living Entities/Natural Spawning](/docs/1.21.8/entities/livingentity#natural-spawning).*
...
- The [datapack registered](/docs/1.21.8/concepts/registries#datapack-registries) `BiomeModifier` used to modify the biome builder.
- The [statically registered](/docs/1.21.8/concepts/registries#methods-for-registering) `MapCodec` that encodes and decodes the modifiers.
...
All `BiomeModifier`s contain a `type` key that references the id of the `MapCodec` used for the `BiomeModifier`. The `codec` takes in the `MapCodec` that encodes and decodes the modifiers. This `MapCodec` is [statically registered](/docs/1.21.8/concepts/registries#methods-for-registering), with its id used as the `type` of the `BiomeModifier`.
