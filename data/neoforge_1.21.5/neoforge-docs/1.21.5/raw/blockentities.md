...
url: "https://docs.neoforged.net/docs/1.21.5/blockentities/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:51:59.327Z"
---
# Block Entities

Block entities allow the storage of data on [blocks](/docs/1.21.5/blocks/) in cases where [block states](/docs/1.21.5/blocks/states) are not suitable. This is especially the case for data with a non-finite amount of options, such as inventories. Block entities are stationary and bound to a block, but otherwise share many similarities with [entities](/docs/1.21.5/entities/), hence the name.

> **Note**
> note

If you have a finite and reasonably small amount (= a few hundred at most) of possible states for your block, you might want to consider using [block states](/docs/1.21.5/blocks/states) instead.

## Creating and Registering Block Entities

Like entities and unlike blocks, the `BlockEntity` class represents the block entity instance, not the [registered](/docs/1.21.5/concepts/registries#methods-for-registering) singleton object. The singleton is expressed through the `BlockEntityType` class instead. We will need both to create a new block entity.
...
[Registration](/docs/1.21.5/concepts/registries#methods-for-registering) happens in a similar fashion to entities. We create an instance of the associated singleton class `BlockEntityType` and register it to the block entity type registry, like so:
...
Remember that the `DeferredRegister` must be registered to the [mod event bus](/docs/1.21.5/concepts/events#event-buses)!
...
And then, you of course need to use this class as the type in your [block registration](/docs/1.21.5/blocks/#basic-blocks):
