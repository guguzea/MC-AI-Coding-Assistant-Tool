...
url: "https://docs.neoforged.net/docs/1.21.5/resources/server/recipes/builtin/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:59:53.089Z"
---
...
- The key associates the characters used in the pattern with [ingredients](/docs/1.21.5/resources/server/recipes/ingredients). In the above example, all `X`s in the pattern must be iron ingots, and all `#`s must be sticks.

</li>
<li class="">`result`: The result of the recipe. This is [an item stack's JSON representation](/docs/1.21.5/items/#json-representation).</li>
...
- `ingredients`: A list of [ingredients](/docs/1.21.5/resources/server/recipes/ingredients). The list order is preserved in code for recipe viewing purposes, but the recipe itself accepts the ingredients in any order.
- `result`: The result of the recipe. This is [an item stack's JSON representation](/docs/1.21.5/items/#json-representation).
...
- `input`: The [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) to transmute.
- `material`: The [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) that transforms the stack into its result.
- `result`: The result of the recipe. This is [an item stack's JSON representation](/docs/1.21.5/items/#json-representation).
...
- `ingredient`: The input [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe.
- `result`: The result of the recipe. This is [an item stack's JSON representation](/docs/1.21.5/items/#json-representation).
...
The `type` defines the recipe serializer (`minecraft:stonecutting`). The ingredient is an [ingredient](/docs/1.21.5/resources/server/recipes/ingredients), and the result is a basic [item stack JSON](/docs/1.21.5/items/#json-representation). Like crafting recipes, they can also optionally specify a `group` for grouping in the recipe book.
...
- `base`: The base [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. Usually, this is some piece of equipment.
- `template`: The template [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. Usually, this is a smithing template.
- `addition`: The addition [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. Usually, this is some sort of material, for example a netherite ingot.
- `result`: The result of the recipe. This is [an item stack's JSON representation](/docs/1.21.5/items/#json-representation).
...
- `base`: The base [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use the `minecraft:trimmable_armor` tag here.
- `template`: The template [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use a smithing trim template here.
- `addition`: The addition [ingredient](/docs/1.21.5/resources/server/recipes/ingredients) of the recipe. All vanilla use cases use the `minecraft:trim_materials` tag here.
