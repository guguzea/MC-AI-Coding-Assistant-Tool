...
url: "https://docs.neoforged.net/docs/1.21.5/resources/server/loottables/glm/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:59:49.571Z"
---
...
GLMs work by first rolling the associated [loot table](/docs/1.21.5/resources/server/loottables/) and then applying the GLM to the result of rolling the table. GLMs are also stacking, rather than last-load-wins, to allow for multiple mods to modify the same loot table, this is similar to [tags](/docs/1.21.5/resources/server/tags).
...
A common use case for GLMs is to add extra loot to one specific loot table. To achieve this, the [neoforge:loot_table_id condition](/docs/1.21.5/resources/server/loottables/lootconditions#neoforgeloot_table_id) can be used.
...
To tell the game about the existence of our loot modifier, we must define and [register](/docs/1.21.5/concepts/registries#methods-for-registering) a [codec](/docs/1.21.5/datastorage/codecs) for it. Reiterating on our previous example with the three fields, this would look something like this:
...
Then, we [register](/docs/1.21.5/concepts/registries#methods-for-registering) the codec to the registry:
...
GLMs can be [datagenned](/docs/1.21.5/resources/#data-generation). This is done by subclassing `GlobalLootModifierProvider`:
