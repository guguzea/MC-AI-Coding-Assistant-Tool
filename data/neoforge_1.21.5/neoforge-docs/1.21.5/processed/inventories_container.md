...
Many systems in Minecraft, such as [block entities](/docs/1.21.5/blockentities/) or [entities](/docs/1.21.5/entities/), store items of some kind. To store items on something, Minecraft uses `Container` implementations.
...
Due to this, `Container`s can not only be implemented on block entities, but any other class as well. Notable examples include entity inventories, as well as common modded [items](/docs/1.21.5/items/) such as backpacks.
...
`BlockEntity`s that implement `Container` handle dropping their contents by default. If you choose not to implement `Container`, then you will need to handle the [removal logic](/docs/1.21.5/blockentities/#removing-block-entities).
...
Until now, we mainly discussed `Container`s on `BlockEntity`s. However, they can also be applied to [ItemStacks](/docs/1.21.5/items/#itemstacks) using the `minecraft:container` [data component](/docs/1.21.5/items/datacomponents):
...
`Container`s on [Entitys](/docs/1.21.5/entities/) are finicky: whether an entity has a container or not cannot be universally determined. It all depends on what entity you are handling, and as such can require a lot of special-casing.
