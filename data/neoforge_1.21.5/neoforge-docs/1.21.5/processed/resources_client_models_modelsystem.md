...
- A `BlockState`: The [blockstate](/docs/1.21.5/blocks/states) being rendered. May be null, indicating that an item is being rendered.
...
Information about item rendering can be found in the [Manually Rendering an Item](/docs/1.21.5/resources/client/models/items#manually-rendering-an-item) section.
...
After writing your model wrapper class, you must apply the wrappers to the models it should affect. Do so in a [client-side](/docs/1.21.5/concepts/sides) [event handler](/docs/1.21.5/concepts/events) for `ModelEvent.ModifyBakingResult` on the [mod event bus](/docs/1.21.5/concepts/events#event-buses):
...
It is generally encouraged to use a [custom model loader](/docs/1.21.5/resources/client/models/modelloaders) over wrapping baked models in `ModelEvent.ModifyBakingResult` when possible. Custom model loaders can also use delegate models if needed.
