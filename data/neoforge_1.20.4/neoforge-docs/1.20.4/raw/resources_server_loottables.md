...
`LootTableLoadEvent` is an [event](/docs/1.20.4/concepts/events#registering-an-event-handler) fired on the NeoForge event bus which is fired whenever a loot table is loaded. If the event is canceled, then an empty loot table will be loaded instead.

> **Info**
> info

Do **not** modify a loot table's drops through this event. Those modifications should be done using [global loot modifiers](/docs/1.20.4/resources/server/glm).
...
NeoForge adds an additional `LootItemCondition` which allows certain items to generate for a specific table. This is typically used within [global loot modifiers](/docs/1.20.4/resources/server/glm).
