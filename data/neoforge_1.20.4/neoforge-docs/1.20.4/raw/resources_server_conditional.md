...
url: "https://docs.neoforged.net/docs/1.20.4/resources/server/conditional/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:57:13.770Z"
---
...
Conditionally-loaded recipes additionally have wrappers for [data generation](/docs/1.20.4/datagen/recipes) through `RecipeOutput#withConditions`. Other generators (like the data map one) will usually have a vararg `ICondition...` array in their methods for adding conditions to objects.
...
Example recipe that will only be loaded if the examplemod mod is loaded
...
Custom conditions can be created by implementing `ICondition` and creating a [Codec](/docs/1.20.4/datastorage/codecs) for it.
...
The `ICondition#codec` method should return the codec used to encode and decode the condition. This codec **must** be [registered](/docs/1.20.4/concepts/registries) to the `NeoForgeRegistries#CONDITION_SERIALIZERS` registry. The name the codec is registered under will be the name used to refer to that condition in the `type` field.
