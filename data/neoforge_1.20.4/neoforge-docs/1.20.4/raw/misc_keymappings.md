...
url: "https://docs.neoforged.net/docs/1.20.4/misc/keymappings/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:57:03.054Z"
---
# Key Mappings
...
A `KeyMapping` can be registered by listening to the `RegisterKeyMappingsEvent` on the [mod event bus](/docs/1.20.4/concepts/events#event-buses) only on the physical client and calling `#register`.
...
A `KeyMapping` can be created using it's constructor. The `KeyMapping` takes in a [translation key](/docs/1.20.4/resources/client/i18n#components) defining the name of the mapping, the default input of the mapping, and the [translation key](/docs/1.20.4/resources/client/i18n#components) defining the category the mapping will be put within in the [Controls option menu](https://minecraft.wiki/w/Options#Controls).

> **Tip**
> tip

A `KeyMapping` can be added to a custom category by providing a category [translation key](/docs/1.20.4/resources/client/i18n#components) not provided by vanilla. Custom category translation keys should contain the mod id (e.g. `key.categories.examplemod.examplecategory`).
...
If you do not own the screen which you are trying to check a **key** for, you can listen to the `Pre` or `Post` events of ScreenEvent.KeyPressed on the [Forge event bus](/docs/1.20.4/concepts/events#registering-an-event-handler) instead.
...
If you do not own the screen which you are trying to check a **mouse** for, you can listen to the `Pre` or `Post` events of ScreenEvent.MouseButtonPressed on the [Forge event bus](/docs/1.20.4/concepts/events#registering-an-event-handler) instead.
