...
After implementation, the provider must be [added](/docs/1.20.4/resources/#data-generation) to the `DataGenerator`.
...
[ItemStack outputs](/docs/1.20.4/resources/server/recipes/#recipe-itemstack-result) in recipes are not supported within vanilla recipe builders. A `FinishedRecipe` must be built in a different manner for existing vanilla recipe serializers to generate this data.
...
[Conditional recipes](/docs/1.20.4/resources/server/conditional) can also be data generated via ConditionalRecipe.Builder. The builder can be obtained using `#builder`.
