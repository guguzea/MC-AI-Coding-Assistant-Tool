...
Data-driven recipes and their unlocking advancement can be [generated](/docs/1.20.4/datagen/recipes) via `RecipeProvider`.
...
Recipes and their unlocking advancement can be [loaded conditionally and defaulted](/docs/1.20.4/resources/server/conditional#implementations) depending on what information is present (mod loaded, item exists, etc.).
...
Larger crafting grids in recipes can be [data generated](/docs/1.20.4/datagen/recipes).

### Ingredient Types

A few additional [ingredient types](/docs/1.20.4/resources/server/recipes/ingredients#neoforge-types) are added to allow recipes to have inputs which check tag data or combine multiple ingredients into a single input checker.
