...
url: "https://docs.neoforged.net/docs/1.20.4/concepts/events/"
platform: "neoforge"
fetchedAt: "2026-08-06T04:46:19.795Z"
---
# Events
...
Some events are only fired on one [side](/docs/1.20.4/concepts/sides). Common examples include the various render events, which are only fired on the client. Since client-only events generally need to access other client-only parts of the Minecraft codebase, they need to be registered accordingly.
...
- The registry events are fired, these include [NewRegistryEvent](/docs/1.20.4/concepts/registries#custom-registries), [DataPackRegistryEvent.NewRegistry](/docs/1.20.4/concepts/registries#custom-datapack-registries) and, for each registry, [RegisterEvent](/docs/1.20.4/concepts/registries#registerevent).
- `FMLCommonSetupEvent` is fired. This is where various miscellaneous setup happens.
- The [sided](/docs/1.20.4/concepts/sides) setup is fired: `FMLClientSetupEvent` if on a physical client, and `FMLDedicatedServerSetupEvent` if on a physical server.
