...
url: "https://docs.neoforged.net/docs/1.20.4/resources/server/glm/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:00:33.920Z"
---
...
<ol>
<li class="">Create a `global_loot_modifiers.json`.<!-- -->

- This will tell NeoForge about your modifiers and works similar to [tags](/docs/1.20.4/resources/server/tags).

</li>
<li class="">A serialized json representing your modifier.<!-- -->

- This will contain all of the data about your modification and allows data packs to tweak your effect.

</li>
<li class="">A class that extends `IGlobalLootModifier`.<!-- -->
...
<li class="">Finally, a codec to encode and decode your operational class.<!-- -->

- This is [registered](/docs/1.20.4/concepts/registries#methods-for-registering) as any other `IForgeRegistryEntry`.
...
`entries` is an *ordered list* of the modifiers that will be loaded. The [ResourceLocation](/docs/1.20.4/misc/resourcelocation)s specified points to their associated entry within `data/<namespace>/loot_modifiers/<path>.json`. This is primarily relevant to data pack makers for resolving conflicts between modifiers from separate mods.
...
`type` represents the registry name of the [codec](#the-loot-modifier-codec) used to read the associated JSON file. This must always be present.
...
`#codec` returns the registered [codec](#the-loot-modifier-codec) used to encode and decode the modifier to/from JSON.
...
The connector between the JSON and the `IGlobalLootModifier` instance is a [Codec<T>](/docs/1.20.4/datastorage/codecs), where `T` represents the type of the `IGlobalLootModifier` to use.

For ease of convenience, a loot conditions codec has been provided for an easy addition to a record-like codec via `LootModifier#codecStart`. This is utilized for [data generation](/docs/1.20.4/datagen/glm) of the associated loot modifier.
...
[Examples](https://github.com/neoforged/NeoForge/blob/1.20.x/tests/src/main/java/net/neoforged/neoforge/debug/loot/GlobalLootModifiersTest.java) can be found on the NeoForge Git repository, including silk touch and smelting effects.
