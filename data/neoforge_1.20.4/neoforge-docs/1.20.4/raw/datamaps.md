...
platform: "neoforge"
fetchedAt: "2026-06-01T10:54:46.075Z"
---
# Data Maps
...
A data map type should be statically created and then registered to the `RegisterDataMapTypesEvent` (which is fired on the [mod event bus](/docs/1.20.4/concepts/events)). The `DataMapType` can be created using a DataMapType.Builder, through `DataMapType#builder`.
...
For more information, please [check out the dedicated page](/docs/1.20.4/datamaps/structure).
...
Data maps can be [generated](/docs/1.20.4/resources/#data-generation) through `DataMapProvider`.
...
There are `add` overloads that accept raw `ResourceLocation`s if you want to attach values to objects added by optional dependencies. In that case you should also provide [a loading condition](/docs/1.20.4/resources/server/conditional) through the var-args parameter to avoid crashes.
