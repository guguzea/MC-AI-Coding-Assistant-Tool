...
url: "https://docs.neoforged.net/docs/1.20.4/resources/server/tags/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:57:18.562Z"
---
# Tags
...
Tags are declared in your mod's [datapack](/docs/1.20.4/resources/server/). For example, a `TagKey` with a given identifier of  `modid:foo/tagname` will reference a tag at `/data//tags/blocks/foo/tagname.json`. Tags for `Block`s, `Item`s, `EntityType`s, `Fluid`s, and `GameEvent`s use the plural forms for their folder location while all other registries use the singular version (`EntityType` uses the folder `entity_types` while `Potion` would use the folder `potion`).
...
myPotionTag = ForgeRegistries.POTIONS.tags().createTagKey(new ResourceLocation("mymod", "mypotiongroup"));

public static final TagKey myVillagerTypeTag = TagKey.create(Registries.VILLAGER_TYPE, new ResourceLocation("mymod", "myvillagertypegroup"));
...
- If there is a NeoForge tag that fits your block or item, add it to that tag. The list of tags declared by NeoForge can be seen on [GitHub](https://github.com/neoforged/NeoForge/tree/1.20.x/src/generated/resources/data/forge/tags).
...
Tags are directly supported by Vanilla. See the respective Vanilla wiki pages for [recipes](https://minecraft.wiki/w/Recipe#JSON_format) and [advancements](https://minecraft.wiki/w/Advancement) for usage details.
