...
version: "1.20.4"
pageId: "items/tools"
url: "https://docs.neoforged.net/docs/1.20.4/items/tools/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:00:05.532Z"
---
# Tools & Armor

Tools are [items](/docs/1.20.4/items/) whose primary use is to break [blocks](/docs/1.20.4/blocks/). Many mods add new tool sets (for example copper tools) or new tool types (for example hammers).
...
When creating a `Tier`, it is assigned a block [tag](/docs/1.20.4/resources/server/tags) containing blocks that need this tool (or a better one) to be broken. For example, the `minecraft:needs_iron_tool` tag contains Diamond Ores (among others), and the `minecraft:needs_diamond_tool` tag contains blocks like Obsidian and Ancient Debris.
...
Instead of or in addition to a `Tier`, you can also pass in other tiers' ids into these lists. For example, say we want to make our material be considered weaker than both iron and the Osmium tools from [Mekanism Tools](https://www.curseforge.com/minecraft/mc-mods/mekanism-tools), we could do that like so:
