...
url: "https://docs.neoforged.net/docs/1.20.4/items/mobeffects/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:57:00.144Z"
---
# Mob Effects & Potions
...
- A `MobEffect` affects an entity every tick. Like [blocks](/docs/1.20.4/blocks/) or [items](/docs/1.20.4/items/), `MobEffect`s are registry objects, meaning they must be [registered](/docs/1.20.4/concepts/registries) and are singletons.
...
<li class="">A `MobEffectInstance` is an instance of a `MobEffect`, with a duration, amplifier and some other properties set (see below). `MobEffectInstance`s are to `MobEffect`s what [ItemStacks](/docs/1.20.4/items/#itemstacks) are to `Item`s.</li>
...
The UUID used must be a valid and unique UUIDv4, as for some reason, Mojang decided to use UUIDs here instead of some text-based identifier. A UUID is best obtained through an online generator, for example [uuidgenerator.net](https://www.uuidgenerator.net/version4).
...
Many effects have their logic applied in other places. For example, the levitation effect is applied in the living entity movement handler. For modded `MobEffect`s, it often makes sense to apply them in an [event handler](/docs/1.20.4/concepts/events). NeoForge also provides a few events related to effects:
...
public static final Supplier MY_POTION = POTIONS.register("my_potion", () -> new Potion(new MobEffectInstance(MY_MOB_EFFECT.get(), 3600)));
...
Potions are traditionally made in the Brewing Stand. Unfortunately, Mojang does not provide [datapack](/docs/1.20.4/resources/server/) support for brewing recipes, so we have to be a little old-fashioned and add our recipes through code. This is done like so:
...
This should be called some time during setup, for example during [FMLCommonSetupEvent](/docs/1.20.4/concepts/events#event-buses). Make sure to wrap this into an `event.enqueueWork()` call, as the brewing recipe registry is not thread-safe.
