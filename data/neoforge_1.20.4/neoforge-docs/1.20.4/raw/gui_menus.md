...
platform: "neoforge"
fetchedAt: "2026-06-01T10:55:35.044Z"
---
# Menus
...
`MenuType`s must be [registered](/docs/1.20.4/concepts/registries#methods-for-registering).
...
Since `Container`s used by `Slot`s are deprecated by NeoForge in favor of using the [IItemHandler capability](/docs/1.20.4/datastorage/capabilities#neoforge-provided-capabilities), the rest of the explanation will revolve around using the capability variant: `SlotItemHandler`.
...
Once a menu type has been registered, the menu itself has been finished, and a [screen](/docs/1.20.4/gui/screens) has been attached, a menu can then be opened by the player. Menus can be opened by calling `NetworkHooks#openScreen` on the logical server. The method takes in the player opening the menu, the `MenuProvider` of the server side menu, and optionally a `FriendlyByteBuf` if extra data needs to be synced to the client.

> **Note**
> note

`NetworkHooks#openScreen` with the `FriendlyByteBuf` parameter should only be used if a menu type was created using an [IContainerFactory](#icontainerfactory).

#### MenuProvider

A `MenuProvider` is an interface that contains two methods: `#createMenu`, which creates the server instance of the menu, and `#getDisplayName`, which returns a component containing the title of the menu to pass to the [screen](/docs/1.20.4/gui/screens). The `#createMenu` method contains three parameter: the container id of the menu, the inventory of the player who opened the menu, and the player who opened the menu.
