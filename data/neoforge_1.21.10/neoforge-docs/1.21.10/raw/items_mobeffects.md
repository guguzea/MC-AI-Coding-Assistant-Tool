...
url: "https://docs.neoforged.net/docs/1.21.10/items/mobeffects/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:09.796Z"
---
# Mob Effects & Potions

Status effects, sometimes known as potion effects and referred to in-code as `MobEffect`s, are effects that influence a [LivingEntity](/docs/1.21.10/entities/livingentity) every tick. This article explains how to use them, what the difference between an effect and a potion is, and how to add your own.

## Terminology

- A `MobEffect` affects an entity every tick. Like [blocks](/docs/1.21.10/blocks/) or [items](/docs/1.21.10/items/), `MobEffect`s are registry objects, meaning they must be [registered](/docs/1.21.10/concepts/registries#methods-for-registering) and are singletons.
...
<li class="">A `MobEffectInstance` is an instance of a `MobEffect`, with a duration, amplifier and some other properties set (see below). `MobEffectInstance`s are to `MobEffect`s what [ItemStacks](/docs/1.21.10/items/#itemstacks) are to `Item`s.</li>
<li class="">A `Potion` is a collection of `MobEffectInstance`s. Vanilla mainly uses potions for the four potion items (read on), however, they can be applied to any item at will. It is up to the item if and how the item then uses the potion set on it.</li>
<li class="">A **potion item** is an item that is meant to have a potion set on it. This is an informal term, the vanilla `PotionItem` class has nothing to do with this (it refers to the "normal" potion item). Minecraft currently has four potion items: potions, splash potions, lingering potions, and tipped arrows; however more may be added by mods.</li>
</ul>
...
Like all registry objects, `MobEffect`s must be [registered](/docs/1.21.10/concepts/registries#methods-for-registering), like so:
...
The `MobEffect` class also provides default functionality for adding [attribute modifiers](/docs/1.21.10/entities/attributes#attribute-modifiers) to affected entities, and also removing them when the effect expires or is removed through other means. For example, the speed effect adds an attribute modifier for movement speed. Effect attribute modifiers are added like so:
...
Then, [register](/docs/1.21.10/concepts/registries#methods-for-registering) your effect like normal.

### Events

Many effects have their logic applied in other places. For example, the levitation effect is applied in the living entity movement handler. For modded `MobEffect`s, it often makes sense to apply them in an [event handler](/docs/1.21.10/concepts/events). NeoForge also provides a few events related to effects:
...
Potions are traditionally made in the Brewing Stand. Unfortunately, Mojang does not provide [datapack](/docs/1.21.10/resources/#data) support for brewing recipes, so we have to be a little old-fashioned and add our recipes through code via the `RegisterBrewingRecipesEvent` event. This is done like so:
