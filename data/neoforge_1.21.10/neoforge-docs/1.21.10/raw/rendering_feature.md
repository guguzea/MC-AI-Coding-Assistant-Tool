...
url: "https://docs.neoforged.net/docs/1.21.10/rendering/feature/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:50:38.215Z"
---
# Features
...
Feature submission is typically handled by the underlying subsystem responsible for those objects: [EntityRenderer for entities](/docs/1.21.10/entities/renderer#entity-renderers), [BlockEntityRenderer for block entities](/docs/1.21.10/blockentities/ber#blockentityrenderer), [ParticleGroupRenderState for particles](#TODO), etc. Each provides their own `submit` method, usually taking in some general render state of the object. The necessary elements are then submitted through the `SubmitNodeCollector` and stored in a `SubmitNodeCollection` tree map for rendering.
...
| submitModel | Models with render states, transparency sorted. |
| submitModelPart | ModelParts. |
