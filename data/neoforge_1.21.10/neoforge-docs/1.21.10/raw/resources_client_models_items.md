...
title: "Items"
version: "1.21.10"
pageId: "resources/client/models/items"
url: "https://docs.neoforged.net/docs/1.21.10/resources/client/models/items/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:16.562Z"
---
# Client Items

Client Items are the in-code representation of how an `ItemStack` should be submitted for rendering within the game, specifying what models to use given what state. The client items are located within the `items` subdirectory within the [assets folder](/docs/1.21.10/resources/#assets), specified by the relative location within `DataComponents#ITEM_MODEL`. By default, this is the registry name of the object (e.g. `minecraft:apple` would be located at `assets/minecraft/items/apple.json` by default).

The client items are stored within the `ModelManager`, which can be accessed through `Minecraft.getInstance().modelManager`. Then, you can call `ModelManager#getItemModel` or `getItemProperties` to get the client item information by its [ResourceLocation](/docs/1.21.10/misc/resourcelocation).

> **Warning**
> warning

These are not to be confused with the actual [models that are baked and actually rendered](/docs/1.21.10/resources/client/models/modelsystem) in-game.
...
More information about how item models are submitted for rendering can be found [below](#manually-rendering-an-item).

## A Basic Model

The `type` field within `model` determines how to choose the model being submitted to render for the item. The simplest type is handled by `minecraft:model` (or `BlockModelWrapper`), which functionally defines the model JSON being submitted to render, relative to the `models` directory (e.g. `assets//models/.json`).
...
Sometimes, you may want to register multiple models for a single item. While this can be done directly with the [composite model loader](/docs/1.21.10/resources/client/models/modelloaders#composite-model), for item models, there is a custom `minecraft:composite` type which takes a list of models to submit for rendering.
...
Not all models can be represented using the basic model JSON. Some models can have dynamic components, or use existing `Model`s created for a [BlockEntityRenderer](/docs/1.21.10/blockentities/ber). In these instances, there is a special model type which allows the user to specify what [features](/docs/1.21.10/rendering/feature) to submit for rendering. These are known as `SpecialModelRenderer`s, which are defined within `SpecialModelRenderers`.
...
Creating your own `SpecialModelRenderer` is broken into three parts: the `SpecialModelRenderer` instance used to submit the [features](/docs/1.21.10/rendering/feature) used to render the item, the `SpecialModelRenderer.Unbaked` instance used to read and write to JSON, and the registration to use the renderer when as an item or, if necessary, when as a block.
...
Finally, we register the objects to their necessary locations. For the client items, this is done via `RegisterSpecialModelRendererEvent` on the [mod event bus](/docs/1.21.10/concepts/events#event-buses). If the special renderer should also be used as part of a `BlockEntityRenderer`, such as when rendering in some item-like context (e.g., enderman holding the block), then an `Unbaked` version for the block should be registered via `RegisterSpecialBlockModelRendererEvent` on the [mod event bus](/docs/1.21.10/concepts/events#event-buses).
...
For the fluid tint to apply to the fluid texture, the item in question must have a `Capabilities.FluidHandler.ITEM` attached. If your item does not directly use `BucketItem` (not a subtype either), then you need to [register the capability to your item](/docs/1.21.10/inventories/capabilities#registering-capabilities).
...
If you need to submit an item [feature](/docs/1.21.10/rendering/feature), such as in some `BlockEntityRenderer` or `EntityRenderer`, it can be achieved through three steps. First, the renderer in question creates an `ItemStackRenderState` to hold the state of the stack. Then, the `ItemModelResolver` updates the `ItemStackRenderState` using one of its methods to update the state to the current item being submitted. Finally, the item is submitted via `ItemStackRenderState#submit`.
...
[Screens](/docs/1.21.10/rendering/screens#items) use the subclass `TrackingItemStackRenderState` to hold model identity elements for caching the rendered state across frames.
...
Then, we register the map codec via `RegisterItemModelsEvent` on the [mod event bus](/docs/1.21.10/concepts/events#event-buses).
