...
url: "https://docs.neoforged.net/docs/1.21.10/resources/server/advancements/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:50:22.024Z"
---
# Advancements
...
`icon`: A [JSON representation of an item stack](/docs/1.21.10/items/#json-representation).
- `title`: A [text component](/docs/1.21.10/resources/client/i18n#components) to use as the advancement's title.
- `description`: A [text component](/docs/1.21.10/resources/client/i18n#components) to use as the advancement's description.
...
- `recipes`: A list of [recipe](/docs/1.21.10/resources/server/recipes/) IDs to unlock.
- `loot`: A list of [loot tables](/docs/1.21.10/resources/server/loottables/) to roll and give to the player.
...
<li class="">`neoforge:conditions`: NeoForge-added. A list of [conditions](/docs/1.21.10/resources/server/conditions) that must be passed for the advancement to be loaded. Optional.</li>
</ul>
...
Triggers must be registered to the `Registries.TRIGGER_TYPE` [registry](/docs/1.21.10/concepts/registries#methods-for-registering):
...
And then, triggers must define a [codec](/docs/1.21.10/datastorage/codecs) to serialize and deserialize the trigger instance by overriding `#codec`. This codec is typically created as a constant within the instance implementation.
...
Advancements can be [datagenned](/docs/1.21.10/resources/#data-generation) using an `AdvancementProvider`. An `AdvancementProvider` accepts a list of `AdvancementSubProviders`s, which actually generate the advancements using `Advancement.Builder`.
