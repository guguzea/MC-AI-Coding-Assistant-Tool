...
url: "https://docs.neoforged.net/docs/1.21.10/datastorage/valueio/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:28.087Z"
---
# Value I/O

The Value I/O system is a standardized serialization method to manipulate data of some backing object, such as [CompoundTags for NBT](/docs/1.21.10/datastorage/nbt).
...
[Codecs](/docs/1.21.10/datastorage/codecs) can also be used to store and read values from the value I/O. In vanilla, all `Codec`s are handled using a `RegistryOps`, allowing the storage of datapack entries. `ValueOutput#store` and `storeNullable` take in the key, the codec to write the object, and the object itself. `storeNullable` will not write anything if the object is `null`. `ValueInput#read` can read the object by taking in the key and the codec, returning an `Optional`-wrapped object.
...
`ValueIOSerializable` is a NeoForge-added interface for objects that can be serialized and deserialized using value I/Os. NeoForge uses this API to handle [data attachments](/docs/1.21.10/datastorage/attachments). The interface provides two methods: `serialize` to write the object to a `ValueOutput`, and `deserialize` to read the object from a `ValueInput`.
...
Value I/O for [NBTs](/docs/1.21.10/datastorage/nbt) is handled via `TagValueOutput` and `TagValueInput`.
