...
url: "https://docs.neoforged.net/docs/1.21.10/datastorage/codecs/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:01.850Z"
---
# Codecs
...
To handle registry entries, Minecraft provides `RegistryOps`, which contains a lookup provider to get available registry elements. These can be created by `RegistryOps#create` that takes in the `DynamicOps` with the specific type to store the data within and the lookup provider containing access to the available registries. NeoForge extends `RegistryOps` to create `ConditionalOps`: a registry codec lookup that can handle [conditions to load the entry](/docs/1.21.10/resources/server/conditions).
...
Unbounded maps only support keys that encode/decode to/from strings. A key-value [pair](#pair) list codec can be used to get around this restriction.
...
This can be used in conjunction with a [transformer](#transformer-codecs) to get a specific object from two different methods of encoding.

#### Xor

`Codec#xor` is a special case of the [either](#either) codec where a result is only successful if one of the two methods are processed successfully. If both codecs can be processed, then an error is thrown instead.
