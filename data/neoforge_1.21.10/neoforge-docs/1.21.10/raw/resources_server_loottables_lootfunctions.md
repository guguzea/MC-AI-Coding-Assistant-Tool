...
url: "https://docs.neoforged.net/docs/1.21.10/resources/server/loottables/lootfunctions/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:26.561Z"
---
# Loot Functions

Loot functions can be used to modify the result of a [loot entry](/docs/1.21.10/resources/server/loottables/#loot-entry), or the multiple results of a [loot pool](/docs/1.21.10/resources/server/loottables/#loot-pool) or [loot table](/docs/1.21.10/resources/server/loottables/#loot-table). In both cases, a list of functions is defined, which is run in order. During datagen, loot functions can be applied to `LootPoolSingletonContainer.Builder`s, `LootPool.Builder`s and `LootTable.Builder`s by calling `#apply`. This article will outline the available loot functions. To create your own loot functions, see [Custom Loot Functions](/docs/1.21.10/resources/server/loottables/custom#custom-loot-functions).
...
All vanilla loot functions except `minecraft:sequence` can specify [loot conditions](/docs/1.21.10/resources/server/loottables/lootconditions) in a `conditions` block. If one of these conditions fails, the function will not be applied. On the code side, this is controlled by the `LootItemConditionalFunction`, which all loot functions except for `SequenceFunction` extend.
...
Sets an item count to use in the result item stack. Uses a [number provider](/docs/1.21.10/resources/server/loottables/#number-provider).
...
During datagen, call `SetCustomDataFunction#setCustomData` with the desired [CompoundTag](/docs/1.21.10/datastorage/nbt) to construct a builder for this function.
...
Copies custom NBT data from a block entity or entity source to the item stack. Use of this is discouraged for block entities, use `minecraft:copy_components` or `minecraft:set_contents` instead. For entities, this requires setting the [entity target](/docs/1.21.10/resources/server/loottables/#entity-targets). Requires the loot parameter corresponding to the specified source (entity target or block entity), no modification is performed if that parameter is absent.
...
Sets [data component](/docs/1.21.10/items/datacomponents) values on the item stack. Most vanilla use cases have specialized functions that are explained below.
...
Copies [data component](/docs/1.21.10/items/datacomponents) values from a block entity to the item stack. Requires one of the loot parameters from `LootContext.BlockEntityTarget`, `LootContext.EntityTarget`, or `LootContext.ItemStackTarget`.
...
During datagen, call `CopyComponentsFunction#copyComponentsFromBlockEntity` for a block entity source or `copyComponentsFromEntity` for an entity source to construct a builder for this function. You can alternatively use `CopyComponentsFunction.ItemStackSource`, provided you [widen the access](/docs/1.21.10/advanced/accesstransformers) of the builder constructor.

## minecraft:copy_state

Copies block state properties into the item stack's `block_state` [data component](/docs/1.21.10/items/datacomponents), used when trying to place a block. The block state properties to copy must be explicitly specified. Requires the `minecraft:block_state` loot parameter, no modification is performed if that parameter is absent.
...
Sets a name for the result item stack. The name can be a [Component](/docs/1.21.10/resources/client/i18n#components) instead of a literal string. It can also be resolved from an [entity target](/docs/1.21.10/resources/server/loottables/#entity-targets). Requires the corresponding entity loot parameter if applicable, no modification is performed if that parameter is absent.
...
Copies an [entity target](/docs/1.21.10/resources/server/loottables/#entity-targets)'s or block entity's name into the result item stack. Requires the loot parameter corresponding to the specified source (entity target or block entity), no modification is performed if that parameter is absent.
...
Sets lore (tooltip lines) for the result item stack. The lines can be [Component](/docs/1.21.10/resources/client/i18n#components)s instead of literal strings. It can also be resolved from an [entity target](/docs/1.21.10/resources/server/loottables/#entity-targets). Requires the corresponding entity loot parameter if applicable, no modification is performed if that parameter is absent.
...
Randomly enchants the item stack with a given amount of levels. Uses a [number provider](/docs/1.21.10/resources/server/loottables/#number-provider).
...
Increases the item stack count based on the enchantment value. Uses a [number provider](/docs/1.21.10/resources/server/loottables/#number-provider). Requires the `minecraft:attacking_entity` loot parameter, no modification is performed if that parameter is absent.
...
Sets a durability damage value on the result item stack. Uses a [number provider](/docs/1.21.10/resources/server/loottables/#number-provider).
...
Adds a list of [attribute modifiers](/docs/1.21.10/entities/attributes#attribute-modifiers) to the result item stack.
...
Sets an ominous bottle amplifier on the result item stack. Uses a [number provider](/docs/1.21.10/resources/server/loottables/#number-provider).
...
Sets the player head owner on the result item stack based on the given [entity target](/docs/1.21.10/resources/server/loottables/#entity-targets). Requires the corresponding loot parameter, no modification is performed if that parameter is absent.
