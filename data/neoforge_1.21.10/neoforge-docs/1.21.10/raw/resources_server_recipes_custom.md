...
url: "https://docs.neoforged.net/docs/1.21.10/resources/server/recipes/custom/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:28.440Z"
---
# Custom Recipes
...
`SlotDisplay`s represent the information on what should render in what slot when viewed by a recipe consumer, like a recipe book. A `SlotDisplay` has two methods. First there's `resolve`, which takes in the `ContextMap` containing the available registries and fuel values (as shown in `SlotDisplayContext`); and the current `DisplayContentsFactory`, which accepts the contents to display for this slot; and returns the transformed list of contents into the output to be accepted. Then there's `type`, which holds the [MapCodec](/docs/1.21.10/datastorage/codecs) and [StreamCodec](/docs/1.21.10/networking/streamcodecs) used to encode/decode the display.

`SlotDisplay`s are typically implemented on the [Ingredient via #display, or ICustomIngredient#display for modded ingredients](/docs/1.21.10/resources/server/recipes/ingredients); however, in some cases, the input may not be an ingredient, meaning a `SlotDisplay` will need to use one available, or have a new one created.
...
Then, with that, we can create a new `SlotDisplay`. The `SlotDisplay.Type` must be [registered](/docs/1.21.10/concepts/registries#methods-for-registering):
...
A `RecipeDisplay` is the same as a `SlotDisplay`, except that it represents an entire recipe. The default interface only keeps track of the `result` of recipe and the `craftingStation` which represents the workbench where the recipe is applied. The `RecipeDisplay` also has a `type` that holds the [MapCodec](/docs/1.21.10/datastorage/codecs) and [StreamCodec](/docs/1.21.10/networking/streamcodecs) used to encode/decode the display. However, no available subtypes of `RecipeDisplay` contain all the information required to properly render our recipe on the client. As such, we will need to create our own `RecipeDisplay`.
...
Next up, our recipe type. This is fairly straightforward because there's no data other than a name associated with a recipe type. They are one of two [registered](/docs/1.21.10/concepts/registries#methods-for-registering) parts of the recipe system, so like with all other registries, we create a `DeferredRegister` and register to it:
...
A recipe serializer provides two codecs, one map codec and one stream codec, for serialization from/to JSON and from/to network, respectively. This section will not go in depth about how the codecs work, please see [Map Codecs](/docs/1.21.10/datastorage/codecs) and [Stream Codecs](/docs/1.21.10/networking/streamcodecs) for more information.
...
In our case, however, we want to apply the recipe when an item is right-clicked on a block. We will do so using an [event handler](/docs/1.21.10/concepts/events). Keep in mind that this is an example implementation, and you can alter this in any way you like (so long as you run it on the server). As we want the interaction state to match on both the client and server, we will also need to [sync any relevant input states across the network](/docs/1.21.10/networking/payload).
...
Alternatively, you can sync the [full recipe to the client instead](/docs/1.21.10/resources/server/recipes/#client-side-recipes):
...
And now, during [datagen](/docs/1.21.10/resources/server/recipes/#data-generation), you can call on your recipe builder like any other:
