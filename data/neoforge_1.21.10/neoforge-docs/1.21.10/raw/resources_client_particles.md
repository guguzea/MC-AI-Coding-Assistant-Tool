...
url: "https://docs.neoforged.net/docs/1.21.10/resources/client/particles/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:18.406Z"
---
# Particles

Particles are visual effects commonly spawned using their associated particle type. They can be spawned both client and server [side](/docs/1.21.10/concepts/sides), but being mostly visual in nature, critical parts exist only on the physical (and logical) client side.

This article covers the construction and use of particle types and particle descriptions. For more rendering-specific information, see the companion [client particles](/docs/1.21.10/rendering/particles) article.

## Registering ParticleTypes
...
To allow a [particle](/docs/1.21.10/rendering/particles) to make use of its description, the `ParticleType` must be associated with a [ParticleProvider](/docs/1.21.10/rendering/particles#particleprovider) that takes in the `SpriteSet` using the [client-side](/docs/1.21.10/concepts/sides) [mod bus](/docs/1.21.10/concepts/events#event-buses) [event](/docs/1.21.10/concepts/events) `RegisterParticleProvidersEvent`:
...
Particle definition files can also be [datagenned](/docs/1.21.10/resources/#data-generation) by extending `ParticleDescriptionProvider` and overriding the `#addDescriptions()` method:
...
As a reminder from before, the server only knows [ParticleTypes](#registering-particletypes) and [ParticleOptions](#custom-particletypes), while the client works directly with `Particle`s provided by `ParticleProvider`s that are associated with a `ParticleType`. Consequently, the ways in which particles are spawned are vastly different depending on the side you are on.
