...
url: "https://docs.neoforged.net/docs/1.21.10/datastorage/attachments/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:01.214Z"
---
# Data Attachments

The data attachment system allows mods to attach and store additional data on block entities, chunks, entities, and levels.

*To store additional level data, you can use [SavedData](/docs/1.21.10/datastorage/saveddata).*

> **Note**
> note

Data attachments for item stacks have been superceeded by vanilla's [data components](/docs/1.21.10/items/datacomponents).
...
There are a few ways to provide an attachment serializer: directly implementing `IAttachmentSerializer`, implementing [ValueIOSerializable](/docs/1.21.10/datastorage/valueio#valueioserializable) and using the static `AttachmentType#serializable` method to create the builder, or providing a map codec to the builder.
...
The two other overloads which delegate to `AttachmentSyncHandler` take in a [StreamCodec](/docs/1.21.10/networking/streamcodecs) for `read` and `write`, and an optional predicate for `sendToPlayer`.
...
By default, [entity](/docs/1.21.10/entities/) data attachments are not copied on player death. To automatically copy an attachment on player death, set `copyOnDeath` in the attachment builder.
