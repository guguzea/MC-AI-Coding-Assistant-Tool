...
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:29.702Z"
---
# Tags
...
To reference tags in code, you must create a `TagKey`, where `T` is the type of tag (`Block`, `Item`, `EntityType`, etc.), using a [registry key](/docs/1.21.10/misc/resourcelocation#resourcekeys) and a [resource location](/docs/1.21.10/misc/resourcelocation):
...
A custom tag provider, whether for an existing or custom [registry](/docs/1.21.10/concepts/registries), can be created by simply extending `TagsProvider`, where `T` is the registry object you are generating a tag for.
