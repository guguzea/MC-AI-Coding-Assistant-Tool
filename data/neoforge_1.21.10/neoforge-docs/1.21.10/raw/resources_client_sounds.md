...
url: "https://docs.neoforged.net/docs/1.21.10/resources/client/sounds/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:19.010Z"
---
# Sounds
...
`SoundEvent`s are [registered objects](/docs/1.21.10/concepts/registries), meaning that they must be registered to the game through a `DeferredRegister` and be singletons:
...
Of course, don't forget to add your registry to the [mod event bus](/docs/1.21.10/concepts/events#event-buses) in the [mod constructor](/docs/1.21.10/gettingstarted/modfiles#javafml-and-mod):
...
*See also: [sounds.json](https://minecraft.wiki/w/Sounds.json) on the [Minecraft Wiki](https://minecraft.wiki)*
...
<li class="">`playSound(Entity entity, double x, double y, double z, SoundEvent soundEvent, SoundSource soundSource, float volume, float pitch)`

- Forwards to `playSeededSound` with a random seed selected and the holder wrapped around the `SoundEvent`

</li>
<li class="">`playSound(Entity entity, BlockPos pos, SoundEvent soundEvent, SoundSource soundSource, float volume, float pitch)`

- Forwards to the above method with `x`, `y` and `z` taking the values of `pos.getX() + 0.5`, `pos.getY() + 0.5` and `pos.getZ() + 0.5`, respectively.

</li>
<li class="">`playLocalSound(double x, double y, double z, SoundEvent soundEvent, SoundSource soundSource, float volume, float pitch, boolean distanceDelay)`
...
<li class="">`playPlayerSound(SoundEvent soundEvent, SoundSource soundSource, float volume, float pitch)`
...
Sound files themselves can of course not be [datagenned](/docs/1.21.10/resources/#data-generation), but `sounds.json` files can. To do so, we extend `SoundDefinitionsProvider` and override the `registerSounds()` method:
