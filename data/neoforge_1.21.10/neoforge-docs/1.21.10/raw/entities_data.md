...
url: "https://docs.neoforged.net/docs/1.21.10/entities/data/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:31.512Z"
---
# Data and Networking
...
- `EntityDataSerializer`s are basically wrappers around a [StreamCodec](/docs/1.21.10/networking/streamcodecs).

Minecraft uses a hard-coded map of serializers. NeoForge transforms this map into a registry, meaning that if you want to add new `EntityDataSerializer`s, they must be added by [registration](/docs/1.21.10/concepts/registries#methods-for-registering).
- Minecraft defines various default `EntityDataSerializer`s in the `EntityDataSerializers` class.

</li>
<li class="">`EntityDataAccessor`s are held by the entity and are used to get and set the data values.</li>
<li class="">`SynchedEntityData` itself holds all `EntityDataAccessor`s for an entity, and automatically calls on the `EntityDataSerializer`s to sync values as needed.</li>
</ul>
...
These two methods are used to read and write data to disk. They work by loading/saving your values from/to a [value I/O](/docs/1.21.10/datastorage/valueio), like so:
...
Please refer to the [Networking articles](/docs/1.21.10/networking/) for more information on custom network packets.

## Data Attachments

Entities have been patched to extend `AttachmentHolder` and as such support data storage via [data attachments](/docs/1.21.10/datastorage/attachments). Its main use is to define custom data on entities that are not your own, i.e., entities added by Minecraft or other mods. Please see the linked article for more information.

## Custom Network Messages

For syncing, you can also always opt to use a custom packet to send additional information when needed. Please refer to the [Networking articles](/docs/1.21.10/networking/) for more information.
