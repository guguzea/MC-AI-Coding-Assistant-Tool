...
url: "https://docs.neoforged.net/docs/1.21.10/resources/server/enchantments/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:22.597Z"
---
# Enchantments

Enchantments are special effects that can be applied to tools and other items. As of 1.21, enchantments are stored on items as [Data Components](/docs/1.21.10/items/datacomponents), are defined in JSON, and are comprised of so-called enchantment effect components. During the game, the enchantments on a particular item are contained within the `DataComponents.ENCHANTMENTS` component, in an `ItemEnchantments` instance.
...
Enchantment effect components are specially-registered [Data Components](/docs/1.21.10/items/datacomponents) that determine how an enchantment functions. The type of the component defines its effect, while the data it contains is used to inform or modify that effect. For instance, the `minecraft:damage` component modifies the damage that a weapon deals by an amount determined by its data.

Vanilla defines various [built-in enchantment effect components](/docs/1.21.10/resources/server/enchantments/builtin), which are used to implement all vanilla enchantments.
...
Enchantment effect component types must be [registered](/docs/1.21.10/concepts/registries) to `BuiltInRegistries.ENCHANTMENT_EFFECT_COMPONENT_TYPE`, which takes a `DataComponentType`. For example, you could register an enchantment effect component that can store an `Increment` object as follows:
...
Wrapping the type in `ConditionalEffect` allows the enchantment effect component to optionally take effect based on a given [LootContext](/docs/1.21.10/resources/server/loottables/#loot-context).
...
Enchantment JSON files can be created automatically using the [data generation](/docs/1.21.10/resources/#data-generation) system by passing a `RegistrySetBuilder` into `DatapackBuiltInEntriesProvider` via `GatherDataEvent#createDatapackRegistryObjects`. The JSON will be placed in `/src/generated/data//enchantment/.json`.
