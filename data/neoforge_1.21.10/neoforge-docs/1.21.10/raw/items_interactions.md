...
url: "https://docs.neoforged.net/docs/1.21.10/items/interactions/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:44.494Z"
---
# Interactions
...
A hit result can be of one of three types, represented through the `HitResult.Type` enum: `BLOCK`, `ENTITY`, or `MISS`. A `HitResult` of type `BLOCK` can be cast to `BlockHitResult`, while a `HitResult` of type `ENTITY` can be cast to `EntityHitResult`; both types provide additional context about what [block](/docs/1.21.10/blocks/) or [entity](/docs/1.21.10/entities/) was hit. If the type is `MISS`, this indicates that neither a block nor an entity was hit, and should not be cast to either subclass.

Every frame on the [physical client](/docs/1.21.10/concepts/sides#the-physical-side), the `Minecraft` class updates and stores the currently looked-at `HitResult` in the `hitResult` field. This field can then be accessed through `Minecraft.getInstance().hitResult`.

## Left-Clicking an Item

- It is checked that all required [feature flags](/docs/1.21.10/advanced/featureflags) for the [ItemStack](/docs/1.21.10/items/#itemstacks) in your main hand are enabled. If this check fails, the pipeline ends.
- `InputEvent.InteractionKeyMappingTriggered` is fired with the left mouse button and the main hand. If the [event](/docs/1.21.10/concepts/events) is [canceled](/docs/1.21.10/concepts/events#cancellable-events), the pipeline ends.
- Depending on what you are looking at (using the [HitResult](#hitresults) in `Minecraft`), different things happen:

If you are looking at an [entity](/docs/1.21.10/entities/) that is within your reach:
...
- Entity base damage (the value of the `minecraft:attack_damage` [attribute](/docs/1.21.10/entities/attributes)) and enchantment bonus damage are calculated as two separate floats. If both are 0, the pipeline ends.

Note that this excludes [attribute modifiers](/docs/1.21.10/entities/attributes#attribute-modifiers) from the main hand item, these are added after the check.
...
<li class="">[Entity#hurtOrSimulate](/docs/1.21.10/entities/#damaging-entities) is called. If it returns false, the pipeline ends.</li>
...
<li class="">If you are looking at a [block](/docs/1.21.10/blocks/) that is within your reach:<!-- -->

- The [block breaking sub-pipeline](/docs/1.21.10/blocks/#breaking-a-block) is initiated.
...
- `InputEvent.InteractionKeyMappingTriggered` is fired with the right mouse button and the main hand. If the [event](/docs/1.21.10/concepts/events) is [canceled](/docs/1.21.10/concepts/events#cancellable-events), the pipeline ends.
- Several circumstances are checked, for example that you are not in spectator mode or that all required [feature flags](/docs/1.21.10/advanced/featureflags) for the [ItemStack](/docs/1.21.10/items/#itemstacks) in your main hand are enabled. If at least one of these checks fails, the pipeline ends.
- Depending on what you are looking at (using the [HitResult](#hitresults) in `Minecraft`), different things happen:

If you are looking at an [entity](/docs/1.21.10/entities/) that is within your reach and not outside the world border:
...
- If you want to add behavior for your own entity, override this method. If you want to add behavior for a vanilla entity, use the event.
- For [Mobs](/docs/1.21.10/entities/livingentity), the override of `Entity#interact` handles things like leashing and spawning babies when the `ItemStack` in your main hand is a spawn egg, and then defers mob-specific handling to `Mob#mobInteract`. The rules for results for `Entity#interact` apply here as well.
...
<li class="">If you are looking at a [block](/docs/1.21.10/blocks/) that is within your reach and not outside the world border:<!-- -->
...
<li class="">If the Control key is held down, the player is in creative and the targeted block has a [BlockEntity](/docs/1.21.10/blockentities/):<!-- -->
