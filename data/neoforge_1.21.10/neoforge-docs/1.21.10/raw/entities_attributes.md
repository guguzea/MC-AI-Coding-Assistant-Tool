...
url: "https://docs.neoforged.net/docs/1.21.10/entities/attributes/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:07:30.650Z"
---
# Attributes

Attributes are special fields of [living entities](/docs/1.21.10/entities/livingentity) that determine basic properties such as max health, speed or armor. All attributes are stored as double values and synced automatically. Vanilla offers a wide range of default attributes, and you can also add your own.
...
| armor_toughness | ARMOR_TOUGHNESS | [0,20] | 0 | The armor toughness value of the entity. See Armor Toughness on the Minecraft Wiki for more information. |
...
| block_break_speed | BLOCK_BREAK_SPEED | [0,1024] | 1 | How fast the entity can mine blocks, as a multiplicative modifier. See Mining Speed for more information. |
...
| luck | LUCK | [-1024,1024] | 0 | The luck value of the entity. This is used when rolling loot tables to give bonus rolls or otherwise modify the resulting items' quality. |
...
| mining_efficiency | MINING_EFFICIENCY | [0,1024] | 0 | How fast the entity can mine blocks, as an additive modifier, only if the used tool is correct. See Mining Speed for more information. |
...
| submerged_mining_speed | SUBMERGED_MINING_SPEED | [0,20] | 0.2 | How fast the entity can mine blocks, as a multiplicative modifier, only if the entity is underwater. See Mining Speed for more information. |
...
When creating a `LivingEntity`, it is required to register a set of default attributes for them. When an entity is [spawned](/docs/1.21.10/entities/#spawning-entities) in, its default attributes are set on it. Default attributes are registered in the [EntityAttributeCreationEvent](/docs/1.21.10/concepts/events) like so:
...
In some situations, for example when making [your own attributes](#custom-attributes), it is needed to add attributes to an existing entity's `AttributeSupplier`. This is done through the `EntityAttributeModificationEvent` like so:
...
If needed, you can also add your own attributes. Like many other systems, attributes are a [registry](/docs/1.21.10/concepts/registries), and you can register your own objects to it. To get started, create a `DeferredRegister<Attribute>` like so:
