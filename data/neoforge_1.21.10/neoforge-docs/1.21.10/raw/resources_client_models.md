...
url: "https://docs.neoforged.net/docs/1.21.10/resources/client/models/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:08:03.122Z"
---
# Models
...
Items use the associated model(s) defined by its [client item](/docs/1.21.10/resources/client/models/items) while blocks use the associated model in the [blockstate file](#blockstate-files). These locations are relative to the `models` directory, so a model referenced by the name `examplemod:item/example_model` would be defined by the JSON at `assets/examplemod/models/item/example_model.json`.

## Specification

*See also: [Model](https://minecraft.wiki/w/Model) on the [Minecraft Wiki](https://minecraft.wiki)*

A model is a JSON file with the following optional properties in the root tag:

- `loader`: NeoForge-added. Sets a custom model loader. See [Model Loaders](/docs/1.21.10/resources/client/models/modelloaders#model-loaders) for more information.
- `parent`: Sets a parent model, in the form of a [resource location](/docs/1.21.10/misc/resourcelocation) relative to the `models` folder. All parent properties will be applied and then overridden by the properties set in the declaring model. Common parents include:
...
<li class="">`ambientocclusion`: Whether to enable [ambient occlusion](https://en.wikipedia.org/wiki/Ambient_occlusion) or not. Only effective on block models. Defaults to `true`. If your custom block model has weird shading, try setting this to `false`.</li>
<li class="">`render_type`: NeoForge-added. Sets the render type group to use. See [Render Type Groups](#render-types) for more information.</li>
<li class="">`gui_light`: Can be `"front"` or `"side"`. If `"front"`, light will come from the front, useful for flat 2D models. If `"side"`, light will come from the side, useful for 3D models (especially block models). Defaults to `"side"`. Only effective on item models.</li>
<li class="">`textures`: A sub-object that maps names (known as texture variables) to [texture locations](/docs/1.21.10/resources/client/textures). Texture variables can then be used in [elements](#elements). They can also be specified in elements, but left unspecified in order for child models to specify them.<!-- -->
...
<li class="">`elements`: A list of cuboid [elements](#elements).</li>
<li class="">`display`: A sub-object that holds the different display options for different [perspectives](/docs/1.21.10/resources/client/models/modelsystem#perspectives), see linked article for possible keys. Only effective on item models, but often specified in block models so that item models can inherit the display options. Every perspective is an optional sub-object that may contain the following options, which are applied in that order:<!-- -->
...
<li class="">`transform`: See [Root Transforms](#root-transforms).</li>
</ul>
...
- `minecraft:cutout_mipped`: Variant of `minecraft:cutout` that will scale down textures at large distances to avoid visual artifacts ([mipmapping](https://en.wikipedia.org/wiki/Mipmap)). Does not apply the mipmapping to item rendering, as it is usually undesired on items and may cause artifacts. Used for example by leaves.
...
If you want, you can also add your own render type group. To do so, subscribe to the [mod bus](/docs/1.21.10/concepts/events#event-buses) [event](/docs/1.21.10/concepts/events) `RegisterNamedRenderTypesEvent` and `#register` your render type groups. `#register` has three parameters:
...
- `neoforge_data`: See [Extra Face Data](#extra-face-data).
...
<li class="">`scale`: The scale relative to the translated origin. Specified as a three-dimensional vector (`[x, y, z]`) and defaults to `[1, 1, 1]` if absent.</li>
<li class="">`post_rotation` or `right_rotation`: Rotation around the translated origin to be applied after scaling. Defaults to no rotation. Specified the same as `rotation`.</li>
<li class="">`origin`: Origin point used for rotation and scaling. The transformation is also moved here as a final step. Specified either as a three-dimensional vector (`[x, y, z]`) or using one of the three builtin values `"corner"` (= `[0, 0, 0]`), `"center"` (= `[0.5, 0.5, 0.5]`) or `"opposing-corner"` (= `[1, 1, 1]`, default).</li>
</ul>

## Blockstate Files

*See also: [Blockstate files](https://minecraft.wiki/w/Tutorials/Models#Block_states) on the [Minecraft Wiki](https://minecraft.wiki)*
...
`type`: NeoForge-added. Sets a custom block state model loader. See [Block State Model Loaders](/docs/1.21.10/resources/client/models/modelloaders#block-state-model-loaders) for more information.
...
Finally, a `neoforge:definition_type` can specify a custom model loader to register the block state file See [Block State Definition Loaders](/docs/1.21.10/resources/client/models/modelloaders#block-state-definition-loaders) for more information.

## Client Items

[Client items](/docs/1.21.10/resources/client/models/items) are used by the game to assign a model or multiple models to the states of the `ItemStack`. While there are some item-specific fields in the model JSON, client items consume models to render based on context, so most of their information has been moved to their own separate [section](/docs/1.21.10/resources/client/models/items).
...
For item tinting, please see the [relevant section in the client items article](/docs/1.21.10/resources/client/models/items#tinting).

## Registering Standalone Models

Models that are not associated with a block or item in some way, but are still required in other contexts (e.g. [block entity renderers](/docs/1.21.10/blockentities/ber)), can be registered through `ModelEvent.RegisterStandalone`:
