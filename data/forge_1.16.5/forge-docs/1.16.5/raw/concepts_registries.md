...
source: "https://docs.minecraftforge.net/en/1.16.x/concepts/registries/"
sourceType: mkdocs
---
# Registries
...
The `RegistryEvent`s are the second and more flexible way to register objects. These [events](../../events/intro/) are fired after the mod constructors and before the loading of configs.
...
- A field is considered for injection if: it has at least the modifiers `public static`;
- one of the following conditions are true: the **enclosing class** has an `@ObjectHolder` annotation, and the field is `final`, and: the name value is the field’s name; and
