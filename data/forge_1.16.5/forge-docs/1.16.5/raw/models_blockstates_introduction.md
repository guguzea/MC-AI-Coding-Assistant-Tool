...
source: "https://docs.minecraftforge.net/en/1.16.x/models/blockstates/introduction/"
sourceType: mkdocs
---
...
When the game searches for a model corresponding to a block in the world, it takes the [blockstate](../../../blocks/states/) for that position, and then it uses a map within `ModelManager` to find the corresponding `ModelResourceLocation` for it, which then refers to the actual model. `BlockModelShapes` uses the block&rsquo;s registry name as the location of the blockstate JSON. (E.g. block `examplemod:testblock` goes to the `ResourceLocation` `examplemod:testblock`.) The variant string is pieced together from the blockstate&rsquo;s properties.
...
For a better understanding of multiparts, let&rsquo;s look at a variant built fence connection:
...
Now let&rsquo;s view a modern day multipart built fence connection:
...
You can find more explanations and examples of these formats on the [wiki](https://minecraft.gamepedia.com/Model#Block_states).