...
source: "https://docs.readthedocs.net/en/1.16.x/tileentities/tesr/"
sourceType: mkdocs
---
# TileEntityRenderer
...
- <li><code>tileentityIn`: This is the instance of the tile entity being rendered.
- <li>`partialTicks`: The amount of time, in fractions of a tick, that has passed since the last full tick.
- <li>`matrixStackIn`: A stack holding four-dimensional matrix entries offset to the current position of the tile entity.
- <li>`bufferIn`: A rendering buffer able to access a vertex builder.
- <li>`combinedLightIn`: An integer of the current light value on the tile entity.
- <li>`combinedOverlayIn`: An integer set to the current overlay of the tile entity, usually `OverlayTexture#NO_OVERLAY` or 655,360.
