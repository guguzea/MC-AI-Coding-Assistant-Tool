...
1. <li><a name="playerentity-playsound-evp"> <code>playSound(SoundEvent, volume, pitch)` (overriding the one in [`Entity`](#entity-playsound-evp))- <li>Forwards to `World`&rsquo;s [overload (2)](#world-playsound-pxyzecvp), passing in `this` as the player. - <li>**Client Behavior**: Does nothing, see override in [`ClientPlayerEntity`](#clientplayerentity-playsound-evp). - <li>**Server Behavior**: Plays the sound to everyone nearby *except* this player. - <li>**Usage**: See [`ClientPlayerEntity`](#clientplayerentity-playsound-evp).
