...
source: "https://docs.readthedocs.net/en/1.17.x/blocks/states/"
sourceType: mkdocs
---
# Block States
...
You can get and place `BlockState`&rsquo;s in the level using `Level#setBlockAndUpdate(BlockPos, BlockState)` and `Level#getBlockState(BlockState)`. If you are placing a `Block`, call `Block#defaultBlockState()` to get the &ldquo;default&rdquo; state, and use subsequent calls to `BlockState#setValue(Property<T>, T)` as stated above to achieve the desired state.