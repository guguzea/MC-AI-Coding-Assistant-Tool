...
source: "https://docs.readthedocs.net/en/1.14.x/effects/particles/"
sourceType: mkdocs
---
# Particles
...
The `Particle` class is [**client only**](../../concepts/sides/), it does not exist on the server. This means that creating a fixed parameter particle (no context information needed) is somewhat simpler, more on that later. However, if the particle has a parameter with dynamic values, server information is required. For example, breaking blocks create a particle effect based on the broken block&rsquo;s texture. This information obviously depends on the `Blockstate` of the block, which is taken from the server.
...
`IParticleFactory` is [**client only**](../../concepts/sides/), like the `Particle` class. A way of dealing with this is using a handler class and the `EventBusSubscriber` annotation with a dist value.