...
source: "https://docs.readthedocs.net/en/1.14.x/events/intro/"
sourceType: mkdocs
---
# Events
...
An example for a static event listener listening to `RenderWorldLastEvent` which will only be called on the Client: ```java @Mod.EventBusSubscriber(Dist.CLIENT) public class MyStaticClientOnlyEventHandler { @SubscribeEvent public static void drawLast(RenderWorldLastEvent event) { System.out.println("Drawing!"); } } ```
...
The mod event bus is primarily used for listening to lifecycle events in which mods should initialize. Many of these events are also ran in parallel so mods can be initialized at the same time. This does mean you can&rsquo;t directly execute code from other mods in these events, use the `InterModComms` system for that.

These are the four most commonly used lifecycle events that are called during mod initialization on the mod event bus: * FMLCommonSetupEvent * FMLClientSetupEvent & FMLDedicatedServerSetupEvent * InterModEnqueueEvent * InterModProcessEvent
