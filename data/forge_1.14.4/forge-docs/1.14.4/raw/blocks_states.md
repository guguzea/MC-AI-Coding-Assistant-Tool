...
source: "https://docs.readthedocs.net/en/1.14.x/blocks/states/"
sourceType: mkdocs
---
# Block States
...
Each _*property*_ of a block is described by an instance of `IProperty<?>`. Examples of block properties include color (`IProperty<DyeColor>`), facing (`IProperty<Direction`), poweredness (`IProperty<Boolean>`), etc. Each property has the value of the type `T` parametrized by `IProperty<T>`.
