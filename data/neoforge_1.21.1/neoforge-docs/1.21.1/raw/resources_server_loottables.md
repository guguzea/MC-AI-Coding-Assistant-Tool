...
url: "https://docs.neoforged.net/docs/1.21.1/resources/server/loottables/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:53:33.870Z"
---
# Loot Tables
...
- Every subclass of `LivingEntity` that is not in the `MobCategory.MISC` category (as determined by `EntityType#getCategory`) will, by default, receive an associated loot table, located at `:entities/`. This can be changed by overriding `#getLootTable` if you are directly extending `LivingEntity`, or by overriding `#getDefaultLootTable` if you are extending `Mob` or a subclass thereof. For example, sheep use this to roll different loot tables depending on their wool color.
...
- The loot tables for gift items that villagers may throw at players after a raid are defined in the [neoforge:raid_hero_gifts data map](/docs/1.21.1/resources/server/datamaps/builtin#neoforgeraid_hero_gifts).
...
Loot tables should generally only be created for stuff that belongs to your mod. For modifying existing loot tables, [global loot modifiers (GLMs)](/docs/1.21.1/resources/server/loottables/glm) should be used instead.
...
- `conditions`: A list of [loot conditions](/docs/1.21.1/resources/server/loottables/lootconditions) to apply to this loot entry. If one condition fails, the entry is treated as if it weren't present.
- `functions`: A list of [loot functions](/docs/1.21.1/resources/server/loottables/lootfunctions) to apply to the outputs of this loot entry.
...
For modders, it is also possible to define [custom loot entry types](/docs/1.21.1/resources/server/loottables/custom#custom-loot-entry-types).
...
- `conditions`: A list of [loot conditions](/docs/1.21.1/resources/server/loottables/lootconditions) to apply to this loot pool. If one condition fails, none of the loot pool's entries will be rolled.
- `functions`: A list of [loot functions](/docs/1.21.1/resources/server/loottables/lootfunctions) to apply to all loot entry outputs of this loot pool.
...
- `name`: A name for the loot pool. NeoForge-added. This can be used by [GLMs](/docs/1.21.1/resources/server/loottables/glm). If unspecified, this is the hash code of the loot pool, prefixed by `custom#`.
...
Modders can also register [custom number providers](/docs/1.21.1/resources/server/loottables/custom#custom-number-providers) and [custom level-based values](/docs/1.21.1/resources/server/loottables/custom#custom-level-based-values) if needed.

## Loot Parameters

A loot parameter, known internally as a `LootContextParam`, is a parameter provided to a loot table when rolled, where `T` is the type of the provided parameter, for example `BlockPos` or `Entity`. They can be used by [loot conditions](/docs/1.21.1/resources/server/loottables/lootconditions) and [loot functions](/docs/1.21.1/resources/server/loottables/lootfunctions). For example, the `minecraft:killed_by_player` loot condition checks for the presence of the `minecraft:player` parameter.
...
- `minecraft:damage_source`: A [damage source](/docs/1.21.1/resources/server/damagetypes#creating-and-using-damage-sources) associated with the loot table, typically the damage source that killed the entity. Access via `LootContextParams.DAMAGE_SOURCE`.
...
Custom loot parameters can be created by calling `new LootContextParam` with the desired id. Since they are merely resource location wrappers, they do not need to be registered.
...
- `neoforge:conditions`: A list of [data load conditions](/docs/1.21.1/resources/server/conditions). **Warning: These are data load conditions, not [loot conditions](/docs/1.21.1/resources/server/loottables/lootconditions)!**
- `functions`: A list of [loot functions](/docs/1.21.1/resources/server/loottables/lootfunctions) to apply to all loot entry outputs of this loot table.
...
Let's start with getting the loot table itself. We can obtain a loot table using `level.getServer().reloadableRegistries().getLootTable(lootTableId)`. As the loot data is only available through the server, this logic must run on a [logical server](/docs/1.21.1/concepts/sides), not a logical client.
...
`LootTable` additionally exposes a method named `#getRandomItemsRaw`. Unlike the various `#getRandomItems` variants, `#getRandomItemsRaw` method will not apply [global loot modifiers](/docs/1.21.1/resources/server/loottables/glm). Use this method only if you know what you are doing.

## Datagen

Loot tables can be [datagenned](/docs/1.21.1/resources/#data-generation) by registering a `LootTableProvider` and providing a list of `LootTableSubProvider` in the constructor:
