...
platform: "neoforge"
fetchedAt: "2026-09-07T03:53:11.840Z"
---
# BlockEntityRenderer

A `BlockEntityRenderer`, often abbreviated as BER, is used to render [blocks](/docs/1.21.1/blocks/) in a way that cannot be represented with a [static baked model](/docs/1.21.1/resources/client/models/) (JSON, OBJ, others). For example, this could be used to dynamically render container contents of a chest-like block. A block entity renderer requires the block to have a [BlockEntity](/docs/1.21.1/blockentities/), even if the block does not store any data otherwise.
...
`BlockEntityWithoutLevelRenderer`, colloquially known as BEWLR, is an adaptation of the regular `BlockEntityRenderer` for special [item](/docs/1.21.1/items/) rendering (hence "without level", as items do not have level context). Its overall purpose is the same: do special rendering for cases where static models aren't enough.
