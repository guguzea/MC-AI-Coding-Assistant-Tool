...
url: "https://docs.neoforged.net/docs/1.21.1/resources/server/datamaps/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:53:30.234Z"
---
# Data Maps

A data map contains data-driven, reloadable objects that can be attached to a registered object. This system allows for more easily data-driving game behaviour, as they provide functionality such as syncing or conflict resolution, leading to a better and more configurable user experience. You can think of [tags](/docs/1.21.1/resources/server/tags) as registry object ➜ boolean maps, while data maps are more flexible registry object ➜ object maps. Similar to [tags](/docs/1.21.1/resources/server/tags), data maps will add to their corresponding data map rather than overwriting.
...
NeoForge provides various [built-in data maps](/docs/1.21.1/resources/server/datamaps/builtin) for common use cases, replacing hardcoded vanilla fields. More info can be found in the linked article.
...
- `neoforge:conditions`: A list of [loading conditions](/docs/1.21.1/resources/server/conditions).
...
Like many other things, data maps are serialized and deserialized using [codecs](/docs/1.21.1/datastorage/codecs). This means that we need to provide a codec for our data map entry that we will use in a bit:
...
Finally, register the data map during the [RegisterDataMapTypesEvent](/docs/1.21.1/concepts/events) on the [mod event bus](/docs/1.21.1/concepts/events#event-buses):
...
Data maps can be [datagenned](/docs/1.21.1/resources/#data-generation) by extending `DataMapProvider` and overriding `#gather` to create your entries. Reusing the `ExampleData` from before (with float values `amount` and `chance`), our datagen file could look something like this:
