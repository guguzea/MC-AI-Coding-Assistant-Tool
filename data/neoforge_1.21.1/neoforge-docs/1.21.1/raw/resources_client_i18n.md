...
platform: "neoforge"
fetchedAt: "2026-06-01T10:53:26.659Z"
---
# I18n and L10n
...
Some places in Minecraft offer you helper methods to get a translation keys. For example, both blocks and items provide `#getDescriptionId` methods. These can not only be queried, but also overridden if needed. A common use case are items that have different names depending on their [NBT](/docs/1.21.1/datastorage/nbt) value. These will usually override the variant of `#getDescriptionId` that has an [ItemStack](/docs/1.21.1/items/#itemstacks) parameter, and return different values based on the stack's NBT. Another common use case are `BlockItem`s, which override the method to use the associated block's translation key instead.

> **Tip**
> tip

The only purpose of translation keys is for localization. Do not use them for game logic, that's what [registry names](/docs/1.21.1/concepts/registries) are for.

### Translating Mod Metadata

Starting with NeoForge 20.4.179, translation files can override certain parts of [mod info](/docs/1.21.1/gettingstarted/modfiles#modstoml) using the following keys (where `modid` is to be replaced with the actual mod id):
...
Language files can be [datagenned](/docs/1.21.1/resources/#data-generation). To do so, extend the `LanguageProvider` class and add your translations in the `addTranslations()` method:
