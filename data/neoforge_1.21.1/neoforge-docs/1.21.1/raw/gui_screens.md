...
version: "1.21.1"
pageId: "gui/screens"
url: "https://docs.neoforged.net/docs/1.21.1/gui/screens/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:53:16.355Z"
---
# Screens

Screens are typically the base of all Graphical User Interfaces (GUIs) in Minecraft: taking in user input, verifying it on the server, and syncing the resulting action back to the client. They can be combined with [menus](/docs/1.21.1/gui/menus) to create an communication network for inventory-like views, or they can be standalone which modders can handle through their own [network](/docs/1.21.1/networking/) implementations.
...
Strings should typically be passed in as [Components](/docs/1.21.1/resources/client/i18n#components) as they handle a variety of usecases, including the two other overloads of the method.
...
Since screens are subtypes of `GuiEventListener`s, the input handlers can also be overridden, such as for handling logic on a specific [key press](/docs/1.21.1/misc/keymappings#inside-a-gui).
...
To use an `AbstractContainerScreen` with a menu, it needs to be registered. This can be done by calling `register` within the `RegisterMenuScreensEvent` on the [mod event bus](/docs/1.21.1/concepts/events#event-buses).
