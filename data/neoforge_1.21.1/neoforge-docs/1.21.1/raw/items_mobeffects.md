...
platform: "neoforge"
fetchedAt: "2026-09-12T12:02:23.355Z"
---
# Mob Effects & Potions
...
- A `MobEffect` affects an entity every tick. Like [blocks](/docs/1.21.1/blocks/) or [items](/docs/1.21.1/items/), `MobEffect`s are registry objects, meaning they must be [registered](/docs/1.21.1/concepts/registries) and are singletons.
...
<li class="">A `MobEffectInstance` is an instance of a `MobEffect`, with a duration, amplifier and some other properties set (see below). `MobEffectInstance`s are to `MobEffect`s what [ItemStacks](/docs/1.21.1/items/#itemstacks) are to `Item`s.</li>
...
Many effects have their logic applied in other places. For example, the levitation effect is applied in the living entity movement handler. For modded `MobEffect`s, it often makes sense to apply them in an [event handler](/docs/1.21.1/concepts/events). NeoForge also provides a few events related to effects:
...
Potions are traditionally made in the Brewing Stand. Unfortunately, Mojang does not provide [datapack](/docs/1.21.1/resources/#data) support for brewing recipes, so we have to be a little old-fashioned and add our recipes through code via the `RegisterBrewingRecipesEvent` event. This is done like so:
