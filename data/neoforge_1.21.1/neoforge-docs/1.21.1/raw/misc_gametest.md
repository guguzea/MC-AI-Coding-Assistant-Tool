...
platform: "neoforge"
fetchedAt: "2026-09-07T03:53:20.898Z"
---
# Game Tests
...
`RegisterGameTestsEvent` can also register either classes or methods using `#register`. The event listener must be [added](/docs/1.21.1/concepts/events#registering-an-event-handler) to the mod event bus. Test methods registered this way must supply their mod id to `GameTest#templateNamespace` on every method annotated with `@GameTest`.
...
If the buildscript was [setup as recommended](/docs/1.21.1/gettingstarted/#simple-buildgradle-customizations), then only Game Tests under the current mod id would be enabled. To enable other namespaces to load Game Tests from, a run configuration must set the property `neoforge.enabledGameTestNamespaces` to a string specifying each namespace separated by a comma. If the property is empty or not set, then all namespaces will be loaded.
...
<details class="details_lb9f alert alert--info details_b_Ee" data-collapsed="true"><summary>Important infromation on NeoGradle</summary>
