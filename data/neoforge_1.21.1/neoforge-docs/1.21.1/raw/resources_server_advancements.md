...
platform: "neoforge"
fetchedAt: "2026-09-12T12:02:48.043Z"
---
# Advancements
...
`icon`: A [JSON representation of an item stack](/docs/1.21.1/items/#json-representation).
- `text`: A [text component](/docs/1.21.1/resources/client/i18n#components) to use as the advancement's title.
- `description`: A [text component](/docs/1.21.1/resources/client/i18n#components) to use as the advancement's description.
...
- `recipes`: A list of [recipe](/docs/1.21.1/resources/server/recipes/) IDs to unlock.
- `loot`: A list of [loot tables](/docs/1.21.1/resources/server/loottables/) to roll and give to the player.
...
<li class="">`neoforge:conditions`: NeoForge-added. A list of [conditions](/docs/1.21.1/resources/server/conditions) that must be passed for the advancement to be loaded. Optional.</li>
</ul>
...
The `SimpleCriterionTrigger<T>` implementation has two purposes: supplying a method to check trigger instances and run attached listeners on success, and specifying a [codec](/docs/1.21.1/datastorage/codecs) to serialize the trigger instance (`T`).
...
Triggers must be registered to the `Registries.TRIGGER_TYPE` [registry](/docs/1.21.1/concepts/registries#methods-for-registering):
...
And then, triggers must define a [codec](/docs/1.21.1/datastorage/codecs) to serialize and deserialize the trigger instance by overriding `#codec`. This codec is typically created as a constant within the instance implementation.
...
Advancements can be [datagenned](/docs/1.21.1/resources/#data-generation) using an `AdvancementProvider`. An `AdvancementProvider` accepts a list of `AdvancementGenerator`s, which actually generate the advancements using `Advancement.Builder`.
