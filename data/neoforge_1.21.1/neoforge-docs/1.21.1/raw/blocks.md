...
version: "1.21.1"
pageId: "blocks"
url: "https://docs.neoforged.net/docs/1.21.1/blocks/"
platform: "neoforge"
fetchedAt: "2026-06-01T10:53:15.348Z"
---
# Blocks
...
Block types are [MapCodecs](/docs/1.21.1/datastorage/codecs#records) used to serialize and deserialize a block object. This `MapCodec` is set via `BlockBehaviour#codec` and [registered](/docs/1.21.1/concepts/registries#methods-for-registering) to the block type registry. Currently, its only use is when the block list report is being generated. A block type should be created once for every subclass of `Block`. For example, `FlowerBlock#CODEC` represents the block type for most flowers while its subclass `WitherRoseBlock` has a separate block type.
