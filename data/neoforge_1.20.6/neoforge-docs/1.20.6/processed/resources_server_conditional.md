...
All JSON files support conditions; however, only a few data providers have been directly patched for [data generation](/docs/1.20.6/resources/#data-generation). Currently, the following data generators support conditions:
...
Custom conditions can be created by implementing `ICondition` and creating a [map codec](/docs/1.20.6/datastorage/codecs) for it.
...
The `ICondition#codec` method should return the codec used to encode and decode the condition. This codec **must** be [registered](/docs/1.20.6/concepts/registries) to the `NeoForgeRegistries#CONDITION_SERIALIZERS` registry. The name the codec is registered under will be the name used to refer to that condition in the `type` field.
