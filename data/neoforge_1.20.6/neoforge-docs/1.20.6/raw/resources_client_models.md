...
url: "https://docs.neoforged.net/docs/1.20.6/resources/client/models/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:01:43.402Z"
---
# Models
...
- `loader`: NeoForge-added. Sets a custom model loader. See [Custom Model Loaders](/docs/1.20.6/resources/client/models/modelloaders) for more information.
- `parent`: Sets a parent model, in the form of a [resource location](/docs/1.20.6/misc/resourcelocation) relative to the `models` folder. All parent properties will be applied and then overridden by the properties set in the declaring model. Common parents include:
...
- `minecraft:builtin/entity`: Specifies no textures other than `particle`. If this is the parent, [BakedModel#isCustomRenderer()](/docs/1.20.6/resources/client/models/bakedmodel#others) returns `true` to allow use of a [BlockEntityWithoutLevelRenderer](/docs/1.20.6/items/bewlr).
...
<li class="">`textures`: A sub-object that maps names (known as texture variables) to [texture locations](/docs/1.20.6/resources/client/textures). Texture variables can then be used in [elements](#elements). They can also be specified in elements, but left unspecified in order for child models to specify them.<!-- -->
...
<li class="">`display`: A sub-object that holds the different display options for different [perspectives](/docs/1.20.6/resources/client/models/bakedmodel#perspectives), see linked article for possible keys. Only effective on item models, but often specified in block models so that item models can inherit the display options. Every perspective is an optional sub-object that may contain the following options, which are applied in that order:<!-- -->
...
If you want, you can also add your own render types. To do so, subscribe to the [mod bus](/docs/1.20.6/concepts/events#event-buses) [event](/docs/1.20.6/concepts/events) `RegisterNamedRenderTypesEvent` and `#register` your render types. `#register` has three or four parameters:
...
The code side of things is pretty simple. Assuming that we want to add a property named `examplemod:property` to our item, we would use the following code in a [client-side](/docs/1.20.6/concepts/sides) [event handler](/docs/1.20.6/concepts/events#registering-an-event-handler):
...
Models that are not associated with a block or item in some way, but are still required in other contexts (e.g. [block entity renderers](/docs/1.20.6/blockentities/ber)), can be registered through `ModelEvent.RegisterAdditional`:
