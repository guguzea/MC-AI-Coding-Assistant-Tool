...
url: "https://docs.neoforged.net/docs/1.20.6/datagen/recipes/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:01:01.424Z"
---
# Recipe Generation
...
After implementation, the provider must be [added](/docs/1.20.6/resources/#data-generation) to the `DataGenerator`.
...
Although [ItemStack outputs](/docs/1.20.6/resources/server/recipes/#recipe-itemstack-result) in recipes are not supported within vanilla recipe builders, the `#save` methods can be implemented to do so. The only purpose of `#getResult` is to determine the name of the recipe when none is provided.
...
[Conditional recipes](/docs/1.20.6/resources/server/conditional) can also be data generated by either using the `#accept` overload that takes in `ICondition`s or by passing in the `RecipeOutput` after calling `#withConditions` to specify the `ICondition`s.
