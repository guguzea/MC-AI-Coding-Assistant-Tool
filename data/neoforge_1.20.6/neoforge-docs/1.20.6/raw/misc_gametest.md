...
url: "https://docs.neoforged.net/docs/1.20.6/misc/gametest/"
platform: "neoforge"
fetchedAt: "2026-09-07T03:57:46.435Z"
---
# Game Tests
...
`RegisterGameTestsEvent` can also register either classes or methods using `#register`. The event listener must be [added](/docs/1.20.6/concepts/events#registering-an-event-handler) to the mod event bus. Test methods registered this way must supply their mod id to `GameTest#templateNamespace` on every method annotated with `@GameTest`.
...
If the buildscript was [setup as recommended](/docs/1.20.6/gettingstarted/#simple-buildgradle-customizations), then only Game Tests under the current mod id would be enabled. To enable other namespaces to load Game Tests from, a run configuration must set the property `neoforge.enabledGameTestNamespaces` to a string specifying each namespace separated by a comma. If the property is empty or not set, then all namespaces will be loaded.
