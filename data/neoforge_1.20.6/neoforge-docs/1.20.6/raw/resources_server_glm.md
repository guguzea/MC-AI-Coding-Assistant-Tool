...
- This will tell NeoForge about your modifiers and works similar to [tags](/docs/1.20.6/resources/server/tags).
...
<li class="">Finally, a map codec to encode and decode your operational class.<!-- -->

- This is [registered](/docs/1.20.6/concepts/registries#methods-for-registering).
...
`type` represents the registry name of the [codec](#the-loot-modifier-map-codec) used to read the associated JSON file. This must always be present.
...
`#codec` returns the registered [map codec](#the-loot-modifier-map-codec) used to encode and decode the modifier to/from JSON.
...
The connector between the JSON and the `IGlobalLootModifier` instance is a [MapCodec<T>](/docs/1.20.6/datastorage/codecs), where `T` represents the type of the `IGlobalLootModifier` to use.

For ease of convenience, a loot conditions codec has been provided for an easy addition to a record-like codec via `LootModifier#codecStart`. This is utilized for [data generation](/docs/1.20.6/datagen/glm) of the associated loot modifier.
