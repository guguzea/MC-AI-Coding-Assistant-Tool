...
url: "https://docs.neoforged.net/docs/1.20.6/datastorage/codecs/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:01:13.902Z"
---
# Codecs
...
To handle registry entries, Minecraft provides `RegistryOps`, which contains a lookup provider to get available registry elements. These can be created by `RegistryOps#create` that takes in the `DynamicOps` with the specific type to store the data within and the lookup provider containing access to the available registries. NeoForge extends `RegistryOps` to create `ConditionalOps`: a registry codec lookup that can handle [conditions to load the entry](/docs/1.20.6/resources/server/conditional).
