...
url: "https://docs.neoforged.net/docs/1.21.11/misc/keymappings/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:09:12.923Z"
---
# Key Mappings
...
A `KeyMapping` can be registered by listening to the `RegisterKeyMappingsEvent` on the [mod event bus](/docs/1.21.11/concepts/events#registering-an-event-handler) only on the physical client and calling `#register`.
...
A `KeyMapping` can be created using it's constructor. The `KeyMapping` takes in a [translation key](/docs/1.21.11/resources/client/i18n#components) defining the name of the mapping, the default input of the mapping, and a `KeyMapping.Category` defining the category the mapping will be put within in the [Controls option menu](https://minecraft.wiki/w/Options#Controls).
...
If you do not own the screen which you are trying to check a **key** for, you can listen to the `Pre` or `Post` events of `ScreenEvent.KeyPressed` on the [game event bus](/docs/1.21.11/concepts/events#registering-an-event-handler) instead.
...
If you do not own the screen which you are trying to check a **mouse** for, you can listen to the `Pre` or `Post` events of `ScreenEvent.MouseButtonPressed` on the [game event bus](/docs/1.21.11/concepts/events#registering-an-event-handler) instead.
