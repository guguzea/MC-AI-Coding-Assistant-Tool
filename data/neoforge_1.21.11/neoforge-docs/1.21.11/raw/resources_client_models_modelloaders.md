...
url: "https://docs.neoforged.net/docs/1.21.11/resources/client/models/modelloaders/"
platform: "neoforge"
fetchedAt: "2026-09-12T12:09:33.185Z"
---
...
To [datagen](/docs/1.21.11/resources/client/models/datagen) this model, use the custom loader class `CompositeModelBuilder`.

> **Warning**
> warning

The composite model loader should not be used for models used by [client items](/docs/1.21.11/resources/client/models/items). Instead, they should use the [composite model](/docs/1.21.11/resources/client/models/items#composite-models) provided in the definition itself.
...
To [datagen](/docs/1.21.11/resources/client/models/datagen) this model, use the custom loader class `ObjModelBuilder`.
...
- A [client-side](/docs/1.21.11/concepts/sides) [event handler](/docs/1.21.11/concepts/events#registering-an-event-handler) for `ModelEvent.RegisterLoaders` that registers the unbaked model loader
- Optional: A [client-side](/docs/1.21.11/concepts/sides) [event handler](/docs/1.21.11/concepts/events#registering-an-event-handler) for `AddClientReloadListenersEvent` for model loaders that cache data about what is being loaded
...
- During model rendering, the `QuadCollection`, along with any other information required by the [client item](/docs/1.21.11/resources/client/models/items) or [block state definition][blockstatedefinition] is used in rendering.
...
Of course, we can also [datagen](/docs/1.21.11/resources/#data-generation) our models. To do so, we need a class that extends `CustomLoaderBuilder`:
...
To use this loader builder, do the following during block (or item) [model datagen](/docs/1.21.11/resources/client/models/datagen):
...
The default implementation of `CustomLoaderBuilder` holds methods for applying visibility. You may choose to use or ignore the `visibility` property in your model loader. Currently, only the [composite model loader](#composite-model) and [OBJ loader](#obj-model) make use of this property.
...
To [datagen](/docs/1.21.11/resources/client/models/datagen) this block state model, use the custom loader class `CompositeBlockStateModelBuilder`.
...
- A [client-side](/docs/1.21.11/concepts/sides) [event handler](/docs/1.21.11/concepts/events#registering-an-event-handler) for `RegisterBlockStateModels` that registers the codec for the unbaked block state model loader

To illustrate how these classes are connected, we will follow a block state model being loaded:

- During definition loading, a block state model within a variant, multipart, or [custom definition](#block-state-definition-loaders) with the `type` property set to your loader is decoded to your `CustomUnbakedBlockStateModel`.
...
Of course, we can also [datagen](/docs/1.21.11/resources/#data-generation) our models. To do so, we need a class that extends `CustomBlockStateModelBuilder`:
...
To use this state definition loader builder, do the following during block (or item) [model datagen](/docs/1.21.11/resources/client/models/datagen):
...
Of course, we can also [datagen](/docs/1.21.11/resources/#data-generation) our definitions. To do so, we need a class that extends `BlockModelDefinitionGenerator`:
