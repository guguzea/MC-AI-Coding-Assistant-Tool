...
url: "https://docs.neoforged.net/docs/1.21.11/entities/renderer/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:39.974Z"
---
# Entity Renderers

Entity renderers are used to define rendering behavior for an entity. They only exist on the [logical and physical client](/docs/1.21.11/concepts/sides).

Entity rendering uses what is known as entity render states. Simply put, this is an object that holds all values that the renderer needs. Every time the entity is rendered, the render state is updated, and then the `#submit` method uses that to submit the desired [features](/docs/1.21.11/rendering/feature) needed to render the entity at a later point in time.
...
Now that we have our entity renderer, we also need to register it and connect it to its owning entity. This is done in [EntityRenderersEvent.RegisterRenderers](/docs/1.21.11/concepts/events) like so:
...
- `LivingEntityRenderer`: The abstract base class for renderers for [living entities](/docs/1.21.11/entities/livingentity). Direct subclasses include `ArmorStandRenderer` and `AvatarRenderer`.
...
`RenderLayer`s define a `#submit` method, which - surprise! - submits the [features](/docs/1.21.11/rendering/feature) required to render the layer. As with most other submit methods, you can basically submit whatever you want in here. However, a very common use case is to submit a separate model in here, for example for armor or similar pieces of equipment.
...
The [Blockbench](https://www.blockbench.net/) modeling program is a great help in creating entity models. To do so, choose the Modded Entity option when creating your model in Blockbench.
...
In some scenarios, it is desirable to add to an existing entity renderer, e.g. for rendering additional effects on an existing entity. Most of the time, this will affect living entities, i.e., entities with a `LivingEntityRenderer`. This enables us to add [render layers](#creating-a-render-layer-and-baking-a-layer-definition) to an entity like so:
...
Animations are defined in JSON files located at `assets//neoforge/animations/entity/.json` (so for the [resource location](/docs/1.21.11/misc/identifier) `examplemod:example`, the file would be located at `assets/examplemod/neoforge/animations/entity/example.json`). The format of an animation file is as follows:
...
It is highly recommended to use this system in combination with the [Blockbench](https://www.blockbench.net/) modeling software, which offers an [animation to JSON plugin](https://www.blockbench.net/plugins/animation_to_json).
...
- `minecraft:catmullrom`: Interpolation along a [Catmull-Rom spline](https://en.wikipedia.org/wiki/Cubic_Hermite_spline#Catmull%E2%80%93Rom_spline).
