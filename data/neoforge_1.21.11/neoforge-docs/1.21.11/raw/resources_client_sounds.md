...
url: "https://docs.neoforged.net/docs/1.21.11/resources/client/sounds/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:53.945Z"
---
# Sounds
...
`SoundEvent`s are [registered objects](/docs/1.21.11/concepts/registries), meaning that they must be registered to the game through a `DeferredRegister` and be singletons:
...
Of course, don't forget to add your registry to the [mod event bus](/docs/1.21.11/concepts/events#event-buses) in the [mod constructor](/docs/1.21.11/gettingstarted/modfiles#javafml-and-mod):
...
Sound files themselves can of course not be [datagenned](/docs/1.21.11/resources/#data-generation), but `sounds.json` files can. To do so, we extend `SoundDefinitionsProvider` and override the `registerSounds()` method:
