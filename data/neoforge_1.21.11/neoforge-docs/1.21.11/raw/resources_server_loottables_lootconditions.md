...
url: "https://docs.neoforged.net/docs/1.21.11/resources/server/loottables/lootconditions/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:02:01.003Z"
---
# Loot Conditions

Loot conditions can be used to check whether a [loot entry](/docs/1.21.11/resources/server/loottables/#loot-entry) or [loot pool](/docs/1.21.11/resources/server/loottables/#loot-pool) should be used in the current context. In both cases, a list of conditions is defined; the entry or pool is only used if all conditions pass. During datagen, they are added to a `LootPoolEntryContainer.Builder` or `LootPool.Builder` by calling `#when` with an instance of the desired condition. This article will outline the available loot conditions. To create your own loot conditions, see [Custom Loot Conditions](/docs/1.21.11/resources/server/loottables/custom#custom-loot-conditions).
...
This condition accepts a [number provider](/docs/1.21.11/resources/server/loottables/#number-provider) representing a chance between 0 and 1, and randomly returns true or false depending on that chance. The number provider should generally not return values outside the `[0, 1]` interval.
...
This condition accepts an enchantment id, a [LevelBasedValue](/docs/1.21.11/resources/server/loottables/#number-provider) and a constant fallback float value. If the specified enchantment is present, the `LevelBasedValue` is queried for a value. If the specified enchantment is absent, or no value could be retrieved from the `LevelBasedValue`, the constant fallback value is used. The condition then randomly returns true or false, with the previously determined value denoting the chance that true is returned. Requires the `minecraft:attacking_entity` parameter, falling back to level 0 if absent.
...
This condition accepts a [number provider](/docs/1.21.11/resources/server/loottables/#number-provider) and an `IntRange`, returning true if the result of the number provided is within the range.
...
This condition checks a given `EntityPredicate` against an [entity target](/docs/1.21.11/resources/server/loottables/#entity-targets). The `EntityPredicate` can check the entity type, mob effects, nbt values, equipment, location, etc.
...
This condition checks the [entity target](/docs/1.21.11/resources/server/loottables/#entity-targets)'s scoreboard. Requires the loot parameter corresponding to the specified entity target, always failing if that parameter is absent.
...
This condition only returns true if the surrounding loot table id matches. This is typically used within [global loot modifiers](/docs/1.21.11/resources/server/loottables/glm).
...
This condition only returns true if the item in the `tool` loot context parameter (`LootContextParams.TOOL`), usually the item used to break the block or kill the entity, can perform the specified [ItemAbility](/docs/1.21.11/items/tools#itemabilitys). Requires the `minecraft:tool` loot parameter, always failing if that parameter is absent.
