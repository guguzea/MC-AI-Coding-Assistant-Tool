...
url: "https://docs.neoforged.net/docs/1.21.11/blockentities/ber/"
platform: "neoforge"
fetchedAt: "2026-09-07T04:01:33.816Z"
---
# BlockEntityRenderer

A `BlockEntityRenderer`, often abbreviated as BER, is used to 'render' [blocks](/docs/1.21.11/blocks/) in a way that cannot be represented with a [static baked model](/docs/1.21.11/resources/client/models/) (JSON, OBJ, others). For example, this could be used to dynamically render container contents of a chest-like block. A block entity renderer requires the block to have a [BlockEntity](/docs/1.21.11/blockentities/), even if the block does not store any data otherwise.

BERs directly implements the `BlockEntityRenderer`, which submits its [features](/docs/1.21.11/rendering/feature) for rendering:
...
Now that we have our BER, we also need to register and connect it to its owning block entity. This is done in [EntityRenderersEvent.RegisterRenderers](/docs/1.21.11/concepts/events#registering-an-event-handler) like so:
...
As not all block entities with renderers can be represented by static item models, a special renderer can be created to more dynamically control the process. This is done using [SpecialModelRenderers](/docs/1.21.11/resources/client/models/items#special-models). In these cases, both a special model renderer must be created to submit the desired [features](/docs/1.21.11/rendering/feature), and a corresponding registered special block model renderer for scenarios when the block itself is being submitted for rendering rather than an item variant (e.g., enderman carrying a block).

Please refer to the [client item documentation](/docs/1.21.11/resources/client/models/items#special-models) for more information.
