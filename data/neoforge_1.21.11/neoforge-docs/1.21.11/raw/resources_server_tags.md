...
platform: "neoforge"
fetchedAt: "2026-09-07T04:02:04.747Z"
---
# Tags
...
To reference tags in code, you must create a `TagKey`, where `T` is the type of tag (`Block`, `Item`, `EntityType`, etc.), using a [registry key](/docs/1.21.11/misc/identifier#resourcekeys) and an [identifier](/docs/1.21.11/misc/identifier):
...
A custom tag provider, whether for an existing or custom [registry](/docs/1.21.11/concepts/registries), can be created by simply extending `TagsProvider`, where `T` is the registry object you are generating a tag for.
