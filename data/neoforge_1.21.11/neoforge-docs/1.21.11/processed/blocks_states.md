...
As with blocks, every `BlockState` exists exactly once in memory. This means that `==` can and should be used to compare `BlockState`s. `BlockState` is also a final class, meaning it cannot be extended. **Any functionality goes in the corresponding [Block](/docs/1.21.11/blocks/) class!**
...
### Blockstates vs. [Block Entities](/docs/1.21.11/blockentities/)
