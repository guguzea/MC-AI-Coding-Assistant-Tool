...
Particles are visual effects commonly spawned using their associated particle type. They can be spawned both client and server [side](/docs/1.21.11/concepts/sides), but being mostly visual in nature, critical parts exist only on the physical (and logical) client side.

This article covers the construction and use of particle types and particle descriptions. For more rendering-specific information, see the companion [client particles](/docs/1.21.11/rendering/particles) article.
...
To allow a [particle](/docs/1.21.11/rendering/particles) to make use of its description, the `ParticleType` must be associated with a [ParticleProvider](/docs/1.21.11/rendering/particles#particleprovider) that takes in the `SpriteSet` using the [client-side](/docs/1.21.11/concepts/sides) [mod bus](/docs/1.21.11/concepts/events#event-buses) [event](/docs/1.21.11/concepts/events) `RegisterParticleProvidersEvent`:
...
Particle definition files can also be [datagenned](/docs/1.21.11/resources/#data-generation) by extending `ParticleDescriptionProvider` and overriding the `#addDescriptions()` method:
