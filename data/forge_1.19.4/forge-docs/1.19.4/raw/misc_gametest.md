...
source: "https://docs.readthedocs.net/en/1.19.x/misc/gametest/"
sourceType: mkdocs
---
# Game Tests
...
> **Tip**: Tip The local variable generated by /test pos can specify its reference name by appending it to the end of the command: /test pos <var> # Exports 'final BlockPos <var> = new BlockPos(...);'
...
> **Important**: Important Game Tests are executed every tick until the test is marked as a success. As such, methods which schedule success on a given tick must be careful to always fail on any previous tick.
...
> **Tip**: Tip Any methods annotated using @GameTest are translated into a TestFunction using GameTestRegistry#turnMethodIntoTestFunction. That method can be used as a reference for creating TestFunctions without the use of the annotation.
...
`RegisterGameTestsEvent` can also register either classes or methods using `#register`. The event listener must be [added](../../concepts/events/#creating-an-event-handler) to the mod event bus. Test methods registered this way must supply their mod id to `GameTest#templateNamespace` on every method annotated with `@GameTest`.
...
> **Note**: Note The value supplied to GameTestHolder#value and GameTest#templateNamespace can be different from the current mod id. The configuration within the buildscript would need to be changed.
...
> **Tip**: Tip A structure template can be created and saved using a structure block.
...
> **Note**: Note Subcommands follow the test command: /test <subcommand>.
...
If the buildscript was [setup as recommended](../../gettingstarted/#simple-buildgradle-customizations), then only Game Tests under the current mod id would be enabled. To enable other namespaces to load Game Tests from, a run configuration must set the property `forge.enabledGameTestNamespaces` to a string specifying each namespace separated by a comma. If the property is empty or not set, then all namespaces will be loaded.
...
> **Warning**: Warning There must be no spaces in-between namespaces; otherwise, the namespace will not be loaded correctly.
