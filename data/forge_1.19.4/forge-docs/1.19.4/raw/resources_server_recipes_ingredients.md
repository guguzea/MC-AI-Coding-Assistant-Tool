...
version: "1.19.4"
forgeVersion: "45.2.0"
chapter: "resources/server/recipes/ingredients"
source: "https://docs.minecraftforge.net/en/1.19.x/resources/server/recipes/ingredients/"
sourceType: mkdocs
---
# Ingredients
...
`PartialNBTIngredient`s are a looser version of [`StrictNBTIngredient`](#strictnbtingredient) as they compare against a single or set of items and only keys specified within the share tag (as defined by `IForgeItem#getShareTag`). This can be used by specifying the `type` as `forge:partial_nbt`.
...
> **Tip**: Tip Custom ingredients should subclass AbstractIngredient as it provides some useful abstractions for ease of implementation.
...
> **Tip**: Tip If using FMLCommonSetupEvent to register an ingredient serializer, it must be enqueued to the synchronous work queue via FMLCommonSetupEvent#enqueueWork as CraftingHelper#register is not thread-safe.