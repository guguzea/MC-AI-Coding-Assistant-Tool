...
source: "https://docs.minecraftforge.net/en/1.19.x/gettingstarted/modfiles/"
sourceType: mkdocs
---
# Mod Files
...
The file uses the [Tom’s Obvious Minimal Language, or TOML](https://toml.io/), format. The file must be stored under the `META-INF` folder in the resource directory of the source set you are using (`src/main/resources/META-INF/mods.toml` for the `main` source set). A `mods.toml` file may look something like this:
...
`version` | string | `"1"` | The version of the mod, preferably in a [variation of Maven versioning](../versioning/). When set to `${file.jarVersion}`, it will be replaced with the value of the `Implementation-Version` property in the JAR’s manifest (displays as `0.0NONE` in a development environment). | `"1.19.4-1.0.0.0"`
...
`displayTest` | string | `"MATCH_VERSION"` | See ‘[sides](../../concepts/sides/#writing-one-sided-mods)’. | `"NONE"`
