...
version: "1.19.4"
forgeVersion: "45.2.0"
chapter: "blockentities"
source: "https://docs.readthedocs.net/en/1.19.x/blockentities/"
sourceType: mkdocs
---
# BlockEntities
...
In order to create a `BlockEntity`, you need to extend the `BlockEntity` class. As such, another object is registered instead to easily create and refer to the _*type*_ of the dynamic object. For a `BlockEntity`, these are known as `BlockEntityType`s.
...
To create a <code>BlockEntity` and attach it to a `Block`, the `EntityBlock` interface must be implemented on your `Block` subclass. The method `EntityBlock#newBlockEntity(BlockPos, BlockState)` must be implemented and return a new instance of your `BlockEntity`.

## Storing Data within your `BlockEntity

In order to save data, override the following two methods: ``` BlockEntity#saveAdditional(CompoundTag tag) BlockEntity#load(CompoundTag tag) ``` These methods are called whenever the <code>LevelChunk` containing the `BlockEntity` gets loaded from/saved to a tag. Use them to read and write to the fields in your block entity class.
