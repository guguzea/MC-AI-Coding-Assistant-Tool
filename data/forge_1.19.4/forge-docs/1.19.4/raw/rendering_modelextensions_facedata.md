...
> **Warning**: Warning The four color components are multiplied with the texture’s pixels. Omitting the alpha component is equivalent to making it 0, which will make the geometry fully transparent.
...
> **Note**: Note If the top-level AO flag is set to false, specifying this flag as true on an element or face won’t be able to override the top-level flag. { "ambientocclusion": false, "elements": [ { "forge_data": { "ambient_occlusion": true // Has no effect } } ] }