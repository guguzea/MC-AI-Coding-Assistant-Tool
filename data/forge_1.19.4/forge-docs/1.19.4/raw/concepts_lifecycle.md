...
source: "https://docs.readthedocs.net/en/1.19.x/concepts/lifecycle/"
sourceType: mkdocs
---
# Mod Lifecycle
...
`RegisterEvent` is for [registering objects](../registries/#methods-for-registering) into the registries. The event is fired for each registry.
...
> **Note**: Note There are two other lifecycle events: FMLConstructModEvent, fired directly after mod instance construction but before the RegisterEvent, and FMLLoadCompleteEvent, fired after the InterModComms events, for when the mod loading process is complete.