...
version: "1.19.4"
forgeVersion: "45.2.0"
chapter: "datagen/client/sounds"
source: "https://docs.minecraftforge.net/en/1.19.x/datagen/client/sounds/"
sourceType: mkdocs
---
# Sound Definition Generation

The `sounds.json` file can be generated for a mod by subclassing `SoundDefinitionsProvider` and implementing `#registerSounds`. After implementation, the provider must be [added](../../#data-providers) to the `DataGenerator`.
...
A sound definition can be generated by specifying the sound name and definition via `#add`. The sound name can either be provided from a [`SoundEvent`](../../../gameeffects/sounds/#creating-sound-events), `ResourceLocation`, or string.

> **Warning**: Warning The sound name supplied will always assume the namespace is the mod id supplied to the constructor of the provider. There is no validation performed on the namespace of the sound name!

### `SoundDefinition

The <code>SoundDefinition` can be created using `#definition`. The definition contains the data to define a sound instance.
...
A sound supplied to the <code>SoundDefinition` can be specified using `SoundDefinitionsProvider#sound`. These methods take in the reference of the sound and a `SoundType` if specified.

The `SoundType` can be one of two values:

Sound Type | Definition
--- | ---
`SOUND` | Specifies a reference to the sound located at `assets/<namespace>/sounds/
.ogg`. `EVENT` | Specifies a reference to the name of another sound defined by the `sounds.json`. <p>Each `Sound` created from `SoundDefinitionsProvider#sound` can specify additional configurations on how to load and play the sound provided:
