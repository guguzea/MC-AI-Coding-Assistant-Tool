...
chapter: "gettingstarted/debugprofiler"
source: "https://docs.readthedocs.net/en/1.13.x/gettingstarted/debugprofiler/"
sourceType: mkdocs
---
# Debug Profiler
...
After you&rsquo;ve stopped the debugger it will create a new file, it can be found within the `debug` subdirectory in your run directory. The file name will be formatted with the date and time as `profile-results-yyyy-mm-dd_hh.mi.ss.txt`
...
[02] | tick | 99.31% | 95.81%
--- | --- | --- | ---
The Depth of the section | The Name of the Section | The percentage of time it took in relation to it&rsquo;s parent. For Layer 0 it&rsquo;s the percentage of the time a tick takes, while for Layer 1 it&rsquo;s the percentage of the time its parent takes | The second Percentage tells you how much Time it took from the entire tick.

## Profiling your own code

The Debug Profiler has basic support for `Entity` and `TileEntity`. If you would like to profile something else, you may need to manually create your sections like so: ``` Profiler#startSection(yourSectionName : String); //The code you want to profile Profiler#endSection(); ``` You can obtain the the `Profiler` instance from a `World`, `MinecraftServer`, or `Minecraft` instance. Now you just need to search the File for your section name.