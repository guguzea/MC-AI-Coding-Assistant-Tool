> 来源：https://fabricmc.net/wiki/doku.php?id=tutorial:items
> 版本：1.18.2
> 页面 ID：tutorial:items
> 优先级：⭐
> 抓取源：wiki_raw
...
You'll use the vanilla registry system for registering new content. The basic syntax is `net/minecraft/util/registry/Registry`.//`register`//(Registry Type, `net/minecraft/util/Identifier`, Content). Registry types are stored as static fields in the class_7923 or `net/minecraft/util/registry/Registry` class, and the identifier is what labels your content. Content is an instance of whatever you're adding. Specifically for item, the syntax is `net/minecraft/util/registry/Registry`.//`register`//(class_7923.field_41178, `net/minecraft/util/Identifier`, `net/minecraft/item/Item`). This can be called anywhere as long as it occurs during initialization. The method itself also returns the registered content itself.
...
## Best practice of registering items (before 1.21.2)
...
> :!: Creating these files manually for each item can be tiring. You may refer to [datagen_model](datagen_model) for data generation.
...
```
If you did everything correctly, using the item should now play a sound. You can also [add some tooltips](tooltip) for the item based on your item class.
...
## Next Steps
Try [adding your item to an item group](itemgroup). Your item also does not have a name, so you can also [learn how to add language files](lang).