...
source: "https://docs.minecraftforge.net/en/1.18.x/resources/server/conditional/"
sourceType: mkdocs
---
...
Conditions are specified by setting `type` to the name of the condition as specified by [`IConditionSerializer#getID`](#iconditionserializer).
...
Method | Description
--- | ---
getID | The registry name of the condition. Must be equivalent to [`IConditionSerializer#getID`](#iconditionserializer). Used only for [data generation](../../../datagen/server/recipes/).
test | Returns true if the condition has been satisfied.

> **Note**: Note Every #test has access to some IContext representing the state of the game. Currently, only tags can be obtained from a registry.
...
Method | Description
--- | ---
getID | The registry name of the condition. Must be equivalent to [`ICondition#getID`](#icondition).
read | Reads the condition data from JSON.
write | Writes the given condition data to JSON.

> **Note**: Note Condition serializers are not responsible for writing or reading the type of the serializer, similar to other serializer implementations in Minecraft.
