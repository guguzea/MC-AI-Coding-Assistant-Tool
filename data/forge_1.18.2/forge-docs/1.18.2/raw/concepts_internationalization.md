...
source: "https://docs.readthedocs.net/en/1.18.x/concepts/internationalization/"
sourceType: mkdocs
---
...
Localization will happen in the game&rsquo;s locale. In a Minecraft client the locale is specified by the language settings. On a dedicated server, the only supported locale is `en_us`. A list of available locales can be found on the [Minecraft Wiki](https://minecraft.wiki/w/Language#Available_languages).
...
<code>TranslatableComponent` is a `Component` that is localized and formatted lazily. It is very useful when sending messages to players because it will be automatically localized in their own locale.
