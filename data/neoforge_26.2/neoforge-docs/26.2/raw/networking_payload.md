...
<li class="">The [stream codec](/docs/networking/streamcodecs) is used to read and write the payload to and from the buffer sent across the network</li>
...
The client payload handler has its own event `RegisterClientPayloadHandlersEvent` to protect against code reaching across both logical and physical [sides](/docs/concepts/sides).
...
With your own payloads you can then use those to configure the client and server using [Configuration Tasks](/docs/networking/configuration-tasks).
